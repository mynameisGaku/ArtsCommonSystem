...
`TypedEvent.h`は利用時に必要な補助型もインクルードするため、従来どおり一つのインクルードでも
...
`FTypedEventHandle` はイベント識別子、スロット、世代を保持します。解除後に同じスロットが
...
`FMessageBroker` と `FTimerManager` が正規の公開型です。`CMessageBroker` と
`CTimerManager` は同じ型を指す `using` ソース互換名です。この互換名は別の実装型や
`C` 名の実装シンボルを作りません。

互換名が保証するのは、ヘッダーを読み直して再コンパイルするコードのソース互換性です。
既に作成済みのバイナリとのABI互換性は、配布時のABI検証で別に確認します。
新しいACSコードでは正規の `F` 型を使用します。
