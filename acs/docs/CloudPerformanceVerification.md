...
既存の `CVolumetricClouds::RenderCompute()` は互換性のため残す。遠方座標を扱う新しい呼び出し元は、
ビュー行列と投影行列から作ったカメラ相対逆行列を
`RenderComputeCameraRelative()` へ明示的に渡す。エディター、通常の `CSky`、
...
一致することに加え、空、雲、時間再構成、深度合成、エディター、通常 C++ アダプターの全経路が
同じ契約を使うことを固定する。
