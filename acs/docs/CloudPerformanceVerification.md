...
薄い層を分断しない。
さらに中層の被覆を最大34%だけ細くするくびれを加え、雲底から雲頂まで同じ幅の柱が連続する
状態を避ける。上部のかなとこ被覆はこのくびれとは別に残すため、積乱雲の上部だけが再び
横へ広がる。かなとこ拡張量が0の高さでは未補正被覆とくびれ後被覆を`max`しない。両者は
...
細い垂れだけを抑える。中層の締め処理とは最大値を共有するため、雲底を二重に削らず、
かなとこ帯の薄い縁と上部の横張り出しは変更しない。
...
さらに中層の密度だけは、低密度の端を二乗側へ寄せる締め処理を高さ0.08～0.76へ適用する。
締め量は塔状雲の強さに0.58を掛けた値で、かなとこ帯へ入る高さ0.76以降は0へ戻す。
これにより、雲底から細い密度が連続する縦筋を抑えつつ、上部のかなとこが薄い縁まで一様に
消えることを防ぐ。空間棄却用の形状密度、低詳細度密度、視線密度、遠距離を含む光積分で
同じ処理順を使う。
