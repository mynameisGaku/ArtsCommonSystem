...
自己遮蔽は働くが、地平線付近には帯状の密度が残り、超高品質な雲の合格状態ではない。さらに
カメラを上層より高い約 `11600 m` へ置いて見下ろすと、雲が表示されず一様な地表色になった。
上空描画の欠落は層厚尺度とは別の重大な不具合として、雲殻との交差区間、下向き視線の被覆、
大気と雲の合成順を次に監査する。Editor はまだ上層設定を公開していないため、既存の下層だけで
上空・地平線の回帰を確認し、上層設定の公開も別工程として残す。

環境光の遮蔽にカメラまでの透過率を使うと、同じ雲でも視点を動かしただけで空と地面反射の明るさが
変わる。現在は低詳細度の局所密度 `d` と正規化高度 `h` から局所遮蔽を求める。太陽が高い場合は、
既に積分した太陽方向の光路を空半球の代表光路として再利用する。

`tau_sky = d * (0.35 + 0.65 * (1 - h))`

`tau_ground = d * (0.35 + 0.65 * h)`

`w_sun = smoothstep(0.15, 0.75, sun.y)`

`tau_path = tau_light * w_sun * lerp(0.80, 0.35, h)`

`V_sky = exp(-0.60 * tau_sky - MultiScatterOcclusion * tau_path)`

`V_ground = exp(-0.60 * tau_ground)`

`tau_light` は直接光と同じ `lightDepth * Density * LightExtinction` である。消散係数を掛けない密度積分値を
使うと、既定の `LightExtinction=5` では光路遮蔽だけを5分の1に過小評価する。低い太陽の水平光路は
空半球の遮蔽を過大評価するため、`w_sun` で除外する。空の環境光には `V_sky`、地面反射には
`V_ground` を掛け、カメラ方向の累積透過率は含めない。追加のテクスチャ採取は発生しない。
