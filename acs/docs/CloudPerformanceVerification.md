...
横へ広がる。
3D形状テクスチャは低周波R成分だけでなく、焼き込み済みPerlin–WorleyのG成分も
`0.18`～`0.70`の実分布で近景の立体的な侵食へ使う。最大侵食率は従来の`14%`～`26%`を
維持して雲体の連結性を崩さない。高さ軸は正規化高度へ`2.75`倍で写像し、9.4 km級の雲層でも高さ方向の
低周波形状が変化するようにする。視線側と光採取側は同じ係数を使い、形状だけが高さ方向へ
引き伸ばされる不整合を避ける。
...
密度差を保てる。
等倍追跡では全画素が現在フレームの標本を持つため、時間履歴の混合率を0.42～0.62へ抑える。
これにより移動する雲縁の残像を減らし、低解像度追跡で使う時間再構成の混合率とは分離する。
