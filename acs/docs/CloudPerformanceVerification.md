...
現在との差の最大値 `d_e` を `saturate(220 d_e)` で履歴の現在値優先率へ変換する。差が `0.08` 未満の
安定した画素だけが高速な履歴経路を使い、それを超える画素は空間再構成へ戻る。通常の等倍経路も
従来の `0.42～0.62` の現在値優先率と `d_e` の大きい方を使うため、形状変化をぼかし続けない。
この追加は位相値の差分と線形補間だけで、雲密度・光採取・テクスチャ採取数は増やさない。
