...
既存の多重散乱へ加えている。`h`は雲層内の正規化高さ、`cosA`は視線と太陽方向の内積である。
そのため太陽が高く、雲頂側で、上空から逆向きに見た場合だけ働き、地平線側の前方散乱、雲底、
夜間には影響しない。これは雲頂の未解像拡散を補う局所近似であり、GIや半球経路追跡を実装した
ことを意味しない。単体試験では三条件のいずれかを外すと0になり、雲頂・太陽高度・逆向き視線が
強いほど単調に増えることと、シェーダー式の一致を確認する。
