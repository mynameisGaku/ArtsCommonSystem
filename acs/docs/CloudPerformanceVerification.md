...
視線密度で求めた雲種、降水量、柱の変形量は、侵食まで採取する近距離3点へ共有する。
遠距離5点は毎フレーム作る影キャッシュが各地点の天候、渦、柱形状を採取する。キャッシュを
利用できない場所も、視線位置の値を既定の2500 m層で約3.2 km先まで流用せず、各光標本の天候、雲種、降水、
...
現れることを確認した。一方、通常照明ではこの細部がまだ弱いため、次の監査では多重散乱と
環境光が表面コントラストを埋める割合を補正する。
