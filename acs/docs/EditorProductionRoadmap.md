...
| Prefab / Blueprint | 保存、配置、Apply/Revert、graph undo、3D stable instance ID、root Visible/Enabled/Color overrideとselective Apply/Revert、root component property overrideとselective Revertあり | component selective Apply、child override、nested prefab、variant、conflict/diff | P2 |
...
現在の全体Prefab Applyはsubtree全体を書き戻し、他instanceを再生成する。root 3 propertyはproperty単位Apply/Revertに対応し、root component propertyは差分保存、source更新時の保持、property単位Revertまで対応した。component propertyのselective Applyとchild差分はまだ持たない。

不足:

- root component propertyのselective Apply。
...
- stable instance ID、root Visible/Enabled/Color overrideとselective Revert、root component overrideとselective Revert（実装済み）、child overrideとcomponent selective Apply。
