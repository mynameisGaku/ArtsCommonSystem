# タイルマップ (FTilemap / FTilemapComponent)

格子状にタイル ID を並べてマップを作る仕組み。`FTilemap` が **データ本体**（タイル ID の 2D 配列＋レイヤー＋座標変換）、`FTilemapComponent` がそれを `FScene2D` 上で **アトラステクスチャを使って描画**する `FComponent2D` です。床/壁/水たまりのようなステージ地形を組むとき、当たり判定付きの地面を作るときに使います。
...
ノードに `FTilemapComponent` を付け、`Map()` でデータを初期化してから atlas を割り当てるだけで描画されます。
...
public:
    void OnReady() noexcept override {
        ...
        tm.Map().FillRect(0, 0, 15, 0, FTileId{2});      // 下端 1 列を tile 2 で
...
> `FTilemap` は **非コピー・非ムーブ**です（シーン所有/プール前提）。`FTilemapComponent::Map()` 経由で触るのが基本。
...
| `OnDraw(RenderContext& rc)` | フレームワークが自動で呼ぶ。手動呼び出し不要 |
...
tm.Map().FillRect(0,  0, 15,  0, FTileId{2}); // 下端 石 (cell1)
...
```cpp
const FVec2 origin = node->World().position;
...
- **マップ原点はノードの world 位置**。`FTilemap` 自体は原点 (0,0) 基準で `TileToWorld` を返し、描画/`BuildCollision` 時に `Owner().World().position` を足します。マップ全体を動かしたいときはノードを動かす。
...
- **`acs/samples/58_HelloTilemap/TilemapDemo.cpp`** — Package C デモ。Title→Level のフェード遷移付きで、`FTilemapComponent` が 2×2 アトラスのセルとして枠付きの部屋＋中央の水たまりを描画します（実機スクショ確認済み）。アトラステクスチャを CPU で生成する `MakeTileAtlas` も参照になります。
