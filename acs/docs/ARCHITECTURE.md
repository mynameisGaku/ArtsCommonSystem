...
単一散乱のHenyey-Greenstein位相は、前方用と後方用をレイごとに一度だけ求めます。
各密度標本では、作者指定の前方混合率へ光源までの透過率と現在区間の透過率を掛け、
その値で後方位相から前方位相へ補間します。光が届く薄い表面では作者指定値を保ち、
自己影または現在区間の密度が大きい雲芯では後方散乱へ連続的に寄せます。現在区間の
透過率は不透明度の積分にも再利用するため、追加の`pow`、光採取、texture、公開APIはありません。
