...
保存し、最終合成で全解像度の被覆を一度だけ適用します。視線積分の中心方向と隣接方向は、
実際の描画先寸法と逆ビュー射影行列から復元します。距離別帯域制限は各視線レイに隣接2方向の
復元を追加しますが、ディスパッチ数、視線積分解像度、視線・光の採取数は変えません。

雲密度の視線積分は、各レイについて横・縦の隣接画素との視線差を一度だけ求めます。
時間再構成では実出力画素、通常の縮小描画では縮小先画素を基準にし、各標本のカメラ距離を
掛けて投影画素幅へ変換します。基本形状、房、侵食の帯域制限には、積分間隔と投影画素幅の
広い方を使います。雲層内の進入距離が0でも近傍は積分間隔の細部を保ち、遠方では画素より
細かい密度だけを段階的に除くため、中層の局所形状を一色へ均さずに上空遠景の点状ノイズを
抑えます。追加処理はレイごとの隣接2方向と標本ごとの実数演算だけで、密度テクスチャ採取数、
視線・光の採取数、公開APIは変えません。

カメラが雲殻内にある場合、殻交差は進入距離を`0`とし、視線方向にある最初の層境界までを
連続区間として返します。視線積分は最初の粗い区間だけカメラ直前から細密刻みで確認し、
空の場合だけ粗い探索へ戻ります。最大視程は層厚の1倍を`2.5～16 km`へ収め、層外では
境界から層厚の50%に相当する範囲で、視程の逆数を使って設定距離へ滑らかに戻します。
したがって雲への進入、内部滞在、退出で交差開始位置や遠景が不連続に切り替わりません。

単一散乱のHenyey-Greenstein位相は`1/(4PI)`を含み、全立体角の積分が1になる形で使います。
Lambert面の`1/PI`へ合わせる倍率は体積散乱には掛けません。前方用と後方用をレイごとに一度だけ
求め、作者指定の固定率で混ぜます。許容上限`abs(g)=0.99`でも位相を欠落させないよう、分母は
異方性の符号へ揃えた余弦と`(1-abs(g))^2`から計算し、近い浮動小数点数同士の差を避けます。
密度や視線刻みの透過率で混合率を変えないため、同じ均質媒質を
異なる刻み数で積分しても単一散乱の物性は変わりません。

二次・三次散乱は、散乱係数を`a^n`、消散係数を`b^n`へ縮小します。光源側だけでなく視線側にも
同じ次数を適用し、各区間を`(a^n/b^n) * (1-exp(-tau*b^n))`で解析積分しながら、次数ごとの
累積透過率を別々に更新します。`a <= b`をCPU側で保証し、`b`が0へ近づく場合は極限式を使います。
光路と視線の早期終了も一次散乱だけでは決めず、有効な最高次数の残り寄与がしきい値を下回るまで
