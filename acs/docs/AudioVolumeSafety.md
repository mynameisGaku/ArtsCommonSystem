...
`FAudioEngine::Play`、`SetVolume`、`SetMasterVolume` は共通の規則で音量を正規化する。有限値は0.0から1.0へ収め、NaNと正負の無限大は0.0へ置き換える。壊れた計算結果を音声APIへそのまま渡さないための境界処理である。
...
正規の公開型は `FAudioEngine` である。`CAudioEngine` は移行中コード向けの一時的なソース互換名としてのみ残す。
