...
generic async I/O scheduler、DDC、profiler は作らない。
...
| T57 | Asset async job のみ採用、DDC は保留 | 新規 async job の完全 path だけを `FAssetPathInterner` で共有する。sync load、cache hit、in-flight hit には interner lock/allocation を追加しない。 |
...
repository に DDC owner、key lifetime、eviction policy が存在しない。Asset interner を
DDC 名目で別 owner に流用すると上限と解放時点を定義できないため追加していない。
DDC cache が導入された時点で、canonical key の生成回数、重複 owned bytes、eviction
hit を baseline 化してから再検討する。
