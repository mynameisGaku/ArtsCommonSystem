...
`EMPTY3D`, `CMP3D`, `CPROP3D`, `PLY3D`, `SPR3D`, `PFAB3D`, `PINS3D`, and `SEL3D`, including sparse IDs and
...
`SPR3D` texture paths, non-executing `PFAB3D` instance-source links, and `PINS3D`
stable identities are validated,
