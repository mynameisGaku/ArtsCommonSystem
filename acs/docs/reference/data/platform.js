  ...
  types: [
    {
      ...
      members: [
        ...
        { sig: "void SetEventCallback(EventCallback cb, void* user)", desc: "イベントが起きた時に呼ぶ関数を登録する。<code>user</code> はそのままコールバックに渡される任意の<t>ポインタ</t>。", when: "<code>Input</code> にイベントを橋渡しする時など。", sample: "w.SetEventCallback([](void* u, const Event&amp; e){ Input::OnEvent(e); }, nullptr);" },
    ...
    },
    {
      ...
      members: [
        { sig: "const wchar_t* title", desc: "タイトルバーに出す文字（既定 <code>L&quot;ACS FWindow&quot;</code>）。" },
    ...
    },
    {
      ...
      sample: "void OnEvent(void* u, const Event&amp; e) {\n    if (e.type == EventType::WindowResize) {\n        u32 w = e.resize.width, h = e.resize.height;\n    } else if (e.type == EventType::KeyPressed) {\n        EKey k = e.key.key;\n    }\n}",
      members: [
        { sig: "EventType type", desc: "イベントの種別。これを見てどのフィールドが有効か判断する。" },
    ...
    },
    {
      ...
      summary: "<t>Event</t> の種別を表す列挙。ウィンドウ系（閉じる・サイズ変更・フォーカス）と入力系（キー・マウス・文字）がある。",
      when: "受け取った <code>Event</code> の <code>type</code> と比較して分岐する。",
      sample: "switch (e.type) {\n    case EventType::WindowClose:  /* 閉じる */ break;\n    case EventType::KeyPressed:   /* 押下   */ break;\n    case EventType::MouseScrolled:/* ホイール*/ break;\n}",
      members: [
    ...
    },
    {
      ...
      members: [
        ...
        { sig: "static void OnEvent(const Event&amp; e)", desc: "<code>FWindow</code> から来た<t>イベント</t>を <code>Input</code> に流し込む橋渡し。<code>SetEventCallback</code> でこれを呼ぶよう繋ぐ。" },
        ...
        { sig: "static f32 GamepadAxisValue(u32 idx, EGamepadAxis axis)", ret: "軸の値", desc: "スティックは -1.0〜+1.0、トリガーは 0.0〜1.0。", sample: "f32 x = Input::GamepadAxisValue(0, EGamepadAxis::LeftX);" }
      ]
    },
    {
      name: "EKey",
      ...
      when: "<code>Input::IsKeyDown</code> などにキーを指定する時。",
      sample: "if (Input::IsKeyDown(EKey::W))      MoveUp();\nif (Input::IsKeyPressed(EKey::Escape)) OpenMenu();\nif (Input::IsKeyDown(EKey::LeftShift)) Run();",
      members: [
        { sig: "A 〜 Z", desc: "文字キー。" },
    ...
    },
    {
      ...
      when: "<code>Input::IsGamepadButtonDown / Pressed</code> に渡す。",
      sample: "if (Input::IsGamepadButtonPressed(0, EGamepadButton::A))     Jump();\nif (Input::IsGamepadButtonDown(0, EGamepadButton::RightBumper)) Guard();",
      members: [
    ...
    },
    {
      ...
      when: "<code>Input::GamepadAxisValue</code> に渡す。スティックは -1.0〜+1.0、トリガーは 0.0〜1.0 が返る。",
      sample: "f32 mx = Input::GamepadAxisValue(0, EGamepadAxis::LeftX);\nf32 my = Input::GamepadAxisValue(0, EGamepadAxis::LeftY);\nf32 rt = Input::GamepadAxisValue(0, EGamepadAxis::RightTrigger);",
      members: [
    ...
    },
    {
      ...
      when: "ベンチマークや経過時間の計測。フレーム間の <code>dt</code> が欲しいだけなら <t>FrameTimer</t> の方が便利。",
      sample: "f64 t0 = Clock::SecondsSinceStartup();\nDoHeavyWork();\nf64 elapsed = Clock::SecondsSinceStartup() - t0;",
      members: [
    ...
    },
    {
      ...
      when: "アセットの読み込み、ログやセーブのバイト書き出しなど、生のファイル操作が必要な時。設定/セーブの key-value 永続化なら <t>Storage</t> が便利。",
      sample: "auto r = FileSystem::ReadAllBytes(L&quot;data/map.bin&quot;);\nif (r.IsErr()) return;\nTArray&lt;byte&gt; bytes = static_cast&lt;TArray&lt;byte&gt;&amp;&amp;&gt;(r.Value());\n// ...\nFileSystem::WriteAllBytes(L&quot;data/out.bin&quot;, bytes.Data(), bytes.Size());",
      members: [
...
Object.assign(ACS_REF.glossary, {
  ...
  "イベント": "『× ボタンが押された』『キーが押された』のような、その瞬間に起きた出来事の通知。ACS では <t>Event</t> として届く。",
  "入力": "キーボード・マウス・<t>ゲームパッド</t>からの操作。ACS では <code>Input</code> でその瞬間の状態を読む。",
  "ゲームパッド": "コントローラ。ACS は XInput 互換でボタンやスティック（軸）を読む。",
  "ポーリング": "状態を必要なタイミングで自分から問い合わせる方式。<code>Input::IsKeyDown</code> はその瞬間の押下状態を返す。",
  ...
  "タイマ": "時間を計る仕組み。ACS では <t>Clock</t>（経過時間）と <t>FrameTimer</t>（フレーム間 dt）。",
  "union": "同じメモリ領域を複数の型で共用する仕組み。<t>Event</t> は種別ごとに有効なフィールドだけを使う。",
