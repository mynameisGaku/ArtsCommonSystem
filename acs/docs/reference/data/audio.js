  ...
  types: [
    {
      ...
      members: [
        { sig: "TResult<void> Init()", ret: "成功/失敗", desc: "<t>XAudio2</t> を初期化する。再生前に 1 回だけ呼ぶ。失敗は <t>Result</t> で返る。", when: "エンジン起動直後。"},
        ...
        { sig: "void SetVolume(FSoundHandle h, f32 volume)", desc: "個別の音の音量を変える。有限値は 0.0〜1.0 に収め、NaN と正負の無限大は 0.0 にする。backendが拒否した場合は以前の音量を維持して警告する。", when: "再生中にフェードしたい、距離で音量を変えたい時。"},
        { sig: "void StopAll()", desc: "再生中の音をすべて停止して解放する。", when: "シーン切り替えやゲームオーバーで全部消したい時。" },
        { sig: "void SetMasterVolume(f32 volume)", desc: "最終出力全体(=すべての音)の音量を設定する。有限値は 0.0〜1.0 に収め、NaN と正負の無限大は 0.0 にする。backendが拒否した場合は以前の音量を維持して警告する。個別音量とは別の<b>マスター</b>側。", when: "設定画面の『全体音量』スライダーなど。"},
        { sig: "void PauseAll()", desc: "再生中の音をすべて一時停止する。再生位置は保持される。", when: "ポーズメニューを開いた時。"},
    ...
    },
    {
      ...
      members: [
        ...
        { sig: "constexpr bool IsValid() const", ret: "有効か", desc: "<code>index</code> が無効値でなければ true。"},
