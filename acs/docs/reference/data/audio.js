  ...
  types: [
    {
      ...
      members: [
        ...
        { sig: "SoundHandle Play(const FAudioAsset& asset, f32 volume = 1.0f, bool loop = false)", ret: "再生ハンドル", desc: "音声アセットの再生を開始し、その音を識別する <code>SoundHandle</code> を返す。<code>volume</code> は 0.0〜1.0、<code>loop=true</code> で無限ループ。", when: "効果音や BGM を鳴らす瞬間。返ったハンドルを保持しておけば後で止められる。", sample: "SoundHandle se = engine.Play(*hitSound);          // 1 回だけ\nSoundHandle bgm = engine.Play(*music, 0.8f, true); // ループ再生" },
        { sig: "void Stop(SoundHandle h)", desc: "指定したハンドルの音を止め、内部スロットを解放する。無効/再生済みハンドルなら何もしない。", when: "ループ BGM を止める、鳴り続けている音を切る時。" },
        { sig: "void SetVolume(SoundHandle h, f32 volume)", desc: "個別の音の音量を変える。0.0〜1.0(範囲外は内部でクランプ)。", when: "再生中にフェードしたい、距離で音量を変えたい時。", sample: "engine.SetVolume(bgm, 0.3f); // この音だけ小さく" },
    ...
    },
    {
      ...
      summary: "再生中の 1 つの音を識別する軽量な<t>ハンドル</t>。<code>FAudioEngine::Play</code> が発行する。<b>世代付き</b>なので、止めた後に同じ内部スロットが別の音に再利用されても、古いハンドルは無効と判定される(取り違え防止)。",
      when: "再生した音をあとから止める/音量変更する時に保持しておく。値で持ち回ってよい小さな構造体。",
      sample: "SoundHandle h = engine.Play(*se);\nif (h.IsValid()) {\n    engine.SetVolume(h, 0.5f);\n    engine.Stop(h);\n}\nSoundHandle none = kInvalidSound;  // 無効を表す定数",
      members: [
        ...
        { sig: "constexpr bool operator==(SoundHandle o) const", ret: "等しいか", desc: "<code>index</code> と <code>generation</code> が両方一致する時だけ等しい。" }
      ]
    },
    {
      name: "kInvalidSound",
      kind: "定数", header: "audio/SoundHandle.h",
      summary: "『どの音も指していない』ことを表す無効な <t>SoundHandle</t> の定数(<code>SoundHandle{0xFFFFFFFF, 0}</code>)。",
      when: "ハンドルの初期値や『まだ鳴らしていない』状態を表したい時。",
      sample: "SoundHandle m_Bgm = kInvalidSound;\n// ...\nif (m_Bgm == kInvalidSound) m_Bgm = engine.Play(*music, 1.0f, true);"
    }
  ]
});

Object.assign(ACS_REF.glossary, {
  "FAudioEngine": "音声再生の中心となるエンジン。<t>FAudioAsset</t> を鳴らし、音量・停止・一時停止を扱う。内部は <t>XAudio2</t>。",
  "SoundHandle": "再生中の 1 つの音を識別する世代付きの<t>ハンドル</t>。止めた後の取り違えを防ぐ。",
  "FAudioAsset": "wav/mp3/flac/ogg を統一 PCM として読み込んだ音声アセット(asset モジュール)。<t>FAudioEngine</t> に渡して再生する。",
