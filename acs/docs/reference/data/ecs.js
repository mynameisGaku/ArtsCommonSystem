  ...
  types: [
    {
      ...
      members: [
        { sig: "u32 index", desc: "<code>CWorld</code> 内のスロット番号。初期値は無効を表す <code>0xFFFFFFFF</code>。" },
    ...
    },
    {
      ...
      members: [
        ...
        { sig: "void Add<T>(FEntityId e, T value)", desc: "エンティティに<t>コンポーネント</t> T を付ける。既にあれば上書き。エンティティが死んでいれば何もしない。"},
        { sig: "T* Get<T>(FEntityId e)", ret: "値ポインタ or null", desc: "コンポーネント T を取り出す。付いていなければ null。const 版もある。", when: "1 体だけ狙って値を読み書きする時。"},
        ...
        { sig: "auto Query<Comps...>()", ret: "TQueryView", desc: "指定した全コンポーネントを持つエンティティをまとめて反復する<t>クエリ</t>を返す。実装は Query.h。", when: "『位置と速度を持つ全員』のように条件で一括処理する時。"},
        { sig: "u32 EntityCount() const", ret: "生存数", desc: "今 CWorld にいる生きたエンティティの数。" },
        ...
        { sig: "using FWorld = CWorld", desc: "旧名を使う既存コード向けの互換別名。新しいコードでは <code>CWorld</code> を使う。" }
      ]
    },
    {
      name: "TQueryView&lt;Comps...&gt;",
      kind: "クラステンプレート", header: "ecs/Query.h",
      summary: "<code>CWorld::Query&lt;A, B, ...&gt;()</code> が返す反復ヘルパ。指定した<b>全</b>コンポーネントを持つエンティティだけを選び、各々に<t>ラムダ</t>を適用する。内部では一番要素の少ない集合を主軸にして効率良く絞り込む。",
      when: "毎フレームの処理(移動・当たり判定・描画準備など)を『該当する全エンティティ』に対して回す時。ECS の主役。",
      members: [
        { sig: "void Each(Fn fn)", desc: "条件に合う各エンティティへ <code>fn(FEntityId, Comps&amp;...)</code> を呼ぶ。反復前に dense 列を<b>スナップショット</b>するので、<code>fn</code> 内で Add/Remove しても安全。", when: "通常の毎フレーム処理。"},
        { sig: "void EachParallel(Fn fn, u32 grain = 1024)", desc: "<t>CThreadPool</t> で chunk 分割し<b>並列</b>に <code>fn</code> を呼ぶ。各エンティティは独立に処理される前提。共有資源を触るなら自分で同期する。<code>grain</code> は 1 chunk の最小エンティティ数。", when: "エンティティ数が非常に多く、各処理が独立しているとき。"}
      ]
    },
    {
      name: "SystemFn",
      kind: "型エイリアス", header: "ecs/System.h",
      summary: "システム関数の型 <code>void(*)(CWorld&amp;, f32 dt)</code>。<code>CWorld</code> と<t>デルタタイム</t>を受け取り、毎フレーム呼ばれる処理の単位。",
      when: "移動・AI・物理など『毎フレームやること』を 1 つの関数にまとめ、スケジューラに登録する時。"
    },
    {
      name: "CSystemScheduler",
      ...
      members: [
        ...
        { sig: "void Tick(CWorld& world, f32 dt)", desc: "登録された全システムを順に 1 回ずつ呼ぶ。<code>dt</code> は前フレームからの経過秒。", when: "毎フレーム 1 回呼ぶ。" },
        { sig: "void Clear()", desc: "登録済みシステムを全消去する。" },
        { sig: "usize Count() const", ret: "登録数", desc: "今登録されているシステムの数を返す。" },
        { sig: "using FSystemScheduler = CSystemScheduler", desc: "旧名を使う既存コード向けの互換別名。新しいコードでは <code>CSystemScheduler</code> を使う。" }
      ]
    },
    {
      ...
      when: "通常ユーザーが直接触ることは少なく、<code>CWorld</code> 内部がコンポーネントの保管場所を引くために使う。低レベルに自前管理したい時のみ。",
      members: [
    ...
    },
    {
      ...
      when: "型消去したコンポーネント管理を自作する時の土台。通常は <code>CWorld</code> の内側で使われる。",
      members: [
        ...
        { sig: "using FComponentRegistry = CComponentRegistry", desc: "旧名を使う既存コード向けの互換別名。新しいコードでは <code>CComponentRegistry</code> を使う。" }
      ]
    },
    {
      name: "ASparseSetBase",
      kind: "基底クラス", header: "ecs/SparseSet.h",
      summary: "<t>TSparseSet</t> の型に依らない共通部分。エンティティ番号 → 密配列上の位置の対応や、要素数・dense 走査を提供する。<code>CWorld</code> が型を知らずに集合を扱うための基底。",
      when: "ふつう直接使わない。<code>CWorld</code> がコンポーネント型を区別せず保管・破棄するための内部基盤。",
      members: [
        ...
        { sig: "virtual void RemoveErased(u32 entity_index)", desc: "型を知らずにそのエンティティの値を削除する(<code>CWorld::Destroy</code> 用)。" },
        { sig: "using FSparseSetBase = ASparseSetBase", desc: "旧名を使う既存コード向けの互換別名。新しいコードでは <code>ASparseSetBase</code> を使う。" }
      ]
    },
    {
      name: "TSparseSet&lt;T&gt;",
      kind: "クラステンプレート", header: "ecs/SparseSet.h",
      summary: "エンティティ番号 → コンポーネント値 T の高速対応表(<t>Sparse Set</t>)。Add/Remove/Contains が <b>O(1)</b> で、全要素は<b>密配列</b>に並ぶため線形走査がキャッシュに優しい。<t>ASparseSetBase</t> を継承。",
      when: "1 種類のコンポーネントを大量のエンティティに対して高速に出し入れ・全走査したい時。<code>CWorld</code> がコンポーネント型ごとに 1 つ内部保有する。",
      members: [
...
Object.assign(ACS_REF.glossary, {
  ...
  "システム": "<t>コンポーネント</t>を持つエンティティ群を毎フレーム処理する関数(移動・AI など)。<t>CSystemScheduler</t> で順に実行する。",
  ...
  "CThreadPool": "複数<t>スレッド</t>に仕事を分配する実行基盤。<code>ParallelFor</code> で範囲を並列処理する。"
});
