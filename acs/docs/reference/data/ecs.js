  ...
  types: [
    {
      ...
      sample: "EntityId e = world.Create();\nif (e.IsValid()) { /* 有効な ID */ }\nif (e == kInvalidEntity) { /* 無効 */ }",
      members: [
        { sig: "u32 index", desc: "<code>World</code> 内のスロット番号。初期値は無効を表す <code>0xFFFFFFFF</code>。" },
        ...
        { sig: "bool operator==(EntityId) const / operator!=", desc: "<code>index</code> と <code>generation</code> の両方が一致して初めて同一とみなす。" }
      ]
    },
    {
      name: "kInvalidEntity",
      kind: "定数", header: "ecs/Entity.h",
      summary: "無効な <t>EntityId</t>。戻り値や初期値の『該当なし』を表す番兵。",
      when: "ID を返す関数が『見つからなかった』を示す時や、未代入の初期値として使う。",
      sample: "EntityId target = kInvalidEntity;\n// ...探索...\nif (target == kInvalidEntity) return;  // 見つからなかった"
    },
    {
      name: "World",
      ...
      members: [
        ...
        { sig: "T* Get<T>(EntityId e)", ret: "値ポインタ or null", desc: "コンポーネント T を取り出す。付いていなければ null。const 版もある。", when: "1 体だけ狙って値を読み書きする時。", sample: "if (auto* h = w.Get&lt;Health&gt;(e)) h-&gt;hp -= 10;" },
        { sig: "bool Has<T>(EntityId e) const", ret: "所持しているか", desc: "コンポーネント T を持っているかを返す(<code>Get&lt;T&gt;(e) != nullptr</code>)。" },
        { sig: "void Remove<T>(EntityId e)", desc: "コンポーネント T を外す。無くても安全(何もしない)。" },
        { sig: "auto Query<Comps...>()", ret: "QueryView", desc: "指定した全コンポーネントを持つエンティティをまとめて反復する<t>クエリ</t>を返す。実装は Query.h。", when: "『位置と速度を持つ全員』のように条件で一括処理する時。", sample: "w.Query&lt;Position, Velocity&gt;().Each([](EntityId, Position&amp; p, Velocity&amp; v){ p.x += v.x; });" },
        { sig: "u32 EntityCount() const", ret: "生存数", desc: "今 World にいる生きたエンティティの数。" },
        { sig: "EntityId MakeIdFromIndex(u32 index) const", ret: "EntityId", desc: "スロット番号だけから世代を補って <code>EntityId</code> を復元する。主に <t>クエリ</t>内部用。" }
      ]
    },
    {
      name: "QueryView&lt;Comps...&gt;",
      kind: "クラステンプレート", header: "ecs/Query.h",
      summary: "<code>World::Query&lt;A, B, ...&gt;()</code> が返す反復ヘルパ。指定した<b>全</b>コンポーネントを持つエンティティだけを選び、各々に<t>ラムダ</t>を適用する。内部では一番要素の少ない集合を主軸にして効率良く絞り込む。",
      when: "毎フレームの処理(移動・当たり判定・描画準備など)を『該当する全エンティティ』に対して回す時。ECS の主役。",
      sample: "w.Query&lt;Position, Velocity&gt;().Each(\n    [dt](EntityId id, Position&amp; p, Velocity&amp; v) {\n        p.x += v.x * dt;\n        p.y += v.y * dt;\n    });",
      members: [
        { sig: "void Each(Fn fn)", desc: "条件に合う各エンティティへ <code>fn(EntityId, Comps&amp;...)</code> を呼ぶ。反復前に dense 列を<b>スナップショット</b>するので、<code>fn</code> 内で Add/Remove しても安全。", when: "通常の毎フレーム処理。", sample: "w.Query&lt;Health&gt;().Each([](EntityId e, Health&amp; h){ if (h.hp &lt;= 0) {/* 死亡処理 */} });" },
        { sig: "void EachParallel(Fn fn, u32 grain = 1024)", desc: "<t>FThreadPool</t> で chunk 分割し<b>並列</b>に <code>fn</code> を呼ぶ。各エンティティは独立に処理される前提。共有資源を触るなら自分で同期する。<code>grain</code> は 1 chunk の最小エンティティ数。", when: "エンティティ数が非常に多く、各処理が独立しているとき。", sample: "w.Query&lt;Position, Velocity&gt;().EachParallel(\n    [](EntityId, Position&amp; p, Velocity&amp; v){ p.x += v.x; }, 1024);" }
      ]
    },
    {
      name: "SystemFn",
      kind: "型エイリアス", header: "ecs/System.h",
      summary: "システム関数の型 <code>void(*)(World&amp;, f32 dt)</code>。<code>World</code> と<t>デルタタイム</t>を受け取り、毎フレーム呼ばれる処理の単位。",
      when: "移動・AI・物理など『毎フレームやること』を 1 つの関数にまとめ、スケジューラに登録する時。",
      sample: "void MovementSystem(World&amp; w, f32 dt) {\n    w.Query&lt;Position, Velocity&gt;().Each(\n        [dt](EntityId, Position&amp; p, Velocity&amp; v){ p.x += v.x * dt; });\n}"
    },
    {
    ...
    },
    {
      ...
      when: "通常ユーザーが直接触ることは少なく、<code>World</code> 内部がコンポーネントの保管場所を引くために使う。低レベルに自前管理したい時のみ。",
    ...
    },
    {
      ...
      sample: "const ComponentOps&amp; ops = FComponentRegistry::Register&lt;Position&gt;();\nvoid* slot = /* 確保した領域 */;\nops.destroy(slot);  // 型を知らずに ~Position() を呼ぶ",
      members: [
    ...
    },
    {
      ...
      summary: "コンポーネント型ごとの操作(<t>ComponentOps</t>)を登録・引き出す静的レジストリ。型 T を 1 度登録すれば、以後は番号から型を知らずに操作を取り出せる。",
      when: "型消去したコンポーネント管理を自作する時の土台。通常は <code>World</code> の内側で使われる。",
      sample: "const ComponentOps&amp; ops = FComponentRegistry::Register&lt;Velocity&gt;();\nComponentTypeId id = GetComponentTypeId&lt;Velocity&gt;();\nconst ComponentOps&amp; same = FComponentRegistry::Get(id);  // 同じ Ops",
      members: [
        { sig: "static const ComponentOps& Register<T>()", ret: "型 T の Ops", desc: "型 T を登録(初回のみ実体作成)し、その <code>ComponentOps</code> を返す。", when: "新しいコンポーネント型を初めて使う前に。" },
        { sig: "static const ComponentOps& Get(ComponentTypeId id)", ret: "Ops", desc: "番号から登録済みの <code>ComponentOps</code> を引く。" }
      ]
    },
    {
      name: "SparseSetBase",
      kind: "基底クラス", header: "ecs/SparseSet.h",
      summary: "<t>SparseSet</t> の型に依らない共通部分。エンティティ番号 → 密配列上の位置の対応や、要素数・dense 走査を提供する。<code>World</code> が型を知らずに集合を扱うための基底。",
      when: "ふつう直接使わない。<code>World</code> がコンポーネント型を区別せず保管・破棄するための内部基盤。",
      sample: "// World 内部で SparseSetBase* として保持される\nSparseSetBase* s = /* ... */;\nif (s-&gt;Contains(e.index)) { usize n = s-&gt;Size(); }",
      members: [
        ...
        { sig: "virtual void RemoveErased(u32 entity_index)", desc: "型を知らずにそのエンティティの値を削除する(<code>World::Destroy</code> 用)。" }
      ]
    },
    {
      name: "SparseSet&lt;T&gt;",
      kind: "クラステンプレート", header: "ecs/SparseSet.h",
      summary: "エンティティ番号 → コンポーネント値 T の高速対応表(<t>Sparse Set</t>)。Add/Remove/Contains が <b>O(1)</b> で、全要素は<b>密配列</b>に並ぶため線形走査がキャッシュに優しい。<t>SparseSetBase</t> を継承。",
      when: "1 種類のコンポーネントを大量のエンティティに対して高速に出し入れ・全走査したい時。<code>World</code> がコンポーネント型ごとに 1 つ内部保有する。",
      sample: "SparseSet&lt;Position&gt; positions;\npositions.Add(e.index, {1, 2, 3});\nif (Position* p = positions.Get(e.index)) p-&gt;x += 1;\npositions.Remove(e.index);",
      members: [
...
Object.assign(ACS_REF.glossary, {
  ...
  "システム": "<t>コンポーネント</t>を持つエンティティ群を毎フレーム処理する関数(移動・AI など)。<t>SystemScheduler</t> で順に実行する。",
  "クエリ": "条件(指定コンポーネントを全部持つ)に合うエンティティを選んで反復する仕組み。ECS の主役。",
  "世代": "同じスロット番号が再利用されたかを区別する番号。古い<t>EntityId</t>を検出するのに使う。",
  "世代付きハンドル": "番号 + <t>世代</t>で構成される安全な参照。スロット再利用後の古い参照を無効と判定できる。",
  "Sparse Set": "疎な参照表と密な値配列を組み合わせ、追加/削除/検索を O(1)、全走査をキャッシュ効率良くするデータ構造。",
  "SparseSet": "ACS の <t>Sparse Set</t> 実装。エンティティ番号→コンポーネント値の高速対応表。",
  "型消去": "コンパイル時の型情報を隠し、サイズや破棄/移動操作だけを実行時に扱えるようにする手法。",
  "EntityId": "1 つの<t>エンティティ</t>を指す軽量な識別子。<code>index</code> と <code>generation</code> の組。",
  "ComponentOps": "型を消去したコンポーネントのサイズ・破棄・<t>ムーブ</t>などの操作束。",
