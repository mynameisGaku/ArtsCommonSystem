  ...
  types: [
    ...
    },
    {
      ...
      members: [
        ...
        { sig: "void EachParallel(Fn fn, u32 grain = 1024)", desc: "<t>CThreadPool</t> で chunk 分割し<b>並列</b>に <code>fn</code> を呼ぶ。各エンティティは独立に処理される前提。共有資源を触るなら自分で同期する。<code>grain</code> は 1 chunk の最小エンティティ数。", when: "エンティティ数が非常に多く、各処理が独立しているとき。", sample: "w.Query&lt;Position, Velocity&gt;().EachParallel(\n    [](FEntityId, Position&amp; p, Velocity&amp; v){ p.x += v.x; }, 1024);" }
      ]
    },
    {
      name: "SystemFn",
    ...
    },
    {
      ...
      sample: "const FComponentOps&amp; ops = CComponentRegistry::Register&lt;Position&gt;();\nvoid* slot = /* 確保した領域 */;\nops.destroy(slot);  // 型を知らずに ~Position() を呼ぶ",
      members: [
    ...
    },
    {
      ...
      sample: "const FComponentOps&amp; ops = CComponentRegistry::Register&lt;Velocity&gt;();\nFComponentTypeId id = GetComponentTypeId&lt;Velocity&gt;();\nconst FComponentOps&amp; same = CComponentRegistry::Get(id);  // 同じ Ops",
      members: [
    ...
    },
    {
      ...
      sample: "// CWorld 内部で ASparseSetBase* として保持される\nFSparseSetBase* s = /* ... */;\nif (s-&gt;Contains(e.index)) { usize n = s-&gt;Size(); }",
      members: [
