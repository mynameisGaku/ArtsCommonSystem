...
ACS_REF.modules.push({
  ...
  types: [
    ...
    },
    {
      ...
      members: [
        { sig: "template<typename E> FSubscriptionHandle Subscribe(MessageCallback cb, void* user)", ret: "購読ハンドル", desc: "<code>E</code> 型イベントを購読する。配信時に <code>cb(payload, user)</code> が呼ばれる。<code>user</code> は自分の状態を持ち回るための任意ポインタ。", when: "そのイベントに反応したい受信側で。返ったハンドルは解除に使うので保持する。" },
        { sig: "template<typename E> void Publish(const E& payload)", desc: "<code>E</code> 型イベントを発行し、購読者全員を<b>その場で順に</b>呼ぶ(同期配信)。", sample: "bus.Publish&lt;FDamageEvent&gt;(FDamageEvent{ enemy, 10.0f });", when: "出来事が起きた瞬間に全員へ知らせたい時。" },
        { sig: "bool Unsubscribe(FSubscriptionHandle h)", ret: "解除できたか", desc: "購読を解除する。<b>配信(Publish)中に呼んでも安全</b>(予約して遅延適用される)。", when: "受信側が不要になった/破棄される時。必ず解除する。" },
