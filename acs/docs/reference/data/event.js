  ...
  types: [
    ...
    },
    {
      ...
      members: [
        ...
        { sig: "using CMessageBroker = FMessageBroker", desc: "旧名を使うコードのために残しているソース互換名。別の実装型や C 名の実装シンボルは作らず、正規の <code>FMessageBroker</code> そのものを指す。新しいコードでは <code>FMessageBroker</code> を使う。" }
      ]
    },
    {
      name: "FSubscriptionHandle",
      kind: "構造体", header: "event/SubscriptionHandle.h",
      summary: "<code>FMessageBroker::Subscribe</code> が返す<b>購読の控え</b>。これを <code>Unsubscribe</code> に渡して解除する。<t>世代</t>付きなので、解除済みの古いハンドルで誤って別の購読を消すことはありません。",
    ...
    },
    {
      ...
      summary: "<code>FMessageBroker</code> の購読コールバックの型。<code>void (*)(const void* payload, void* user)</code>。<code>payload</code> は発行されたイベントの中身、<code>user</code> は登録時に渡した任意ポインタ。STL 非依存方針のため<t>ラムダ</t>のキャプチャではなく <code>user</code> で状態を持ち回ります。",
    ...
    },
    {
      ...
      summary: "イベント型 <code>E</code> に対して<b>一意な番号</b>(<code>EventTypeId</code>)を割り当てて返す。同じ型なら常に同じ番号になり、<code>FMessageBroker</code> が型ごとのチャンネルを区別するのに使います。",
      when: "通常は <code>Subscribe</code>/<code>Publish</code> が内部で呼ぶので、直接使うことは稀。チャンネル番号が欲しい時だけ。",
      sample: "EventTypeId id = GetEventTypeId&lt;FDamageEvent&gt;();\nu32 n = bus.SubscriberCount(id);   // この型の購読者数"
    },
    {
      name: "EventTypeId",
      kind: "型エイリアス", header: "event/EventTypeId.h",
      summary: "<code>FMessageBroker</code> がメッセージ型ごとの通路を識別する番号。",
      when: "購読数の確認など、型から得た通路番号を明示的に保持する時。",
      sample: "EventTypeId channel = GetEventTypeId&lt;FDamageEvent&gt;();\nif (IsValidEventTypeId(channel)) { /* 利用できる通路 */ }",
      members: [
        { sig: "EventTypeId = u32", desc: "通路番号の実体は <code>u32</code>。" },
        { sig: "kMaxEventTypes = 256", desc: "同時に扱えるメッセージ型の上限。" },
        { sig: "bool IsValidEventTypeId(EventTypeId channel)", ret: "範囲内か", desc: "通路番号が256未満ならtrue。仲介器は範囲外の通路を作成しない。" }
      ]
    },
    {
      name: "TMessagePipe&lt;T&gt;",
      ...
      when: "ワーカースレッドの結果をメインスレッドで受け取る等、<b>違うスレッド間</b>でイベントや値を受け渡したい時。同一スレッド内の即時配信は <t>FMessageBroker</t> を使う。",
    ...
    },
    {
      ...
      members: [
        ...
        { sig: "using CTimerManager = FTimerManager", desc: "旧名を使うコードのために残しているソース互換名。別の実装型や C 名の実装シンボルは作らず、正規の <code>FTimerManager</code> そのものを指す。新しいコードでは <code>FTimerManager</code> を使う。" }
      ]
    },
    {
      name: "FTimerHandle",
...
Object.assign(ACS_REF.glossary, {
  ...
  "FMessageBroker": "型ごとにイベントを同期配信する<t>pub/sub</t>バス。1 スレッド内で使う。",
