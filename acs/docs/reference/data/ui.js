  ...
  types: [
    {
      ...
      when: "ボタンやパネルなど具体的な部品はこれを継承して作る。直接インスタンス化するより、派生型(<code>FStackPanel</code> / <code>FButton</code> 等)を使うのが普通。",
      sample: "FStackPanel root;\nFButton* btn = root.Add&lt;FButton&gt;(\"OK\");  // 子を追加して生ポインタを得る\n// 毎フレーム ui_renderer.Render(root, cmd, 1280, 720)\n// が Layout と描画をまとめて実行",
      members: [
        { sig: "template&lt;W,Args...&gt; W* Add(Args&amp;&amp;... args)", ret: "追加した子の生ポインタ", desc: "子<t>ウィジェット</t>を <code>W</code> 型で作って自分の子に取り込み、その生<t>ポインタ</t>を返す。所有権は親が持つ。", sample: "FLabel* l = root.Add&lt;FLabel&gt;(\"スコア\");", when: "UI ツリーに部品を足す時の基本操作。" },
        { sig: "FWidget* Parent() const", ret: "親 or null", desc: "自分を取り込んでいる親を返す。ルートなら null。" },
        { sig: "usize ChildCount() const", ret: "子の数", desc: "直接の子<t>ウィジェット</t>の個数。" },
        { sig: "FWidget* Child(usize i) const", ret: "i 番目の子", desc: "範囲外なら null。" },
        { sig: "virtual void Layout(f32 x, f32 y, f32 w, f32 h)", desc: "自分の絶対座標(<code>rect</code>)を確定し、子があれば配置する。派生型ごとに並べ方が違う。", when: "<code>FUiRenderer::Render</code> が毎フレーム root に対して呼ぶ。layout 単体の検証時は直接呼べる。" },
        ...
        { sig: "FWidget* HitTestRecursive(f32 px, f32 py)", ret: "当たった一番上の子", desc: "ツリーを上に描かれた子から優先して<t>ヒットテスト</t>し、最初に当たった<t>ウィジェット</t>を返す。" },
        { sig: "virtual void OnPointerDown / OnPointerUp / OnPointerMove(f32 px, f32 py)", desc: "マウス押下/離す/移動が来た時に呼ばれる。既定は何もしない。派生型で上書きする。" },
        { sig: "virtual void OnTextInput(u32 codepoint)", desc: "文字入力が来た時に呼ばれる(<code>FTextInput</code> 等が使う)。" },
        { sig: "virtual void OnKey(i32 key, bool pressed)", desc: "従来互換のキー押下/解放イベント。<code>FUiInput</code> は押下を true、解放を false で配信する。修飾キー付き既定実装からも転送されるため、既存の override はそのまま動作する。" },
    ...
    },
    {
      ...
      sample: "FContainer layer;\nlayer.Add&lt;FLabel&gt;(\"背景の上に乗せる\");\nlayer.Add&lt;FButton&gt;(\"OK\");  // 同じ範囲に重なる",
      members: [
        { sig: "（継承メンバ）", desc: "独自のメンバは持たず、<code>Layout(x,y,w,h)</code> だけを上書きして全子に親と同じ矩形を渡す。<code>Add&lt;W&gt;()</code> / <code>Parent()</code> / <code>Child()</code> / <code>ChildCount()</code> / <code>HitTestRecursive()</code> や <code>visible</code> / <code>rect</code> / <code>requested</code> などはすべて基底 <t>FWidget</t> から継承する。<code>Render</code> も基底既定（子の再帰描画のみ）で、自身は何も描かない。" }
      ]
    },
    {
      name: "FLabel",
    ...
    },
    {
      ...
      members: [
        ...
        { sig: "void OnKey(i32 key, bool pressed, const FUiKeyModifiers&amp; modifiers) noexcept", desc: "Shift+Left/Right/Home/End で anchor を固定した選択拡張・縮小・方向反転を行い、Ctrl+A で全体を選択する。Ctrl+Alt の AltGr は全選択と誤認しない。3引数版は modifier snapshot を保持して仮想2引数版へ転送するため、2引数版だけを override した既存 FTextInput 派生にも実 Dispatch の押下/解放が届く。" },
        ...
        { sig: "void Render(FUiRenderer&amp; r) noexcept", desc: "フォーカス中の選択範囲は <code>input_selection</code> 色で背景・枠の後、文字列・caret の前に描画する。矩形は入力欄の左右 6 px の内容領域と上下 3 px の範囲へ clamp し、隣接 UI へはみ出させない。" },
    ...
    },
    {
      ...
      summary: "<t>FWidget</t> ツリーを <code>FSpriteBatch</code> + <code>FFont</code> で<b>1 フレーム分まとめて描画</b>する描画役。矩形・枠線・文字・カーソル位置用の文字列計測ヘルパを持ち、各<t>ウィジェット</t>の <code>Render</code> がこれに描画を依頼する。コピー不可。",
      ...
      members: [
        { sig: "TResult&lt;void&gt; Init(IRhiDevice&amp; device, EFormat rt_format, FFont* default_font) noexcept", ret: "成否(<t>Result</t>)", desc: "内部の <code>FSpriteBatch</code> を初期化し、既定<t>フォント</t>と描画先フォーマットを設定する。", when: "アプリ起動時に 1 回。" },
        { sig: "void Shutdown()", desc: "確保した描画リソースを解放する。" },
        { sig: "void Render(FWidget&amp; root, IRhiCommandList&amp; cmd, u32 screen_w, u32 screen_h) noexcept", desc: "visible な root を画面全体へ Layout してから<t>ウィジェット</t>を再帰描画し、コマンドリストに発行する。<code>root.visible == false</code> なら Layout、batch Begin/End、root/child Render をすべて省略する。", when: "毎フレーム 1 回呼ぶ。" },
        ...
        { sig: "f32 MeasureText(const char* utf8) const / f32 MeasureTextBytes(const char* utf8, usize byte_count) const", desc: "文字列全体または指定 byte prefix の描画幅を測る。<code>FTextInput</code> の caret 位置計算にも使う。" },
    ...
    },
    {
      ...
      when: "独自 widget が Shift や Ctrl を組み合わせたキー操作を実装する時。通常は <code>FUiInput</code> が構築して3引数版 <code>OnKey</code> へ渡す。",
    ...
    },
    {
      ...
      members: [
        { sig: "void Dispatch(FWidget&amp; root) noexcept", desc: "現在のマウス位置・クリック・キー押下/解放を <t>Input</t> から取得し、ルートを<t>ヒットテスト</t>して該当<t>ウィジェット</t>へ届ける。編集キーには左右 Shift / Ctrl / Alt / Super の押下中状態を <code>FUiKeyModifiers</code> として付与し、Ctrl+A も選択コマンドとして配信する。保存した複合 identity は毎回現在の生存 subtree から解決するため、各 On* callback は current/other child を同期除去できる。呼び出し中は root 自身を破棄してはならない。", when: "毎フレーム 1 回呼ぶ。" },
        { sig: "void Reset() noexcept / void Reset(FWidget&amp; live_root) noexcept", desc: "保存中の root / hover / pressed / focus / Ctrl+A 状態を破棄する。引数なし版は widget を参照しないので root 破棄直前にも安全で、同じ live root を次に Dispatch すると新規採用時に subtree の一時入力フラグも初期化する。生存 root を渡す版は、その場で subtree の <code>hovered</code> / <code>focused</code> / <code>pressed</code> も再帰的に解除する。DLL unload/reload では module address まで再利用され得るため、host は module 所有 root が生存中なら Reset(root) の後に破棄・unloadし、既に破棄済みなら reload / 次の Dispatch より前に Reset() を必ず呼ぶ。", when: "scene 切り替え、UI ツリー再構築、module root destruction / DLL unload 境界。root 変更自体は通常 Dispatch でも自動検出するが、DLL reload 境界の Reset は必須。" }
      ]
    },
    {
      name: "FUiRect",
    ...
    },
    {
      ...
      when: "<code>FStackPanel</code> の内側余白など、要素の周囲の空きを指定する時。",
      sample: "FUiPadding pad{ 12, 8, 12, 8 };  // 左右12, 上下8\npanel.padding = pad;"
    },
    {
      name: "EStackDir",
      kind: "列挙(enum)", header: "ui/Widget.h",
      summary: "<code>FStackPanel</code> が子を並べる<b>方向</b>。<code>Vertical</code>(縦) と <code>Horizontal</code>(横)。",
...
Object.assign(ACS_REF.glossary, {
  "ウィジェット": "ボタンやラベルなど画面上の UI 部品 1 つ 1 つのこと。<t>FWidget</t> を継承して作る。",
  ...
  "UTF-8 cursor": "<code>FTextInput</code> が UTF-8 の byte offset とコードポイント境界を対応付けて保持する編集位置。"
});
