  ...
  types: [
    {
      ...
      members: [
        { sig: "TResult&lt;void&gt; Connect(const char* ServerUrl)", ret: "成功/失敗", desc: "出力先ファイルを開く(無ければ作る)。<code>ServerUrl</code> は <code>\"file:パス\"</code> 形式で、<code>file:</code> 接頭辞は自動で取り除かれる。開けない時は失敗を返す。", when: "イベントを書き始める前に一度呼ぶ。"},
        { sig: "void Disconnect()", desc: "開いているファイルを閉じる。未接続でも安全(<t>べき等</t>)。" },
        { sig: "bool IsConnected() const", ret: "接続中か", desc: "ファイルが開いていれば true。" },
        { sig: "TResult&lt;void&gt; SendTelemetry(const char* EventName, const char* JsonPayload)", ret: "成功/失敗", desc: "イベント 1 件を 1 行としてファイルに追記する。<code>JsonPayload</code> 内の特殊文字は安全に<t>エスケープ</t>される。書き込み失敗時は失敗を返す。", when: "計測したい出来事が起きたタイミングで呼ぶ。"},
        ...
        { sig: "u32 WrittenCount() const", ret: "書込件数", desc: "これまでにファイルへ書いたイベントの累計件数。動作確認やテストに便利。"},
