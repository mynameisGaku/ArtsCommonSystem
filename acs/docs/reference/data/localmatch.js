  ...
  types: [
    {
      ...
      summary: "<t>CLocalMatchmaker</t> の設定。今は『どれだけ<t>レーティング</t>が離れていてもマッチさせてよいか』の上限値だけを持ちます。",
    ...
    },
    {
      ...
      members: [
        { sig: "explicit CLocalMatchmaker(const FLocalMatchmakerConfig& Config)", desc: "設定を渡して構築する。引数なし版は既定設定(レート差 150)。" },
        { sig: "TResult<MatchTicket> StartSearch(const char* Mode, acs::u32 EloHint)", ret: "発行された ticket", desc: "<code>Mode</code>(\"ranked_1v1\" 等のキー)と自分の<t>レーティング</t>推定値で検索を開始し、<t>MatchTicket</t> を発行する。同じ Mode でレートが近い待機者がいれば即座に双方を <code>Matched</code> にする。空きが無い時は <t>Result</t> がエラー。", when: "プレイヤーが『対戦相手を探す』ボタンを押した時。"},
        ...
        { sig: "using FLocalMatchmaker = CLocalMatchmaker", desc: "旧名を使う既存コード向けの互換別名。新しいコードでは <code>CLocalMatchmaker</code> を使う。" }
      ]
    },
    {
      name: "GetDefaultLocalMatchmaker()",
      kind: "関数", header: "localmatch/LocalMatchmaker.h",
      summary: "プロセス全体で共有される既定の <t>CLocalMatchmaker</t>(<t>シングルトン</t>)を <code>acs::game::IMatchmaker&amp;</code> として返します。",
...
Object.assign(ACS_REF.glossary, {
  ...
  "CLocalMatchmaker": "サーバ無しで同一プロセス内のプレイヤーを組ませる <code>IMatchmaker</code> 実装。",
