  ...
  types: [
    {
      ...
      members: [
        ...
        { sig: "ConvexPoly2 HullPolygon() const", ret: "凸ポリゴン", desc: "<t>凸包</t>を物理用の <code>ConvexPoly2</code>(最大 16 頂点)に変換して返す。頂点が多ければ均等に間引く。<code>FCollisionWorld2D::AddPolygon</code> にそのまま渡せる。", when: "2D 物理ワールドにこのスプライトを当たり判定として登録する時。" },
        ...
        { sig: "Aabb2 Bounds() const", ret: "外接矩形", desc: "形を囲む 2D の<t>AABB</t>(軸並行の最小矩形)。粗い当たり判定の前段に使える。" },
    ...
    },
    {
      ...
      members: [
        ...
        { sig: "RayHit3 Raycast(const Ray3& ray, f32 t_max = FLT_MAX) const", ret: "ヒット情報", desc: "レイを飛ばし<b>最も近い</b>当たりを返す。結果の <code>hit</code>(当たったか)・<code>t</code>(距離)・<code>point</code>(位置)・<code>normal</code>(三角形の面法線)を見る。当たらなければ <code>hit=false</code>。", when: "マウスピッキング・着弾点・視線判定など。" },
        { sig: "bool OverlapsAabb(const Aabb3& box) const", ret: "重なるなら true", desc: "指定した <t>AABB</t> がメッシュ(の三角形)と重なるかを <t>BVH</t> で素早く判定する。<t>ブロードフェーズ</t>のふるい分けに。" },
        { sig: "Aabb3 Bounds() const", ret: "外接箱", desc: "メッシュ全体を囲む 3D の<t>AABB</t>。" },
    ...
    },
    {
      ...
      when: "複雑なメッシュをそのまま当たり判定にすると重い時に、その外形を包む凸の形だけ取り出して軽量化したい時。出力はそのまま <code>FMeshCollider::BuildFromTriangles</code> に渡せる。",
