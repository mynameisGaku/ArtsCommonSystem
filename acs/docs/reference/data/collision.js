  ...
  types: [
    {
      ...
      members: [
        ...
        { sig: "bool ContainsPoint(FVec2 p) const", ret: "内側なら true", desc: "簡略化済み<t>輪郭</t>ポリゴンで点の内外を判定する(<t>レイ交差法</t>、凹形状にも対応)。座標はピクセル空間(左上原点)。"},
    ...
    },
    {
      ...
      members: [
        ...
        { sig: "TResult<void> BuildFromTriangles(const FVec3* positions, u32 vertex_count, const u32* indices, u32 index_count)", ret: "成功/失敗", desc: "生の頂点位置配列 + 三角形<t>インデックス</t>列(3 個 1 組)から構築する。<code>BuildConvexHull3</code> の出力をそのまま渡せる。"},
