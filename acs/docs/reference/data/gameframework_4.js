...
ACS_REF.modules.push({
  ...
  types: [
    ...
    },
    {
      ...
      when: "FHungerSystem の各 API で「どの stat か」を指定する時。Custom1/2 はゲーム固有 (放射線・酸素など) の拡張枠。",
    ...
    // =====================================================================
    {
      ...
      members: [
        ...
        { sig: "bool TryBindGamepadAxis(FActionId a, EGamepadAxis axis, u32 player, const FInputAxisOptions& options)", ret: "追加できたか", desc: "axis、player、dead-zone、倍率、反転を検証し、成功時だけ追加する。" },
        { sig: "void Unbind(FActionId a) / void ClearAll()", desc: "指定アクションの全割り当てを削除 / 全削除。" },
        { sig: "FInputActionState Evaluate(FActionId a, const IInputStateView& input) const", desc: "明示入力から Pressed/Held/Released/Axis を一括評価する。platform 状態を参照しない。" },
    ...
    // =====================================================================
    {
      ...
      when: "拾った/報酬で得たアイテムをプレイヤーが持ち歩く状態を管理したい時。即時購入は FEconomyDirector、装着 cosmetic は FCharacterCustomizer と役割分担。",
    ...
    },
    {
      ...
      when: "FNetSnapshot に socket を差し替えたい時。Send/Receive は非ブロッキングでメッセージ境界を保持する契約。",
      members: [
    ...
    },
    {
      ...
      summary: "FNetSnapshot の役割。Client (受信のみ) / Server (送信のみ) / ServerListener (listen-server で両方) / Standalone (通信なし、全 API no-op)。",
