  ...
  types: [
    ...
    },
    {
      ...
      members: [
        ...
        { sig: "bool BeginRenderToTextureMrt(rts, rt_count, clear, depth=null, depth_clear=1)", desc: "複数 color RT を同時 bind する MRT 開始。Raw DX12 でも実装済み。全 attachment の view・寸法・sample count を bind 前に検証し、成功時だけ <code>true</code>。<code>false</code> では backend state は変わらず、Draw と EndRenderToTextureMrt は禁止。" },
    ...
    },
    {
      ...
      members: [
        ...
        { sig: "bool BeginRenderToTextureMrt(rts, rt_count, clear, depth=null, depth_clear=1)", desc: "複数 color RT を同時 bind する MRT。Diligent / Raw DX12 の双方で実装済み。全 attachment の view・寸法・sample count を bind 前に検証し、成功時だけ <code>true</code>。<code>false</code> では backend state は変わらず、Draw と EndRenderToTextureMrt は禁止。", when: "G-Buffer や SSGI など複数出力パスを書く時。" },
