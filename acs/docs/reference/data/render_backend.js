  ...
  types: [
    {
      ...
      members: [
        ...
        { sig: "HrResult Init(const DeviceConfig& cfg)", ret: "成否(HRESULT)", desc: "DXGI ファクトリ・デバイス・キュー・各<t>ディスクリプタヒープ</t>を作る初期化。<code>CreateRhiDevice</code> から呼ばれる。", when: "通常は直接呼ばない。" },
    ...
    },
    {
      ...
      sample: "HrResult r = device.Init(cfg);\nif (r.IsErr()) { /* HRESULT は r.hr */ }",
      members: [
    ...
    },
    {
      ...
      when: "Diligent バックエンドでビルドした時に <code>CreateRhiDevice()</code> が生成する実体。<code>DeviceConfig::backend</code> で D3D12 / Vulkan / Auto を選べる(Vulkan は <code>ACS_DILIGENT_VULKAN=ON</code> 時のみ)。",
      sample: "DeviceConfig cfg;\ncfg.backend = ERhiBackendKind::Auto;   // 最良を自動選択\nauto dev = CreateRhiDevice(cfg).Value();\n// 実際に選ばれた API を見る:\n// static_cast&lt;DiligentDevice*&gt;(dev.get())-&gt;ActualBackend();",
      members: [
        ...
        { sig: "TResult<void> Init(const DeviceConfig& cfg)", ret: "成否", desc: "<code>EngineFactory</code> からデバイス・コンテキストを作る初期化。<code>CreateRhiDevice</code> から呼ばれる。", when: "通常は直接呼ばない。" },
    ...
    {
      name: "DiligentSwapchain",
      kind: "クラス", header: "render/Diligent/DiligentSwapchain.h",
      summary: "<t>Diligent</t> の<t>スワップチェイン</t>(<code>IRhiSwapchain</code> 派生)。Diligent の <code>ISwapChain</code> を薄くラップし、取得・提示・リサイズを <code>Dx12Swapchain</code> と同じ API で提供します。",
    ...
    {
      name: "DiligentCommandList",
      kind: "クラス", header: "render/Diligent/DiligentCommandList.h",
      summary: "<t>Diligent</t> の<t>コマンドリスト</t>(<code>IRhiCommandList</code> 派生)。Diligent は <code>IDeviceContext</code> 自体が即時 API なので、本クラスはその薄いラッパです。<code>Dx12CommandList</code> と同じインターフェイスで、加えて <b>MRT(複数 RT 同時描画)を実装</b>します。",
      ...
      members: [
        ...
        { sig: "void BeginRenderToSwapchain(...) / EndRenderToSwapchain(...)", desc: "バックバッファへの描画パス開始/終了。<code>Dx12CommandList</code> と同シグネチャ。" },
    ...
    },
    {
      ...
      summary: "<t>Diligent</t> の GPU バッファ(<code>IRhiBuffer</code> 派生)。Diligent の <code>IBuffer</code> を頂点・インデックス・定数バッファとして確保し、<code>Dx12Buffer</code> と同じ API で更新できます。",
    ...
    },
    {
      ...
      when: "<code>CreateRhiTexture()</code> が Diligent で生成する実体。使い方は <code>Dx12Texture</code> と同じく <code>FTextureDesc</code> のフラグで切り替える。",
    ...
    },
    {
      ...
      when: "<code>CreateRhiShader()</code> が Diligent で生成する実体。HLSL の渡し方は <code>Dx12Shader</code> と同じ。",
    ...
    },
    {
      ...
      when: "Diligent デバイス初期化時に、エンジン内部のメモリも ACS の予算管理・可視化に乗せたい場合。<code>DiligentDevice::Init</code> が内部で使う想定。",
      sample: "FAllocator& a = DefaultAllocator();\nvoid* memAlloc = DiligentMemoryAdapter::Create(&amp;a);\n// EngineCreateInfo::pRawMemAllocator = (Diligent::IMemoryAllocator*)memAlloc;",
      members: [
