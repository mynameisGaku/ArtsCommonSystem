  ...
  types: [
    ...
    },
    {
      ...
      members: [
        ...
        { sig: "TResult<void> AddFile(const wchar_t* virtual_path, const void* data, u64 size)", desc: "1 ファイルを追加する。<code>/</code> 区切りの正規形仮想パスと全バイトを内部配列へコピーし、path hash を一度保持する。重複確認は hash 一致候補だけを完全比較し、同名パスや不正な <code>.</code>/<code>..</code> segment は拒否する。成功後は呼び出し側が入力を直ちに再利用・解放してよい。", when: "梱包したいファイルを 1 つずつ登録していく時。" },
    ...
    },
    {
      ...
      members: [
        ...
        { sig: "TResult<u64> ReadFile(const wchar_t* path, void* out_buffer, u64 buffer_size)", ret: "書き込んだバイト数", desc: "ファイルを <code>out_buffer</code> へ読み出す。<code>buffer_size</code> は <code>GetUncompressedSize</code> 以上必要。読み出し後 <t>CRC32</t> を照合し、不一致(破損/改竄)はエラー。", when: "実際にファイルの中身が欲しい時。" },
