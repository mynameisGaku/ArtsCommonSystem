  ...
  types: [
    ...
    },
    {
      ...
      members: [
        ...
        { sig: "TResult<void> ReportCrash(const CrashContext& Context)", ret: "成否（<t>Result</t>）", desc: "クラッシュ 1 件を処理する。<b>ミニダンプ（.dmp）</b>と、例外種別・メッセージ・スタック・<t>パンくず</t>を含む<b>テキスト報告書</b>を出力フォルダに書く。", when: "自前のクラッシュハンドラからプロセスがまだ生きている間に呼ぶ。", sample: "acs::game::CrashContext ctx;\nctx.exception_type = \"SEH_ACCESS_VIOLATION\";\nctx.message        = \"null deref in Foo::Bar\";\nreporter.ReportCrash(ctx);" },
...
Object.assign(ACS_REF.glossary, {
  ...
  "CrashContext": "クラッシュ 1 件分の文脈（例外種別・メッセージ・スタック・シーン名等）をまとめた構造体（<t>gameframework</t> 側）。",
