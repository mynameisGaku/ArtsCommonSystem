  ...
  types: [
    ...
    },
    {
      ...
      members: [
        ...
        { sig: "TResult<void> Init(const char* ProductId, const char* Version)", ret: "成否（<t>Result</t>）", desc: "報告に付ける<b>製品 ID とバージョン</b>を控えて初期化する。両文字列の寿命は呼出側が保証。", when: "アプリ起動直後に一度だけ。"},
        ...
        { sig: "TResult<void> ReportCrash(const FCrashContext& Context)", ret: "成否（<t>Result</t>）", desc: "クラッシュ 1 件を処理する。<b>ミニダンプ（.dmp）</b>と、例外種別・メッセージ・スタック・<t>パンくず</t>を含む<b>テキスト報告書</b>を出力フォルダに書く。", when: "自前のクラッシュハンドラからプロセスがまだ生きている間に呼ぶ。"},
        { sig: "TResult<void> ReportError(const char* Category, const char* Message)", ret: "成否（<t>Result</t>）", desc: "<b>非致命的エラー</b>を 1 件、テキスト報告書として書き出す。<code>Category</code> は集計用のキー（\"net\" / \"save\" 等）。"},
        { sig: "TResult<void> AddBreadcrumb(const char* Category, const char* Message)", ret: "成否（<t>Result</t>）", desc: "クラッシュ前の文脈を<b>リングバッファ</b>に 1 件積む（最大 32 件、古いものから上書き）。送信はせず、次の <code>ReportCrash</code> でまとめて添付される。", when: "「どこまで進んでいたか」を残したい節目ごとに呼ぶ。"},
        { sig: "void SetUserId(const char* AnonymousId)", desc: "報告に付ける<b>匿名ユーザー ID</b>を設定する。GDPR/CCPA 対応のため、生の email でなく初回起動時に作った UUID 等を渡す。"},
