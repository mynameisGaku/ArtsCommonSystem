...
ACS_REF.modules.push({
  ...
  blurb: "ゲーム制作を支援する <b>ImGui ベースのエディタ部品群</b>(<code>acs::game::tools</code>)。パネル基底 <code>FEditorPanel</code> とワークスペース <code>FEditorWorkspace</code> を土台に、シーン階層 / インスペクタ / アセットブラウザ / ギズモ、そして Particle・Level・SpriteAtlas・Font・BehaviorTree・Cinematics・ModelViewer といった専用エディタが載る。サンプル 29〜37 がこの層を使う。",
  types: [
{
  name: "FEditorPanel",
  ...
  when: "新しいエディタ panel を作る時に継承する。<code>FEditorWorkspace</code> に登録すると毎フレーム自動で駆動される。",
  sample: "class FMyPanel : public acs::game::editor_core::FEditorPanel {\npublic:\n    const char* Title() const noexcept override { return \"My Panel\"; }\n    void DrawUI() noexcept override {\n        if (!IsVisible()) return;\n        // ... ImGui::Begin(Title(), &amp;m_Visible) ... ImGui::End()\n    }\n};",
  members: [
    ...
    { sig: "virtual void OnInit(FEditorWorkspace&amp; workspace) noexcept", ret: "—", desc: "Workspace 登録時に 1 度呼ばれる。基底実装が <code>workspace</code> 参照を保存し以降 <code>Workspace()</code> で取得可能にする。override する場合は基底呼び出しを先に。", when: "追加初期化が要る時。" },
    ...
    { sig: "virtual void OnSelectionChanged(inspector::FSelectionService&amp; selection) noexcept", ret: "—", desc: "Scene 内 Node の選択が変わった時に呼ばれる(Phase 20 <code>FSelectionService</code> 統合)。既定 no-op。", when: "Inspector 等 node 系 panel。" },
    { sig: "virtual void OnAssetSelected(const char* asset_path) noexcept", ret: "—", desc: "<code>FAssetBrowser</code> からファイル選択された時の通知。<code>asset_path</code> は Browser 所有文字列(コピー禁止)。<code>nullptr</code> は「選択解除」。既定 no-op。", when: "ModelViewer 等 asset 系 panel。" },
    ...
    { sig: "virtual void OnSaveLayout(EditorLayoutSerializer&amp; out) noexcept / OnLoadLayout(... in)", ret: "—", desc: "panel 独自のレイアウト情報(splitter 比率 / カラム幅 / 表示モード)の永続化予約点。Phase 21c 本実装、現状 stub(no-op)。" },
    ...
    { sig: "FEditorWorkspace* Workspace() const noexcept", ret: "Workspace*", desc: "<code>OnInit</code> で保存された Workspace ポインタ(non-owning)。OnInit 前 / OnShutdown 後は <code>nullptr</code>。" }
  ]
},
{
  name: "FEditorWorkspace",
  kind: "クラス(サービス hub)", header: "gameframework/tools/editor_core/EditorWorkspace.h",
  summary: "<b>複数の <code>FEditorPanel</code> を統括するワークスペース</b>。panel 群の登録・探索、毎フレームの main loop(<code>OnFrameBegin</code> → DockSpace → MenuBar → <code>DrawUI</code>)、ImGui DockSpace / FWindow メニュー、<code>.acslayout</code> でのレイアウト永続化、選択 / asset 選択イベントの全 panel への broadcast を担う。panel は <b>raw pointer の非所有保持</b>(caller 所有)で、登録順 = dispatch 順。非コピー / 非ムーブ、全 <code>noexcept</code>。",
  when: "複数 panel を 1 つの editor アプリとしてまとめて配線する中央 hub が要る時。",
  sample: "FEditorWorkspace ws;\nws.Init();\nws.RegisterPanel(&amp;inspector_panel);   // panel は caller 所有\nws.RegisterPanel(&amp;model_viewer);\n// 毎フレーム:\nws.TickAllPanels(dt);\n// 終了時:\nws.Shutdown();",
  members: [
    ...
    { sig: "void RegisterPanel(FEditorPanel* panel) noexcept", ret: "—", desc: "panel を末尾追加し <code>OnInit</code> を呼ぶ。所有は caller。二重登録 / <code>nullptr</code> は no-op。<code>kMaxPanels</code> 到達後も silent no-op。" },
    { sig: "void UnregisterPanel(FEditorPanel* panel) noexcept", ret: "—", desc: "登録解除して <code>OnShutdown</code> を呼ぶ。順序保存削除(shift)。未登録 / <code>nullptr</code> は no-op。" },
    { sig: "u32 PanelCount() const noexcept", ret: "件数", desc: "現在の登録 panel 数。" },
    { sig: "FEditorPanel* GetPanelByIndex(u32 i) const noexcept", ret: "panel or null", desc: "<code>i</code> 番目の panel。範囲外は <code>nullptr</code>。" },
    { sig: "FEditorPanel* FindPanelByTitle(const char* title) const noexcept", ret: "panel or null", desc: "<code>Title()</code> が完全一致(strcmp)する panel。なし / <code>nullptr</code> は <code>nullptr</code>。" },
    ...
    { sig: "void DrawMenuBar() noexcept", ret: "—", desc: "MainMenuBar に \"FWindow\"(panel toggle) / \"Layout\"(Save / Load) メニューを追加。<code>SetEnableMenuBar(false)</code> で抑制可。" },
    ...
    { sig: "void SetSelectionService(inspector::FSelectionService* svc) noexcept", ret: "—", desc: "<code>FSelectionService</code> 参照を登録 / 解除(<code>nullptr</code> で解除)。caller 所有。" },
    { sig: "inspector::FSelectionService* GetSelectionService() const noexcept", ret: "service*", desc: "登録中の selection service。未注入時 <code>nullptr</code>。" },
...
},
{
  ...
  summary: "<b>拡張子から推測されるアセット種別</b>。<code>FAssetBrowser</code> の kind フィルタ / アイコン表示 / <code>ClassifyByExtension</code> の戻り値に使う。<code>Unknown</code> は未分類(未知拡張子 or path が <code>nullptr</code> / 空)、<code>Other</code> は既知だがどの分類にも該当しない汎用ファイル(現状未使用 / 利用側拡張用)。",
...
},
{
  ...
  summary: "<b>列挙された 1 件のディレクトリ / ファイル</b>。<code>path</code> / <code>short_name</code> は <code>FAssetBrowser</code> の内部 pool を指し、<b>寿命は次の <code>Refresh()</code> まで</b>(再列挙で pool がクリアされ pointer 無効化)。保持したいなら利用側でバッファに退避する。",
  when: "<code>FAssetBrowser::GetEntry</code> で current directory のエントリを 1 件ずつ読む時。",
  members: [
...
},
{
  ...
  members: [
    ...
    { sig: "void Refresh() noexcept", ret: "—", desc: "current directory を rescan。<b>pool が再構築され既存 <code>AssetEntry::path</code> / <code>short_name</code> が無効化される</b>ので Refresh 越しに pointer を保持しないこと。" },
    ...
    { sig: "const AssetEntry* GetEntry(u32 index) const noexcept", ret: "entry or null", desc: "<code>index</code> 番目の entry。範囲外は <code>nullptr</code>。寿命は次の Refresh まで。" },
...
},
{
  ...
  when: "<code>FEditorGizmo::SetMode</code> で translate / rotate / scale を切り替える時。",
  members: [
...
},
{
  name: "EGizmoSpace",
  kind: "列挙(enum class : u8)", header: "gameframework/tools/editor_core/EditorGizmo.h",
  summary: "<b>ギズモの操作座標系</b>。<code>World</code> は固定ワールド軸、<code>Local</code> は対象 Node の rotation を考慮したローカル軸を表示・操作する。",
  when: "<code>FEditorGizmo::SetSpace</code> で world / local を切り替える時。",
  members: [
    { sig: "World = 0", desc: "ワールド軸(X/Y/Z 固定)。既定。" },
    { sig: "Local = 1", desc: "Node のローカル軸(rotation を考慮)。" }
  ]
},
{
  name: "EGizmoAxis",
  ...
  when: "<code>FEditorGizmo::HotAxis()</code> や <code>GizmoState::hot_axis</code> で現在 hot な軸を読む時。",
  members: [
...
},
{
  ...
  summary: "<b>現フレームのギズモ操作状態</b>。公開フィールドだが書き換えは <code>FEditorGizmo</code> 内部のみ。テストや inspector から「今 drag 中か」「どの軸が hot か」を <code>ImGui::Text</code> 等で覗ける。<code>drag_start_world</code> は drag 開始時の world ヒット点(delta 計算の基点)。",
  when: "<code>FEditorGizmo::State()</code> でギズモの内部状態を読み取る時。",
  members: [
...
},
{
  ...
  summary: "<b>drag 終了時に外部へ delta を通知する callback 型</b>。drag 完了時に 1 度だけ呼ばれ、<code>FUndoStack</code> に <code>MoveNodeCommand</code> 等を push する適切なタイミングになる。<code>delta</code> はモード依存(Translate=移動量 / Rotate=euler 差分[radians] / Scale=倍率差分)。<code>user</code> は <code>SetOnManipulateCallback</code> の第二引数がそのまま戻る。",
...
},
{
  ...
  summary: "<b>選択 Node の Transform を viewport 上で直接ドラッグ操作するハンドル</b>。translate / rotate / scale の 3 モードを持ち、各モードで X/Y/Z 軸 + 平面 + 画面並列ハンドルを提供する。<b><code>ProcessInput</code> → <code>Manipulate</code> → <code>DrawGizmo</code> の 3 段</b>に完全分離(headless test 可)。描画は <code>FDebugDraw</code> 経由でレンダラ非依存(現状 2D 射影 = top-down)。非コピー / 非ムーブ、全 <code>noexcept</code>。",
  when: "シーンビュー上で Node の移動 / 回転 / 拡縮ハンドルを実装する時。",
  sample: "FEditorGizmo gizmo;\ngizmo.Init();\ngizmo.SetMode(EGizmoMode::Translate);\ngizmo.ProcessInput(ray_o, ray_d, lmb_down, lmb_held, lmb_up);\nif (gizmo.Manipulate(pos, rot, scl)) {\n    node.SetWorldPosition(pos);\n}\ngizmo.DrawGizmo(debug_draw, pos, rot, scl);",
  members: [
    ...
    { sig: "void DrawGizmo(FDebugDraw&amp; dd, acs::FVec3 position, acs::FVec3 rotation_euler, acs::FVec3 scale) noexcept", ret: "—", desc: "<code>FDebugDraw</code> 経由で軸 line + ハンドルを描く。FDebugDraw が 2D のため Z は XY 平面へ射影。色: X=red / Y=green / Z=blue / 平面=半透明黄 / hot=白。" },
    { sig: "bool IsDragging() const noexcept / EGizmoAxis HotAxis() const noexcept", ret: "状態", desc: "drag 中か / 現在 hot な軸を返す。" },
    { sig: "const GizmoState&amp; State() const noexcept", ret: "GizmoState&amp;", desc: "<code>GizmoState</code> の現値をまるごと参照で返す(テスト / inspector 用)。" },
...
},
{
  ...
  summary: "<b><code>FEditorCamera</code> の操作モデル</b>。<code>Mode2D</code> は pan / zoom のみ(top-down / tilemap editor)、<code>Mode3D</code> は target を中心に orbit + pan + dolly(model viewer)。",
  when: "<code>FEditorCamera::Init</code> / <code>SetMode</code> で 2D / 3D を選ぶ時。",
  members: [
...
},
{
  ...
  summary: "<b>カメラの公開 state</b>。debug overlay / serializer から position / target 等を直接覗くための可視 struct(<code>FEditorCamera::State()</code> が const 参照で返す)。直接書き換えは <code>SetTarget</code> 等の accessor 経由推奨(smooth_target との 2 重管理が崩れるため)。",
...
},
{
  ...
  summary: "<b><code>FUndoStack</code> の Push / Undo / Redo 後の副反応 callback 型</b>。「Undo されたから hierarchy 再描画」「Redo されたから dirty 立て直し」等を書くための出口。<code>cmd</code> は最終的にスタックに載った command(merge があれば top)、<code>is_redo</code> は Redo 経路なら <code>true</code>(Push / Undo は <code>false</code>)。",
  when: "undo / redo 操作に連動した UI 更新や dirty フラグ管理が要る時。",
  members: [
    { sig: "using CommandExecutedCallback = void (*)(void* user, const class FEditorCommand* cmd, bool is_redo) noexcept", desc: "副反応通知。<code>user</code> は <code>SetOnExecutedCallback</code> の第二引数がそのまま戻る。" }
  ]
},
{
  name: "FEditorCommand",
  ...
  members: [
    ...
    { sig: "virtual bool CanMerge(const FEditorCommand&amp; next) const noexcept", ret: "merge 可否", desc: "直後に Push される <code>next</code> を自分に merge 可能か返す。既定 <code>false</code>。同対象 + 同 kind のときのみ true を返すよう派生で実装。" },
    { sig: "virtual void MergeWith(const FEditorCommand&amp; next) noexcept", ret: "—", desc: "<code>CanMerge</code> が true の直後に <code>FUndoStack</code> が呼ぶ。<code>next</code> の「new 値」を取り込み(old 値は保持)。既定 no-op。" }
  ]
},
{
  name: "MoveNodeCommand",
  kind: "クラス(FEditorCommand 派生サンプル)", header: "gameframework/tools/editor_core/EditorCommand.h",
  summary: "<b><code>FNode2D</code> の position を変更する <code>FEditorCommand</code> 派生の教科書的サンプル</b>。old / new 位置を保持し、<code>Execute</code> で new、<code>Undo</code> で old を <code>Target-&gt;Local().position</code> に書き込む。<b>同一 target の連続 <code>MoveNodeCommand</code> を 1 件にまとめる <code>CanMerge</code> / <code>MergeWith</code></b> を実装(Kind tag のポインタ一致 + target 一致で判定、new 値だけ更新し old 値は始点を保持)。",
  when: "ドラッグでノード位置を変えた操作を undo 可能にする時のひな形として。",
  sample: "const acs::FVec2 old_pos = node.Local().position;\nconst acs::FVec2 new_pos = old_pos + delta;\nauto c = acs::MakeUnique<MoveNodeCommand>(&amp;node, old_pos, new_pos);\nundo_stack.Push(c.Release());",
  members: [
    { sig: "MoveNodeCommand(FNode2D* target, FVec2 old_pos, FVec2 new_pos) noexcept", ret: "—", desc: "対象ノードと変更前 / 後の位置で構築。" },
    { sig: "void Execute() noexcept override / void Undo() noexcept override", ret: "—", desc: "<code>target != nullptr</code> のとき <code>Local().position</code> を new / old に書き込む。" },
    ...
    { sig: "bool CanMerge(const FEditorCommand&amp; next) const noexcept override", ret: "merge 可否", desc: "<code>next.Kind()</code> が一致 かつ target ポインタが一致するときだけ <code>true</code>(連続 drag 用)。" },
    { sig: "void MergeWith(const FEditorCommand&amp; next) noexcept override", ret: "—", desc: "<code>m_NewPos</code> を next の new 値で上書き(old 値 = drag 始点は保持)。" },
    { sig: "const FNode2D* Target() const / FVec2 OldPosition() const / FVec2 NewPosition() const", ret: "値", desc: "テスト / inspector 表示用アクセサ。" }
  ]
},
{
...
},
{
  ...
  when: "<code>PropertyDrawerRegistry::RegisterDrawer</code> に渡すカスタム drawer を定義する時。",
  members: [
    { sig: "using DrawerFn = void (*)(const PropertyContext&amp; ctx) noexcept", desc: "<code>ctx</code> を受け取って 1 property を描画する関数ポインタ型。" }
  ]
},
{
  name: "PropertyDrawerRegistry",
  kind: "クラス(サービス)", header: "gameframework/tools/editor_core/PropertyDrawer.h",
  summary: "<b><code>type_name</code>(const char* literal)→ <code>DrawerFn</code> の登録レジストリ</b>。<code>FInspectorPanel</code> の hardcode switch を超える「ゲーム固有 / エディタ拡張型」の field 表示を後付け登録できる拡張点。<code>Init()</code> が <b>bundled drawer 9 種</b>(F32Slider / Vec2Drag / Vec3Drag / Vec4Drag / ColorRGB / ColorRGBA / AssetPath / EnumCombo / TextInput)を自動登録する。同名は<b>後勝ち</b>で上書き。内部は線形探索(少数想定)。非コピー / 非ムーブ、全 <code>noexcept</code>。",
  when: "ゲーム固有型(Health / WeaponSlot 等)を inspector で美麗表示したい / field drawer を差し替えたい時。",
  sample: "PropertyDrawerRegistry reg;\nreg.Init();   // bundled 9 種を自動登録\nreg.RegisterDrawer(\"Health\", &amp;DrawHealth);\nPropertyContext ctx{};\nif (!reg.DrawProperty(\"Health\", ctx)) {\n    // 未登録 → 既存 EFieldKind switch にフォールバック\n}",
  members: [
    ...
    { sig: "bool DrawProperty(const char* type_name, const PropertyContext&amp; ctx) const noexcept", ret: "描画した", desc: "該当 drawer を呼んで <code>ctx</code> で描画。成功なら <code>true</code>、未登録 / <code>nullptr</code> なら <code>false</code>(呼び出し側はフォールバックする)。" },
...
},
{
  ...
  summary: "<b><code>FEditorTheme</code> のプリセット種別</b>。<code>ApplyPreset</code> に渡すと色 / spacing / corner のスナップショットが適用される。<code>SetCustomColors</code> を呼ぶと自動で <code>Custom</code> になる。",
  when: "エディタの配色を切り替える時。",
  members: [
...
},
{
  ...
  members: [
    ...
    { sig: "void SetCustomColors(const EditorThemeColors&amp; colors) noexcept", ret: "—", desc: "任意パレットを設定し preset を <code>Custom</code> に切替・即時適用。各色は RGBA <code>[0,1]</code> 想定(clamp しない = HDR 許可)。" },
    { sig: "const EditorThemeColors&amp; Colors() const noexcept", ret: "colors&amp;", desc: "現在のカラーパレットを const 参照で取得。" },
    ...
    { sig: "void SetRoundedCorners(f32 radius) noexcept / f32 RoundedCorners() const", ret: "—", desc: "全 corner radius(FWindow / Frame / Popup / Grab / Tab / Scrollbar)を統一 / 取得。負値は 0 に clamp。" },
    ...
    { sig: "void DrawThemeSettingsUI() noexcept", ret: "—", desc: "\"Theme FSettings\" 独立 window を描画(preset combo + 全 ColorEdit4 + slider + Save/Load)。毎フレーム 1 行で呼ぶ。" },
...
{
  name: "FInspectorPanel",
  kind: "クラス(FEditorPanel 派生)", header: "gameframework/tools/inspector/InspectorPanel.h",
  summary: "選択中の Node に紐づく <t>FInspectorSeam</t> 経由の <code>InspectableField</code> 配列を、ImGui の field widget (Checkbox / InputInt / SliderFloat / DragFloatN / ColorEdit / InputText / Combo 等) として描画・編集する<b>プロパティエディタ</b>パネル。値の<b>永続化や undo は行わず</b>、変更を <t>FieldChangeCallback</t> で外部へ通知するだけ。<code>editor_core::FEditorPanel</code> を継承し、ヘッダから ImGui 依存は漏らさない。<b>ランタイム側の同名 FInspectorSeam とは別レイヤの UI 部品</b>。",
  when: "<code>FHierarchyPanel</code> 等で選択された Node の各フィールドを GUI で編集させたい時。<code>SetInspectorSeam</code> で provider 解決元を、<code>SetSelectionService</code> で選択取得元を注入し、毎フレーム <code>DrawUI()</code> を呼ぶ。",
  sample: "FInspectorPanel insp;\ninsp.Init();\ninsp.SetInspectorSeam(&amp;seam);        // Node → provider 解決元\ninsp.SetSelectionService(&amp;sel);       // 現在の選択を共有\n// 毎フレーム:\ninsp.DrawUI();\nif (insp.IsAnyFieldDirty()) {          // 何か編集された\n    // ... 永続化 / undo 記録 ...\n    insp.ClearDirtyFlag();\n}",
  members: [
    { sig: "using FieldChangeCallback = void (*)(void* user, FNodeId target, const char* field_name, EFieldKind kind) noexcept", desc: "field 変更通知 <t>コールバック</t>型。<code>user</code> は <code>SetOnFieldChangeCallback</code> 第二引数がそのまま戻る。<code>field_name</code> は Provider 所有のリテラル (<code>InspectableField::name</code>)。", when: "undo / dirty tracker / 永続化を外部に委譲する時。" },
    ...
    { sig: "void SetInspectorSeam(FInspectorSeam* seam) noexcept", ret: "—", desc: "Node → Provider 解決に使う <code>FInspectorSeam</code> を事前 set する。<code>DrawUI</code> 内で <code>seam.GetProvider(node_id)</code> を呼ぶ。" },
    { sig: "FInspectorSeam* InspectorSeamPtr() const noexcept", ret: "seam ポインタ", desc: "現在設定中の <code>FInspectorSeam</code> を返す。未設定なら nullptr。" },
    { sig: "const char* Title() const noexcept override", ret: "\"Inspector\"", desc: "ImGui window タイトル。" },
    { sig: "void DrawUI() noexcept override", ret: "—", desc: "<code>Begin(\"Inspector\")</code> で 1 window を描画。選択は <code>FSelectionService</code> 設定時はその <code>CurrentSelection()</code> を採用。Provider が取れなければ \"(No provider)\"、選択無しなら \"(Nothing selected)\" を表示。", when: "毎フレーム。" },
    { sig: "void SetSelectionService(FSelectionService* svc) noexcept", ret: "—", desc: "選択取得元の <code>FSelectionService</code> を注入する。nullptr で解除。保持期間は呼び出し側責務 (non-owning)。" },
    ...
    { sig: "static constexpr f32 kDefaultSliderMin = -1000.0f", desc: "F32 を <code>SliderFloat</code> で編集する際の既定下限。<code>InspectableField</code> に min/max metadata が無い間の暫定値。" },
...
{
  name: "FSelectionService",
  kind: "クラス", header: "gameframework/tools/inspector/SelectionService.h",
  summary: "今「選択されている <code>FNodeId</code>」を全 editor panel (<code>FHierarchyPanel</code> / <code>FInspectorPanel</code> / <code>FEditorToolbar</code> / SceneView 等) で共有するための<b>選択ハブ</b>。単一の選択だけを持ち、変化時に登録済み <t>コールバック</t>を <code>(from, to)</code> 付きで一斉発火する。<code>FGame</code> / <code>FSceneManager</code> には依存せず、対象 Node が生きているかの検証は購読側責務。STL 不使用 (内部は <code>TArray</code>)、非コピー・非ムーブ。",
...
{
  name: "FHierarchyPanel",
  kind: "クラス(FEditorPanel 派生)", header: "gameframework/tools/inspector/HierarchyPanel.h",
  summary: "<code>FNode2D</code> ツリー (シーン) を ImGui の hierarchy パネルで可視化・編集する、Unity の Hierarchy / UE の World Outliner 相当の panel。root 配下を再帰 <code>TreeNode</code> で描画し、クリックで選択 (<code>FSelectionService</code> 経由 or 内部保持)、右クリックで Delete / Duplicate / Reparent、Drag &amp; Drop で reparent、ExpandAll / CollapseAll で一括開閉する。<code>editor_core::FEditorPanel</code> を継承。STL 不使用 (折りたたみ状態は <code>TArray</code> の linear search)。",
  when: "シーンの Node 階層を GUI で見せ・選択・削除・親子付けさせたい時。<code>SetRootNode</code> で起点 Node を、必要なら <code>SetSelectionService</code> で選択を他 panel と共有し、毎フレーム <code>DrawUI()</code> を呼ぶ。",
  sample: "FHierarchyPanel hier;\nhier.Init();\nhier.SetRootNode(&amp;scene_root);\nhier.SetSelectionService(&amp;sel);   // Inspector と選択を共有\n// 毎フレーム:\nhier.DrawUI();",
  members: [
    { sig: "using NodeRightClickCallback = void (*)(void* user, FNode2D* node) noexcept", desc: "右クリック context menu の<b>追加</b>項目を外部委譲する <t>コールバック</t>型。標準の Delete / Duplicate / Reparent はパネル内描画で、これは \"Create Child\" 等の追加項目のみ担当。<code>user</code> は登録時の第二引数。" },
    ...
    { sig: "void SetRootNode(FNode2D* root) noexcept", ret: "—", desc: "再帰描画の起点となる root Node を事前 set する。root 自体も最上位 <code>TreeNode</code> として表示される。" },
    { sig: "FNode2D* RootNode() const noexcept", ret: "root ポインタ", desc: "現在設定中の root Node を返す。" },
    ...
    { sig: "void SetSelectionService(FSelectionService* svc) noexcept", ret: "—", desc: "選択ハブを注入する。nullptr で内部 selection モードに戻る。" },
    { sig: "FNodeId SelectedNodeId() const noexcept", ret: "選択 id", desc: "現選択 Node の <code>FNodeId</code>。<code>FSelectionService</code> 注入時はそちら経由、なければ内部値。未選択は <code>FNodeId{}</code> (packed==0)。" },
    { sig: "void SelectNode(FNode2D* node) noexcept", ret: "—", desc: "選択を <code>node</code> に切り替える。nullptr で選択解除。<code>FSelectionService</code> 注入時はそちらにも反映する。" },
    ...
    { sig: "void DeleteSelected() noexcept", ret: "—", desc: "選択中 Node に <code>FNode2D::Destroy()</code> を呼ぶ (実 destroy は次フレームの structural change 解決で)。選択は解除される。未選択時は no-op。" },
...
},
{
  ...
  summary: "<code>FEditorToolbar</code> が保持する editor の<b>再生状態</b>。Playing / Paused / Stepping の 3 値で、<code>FGame</code> の TimeScale 反映 (Play=通常値 / Pause=0) と 1 step 進行を表す。",
  when: "ツールバーの再生制御状態を読み取りたい時 (<code>FEditorToolbar::State()</code> の戻り値)。",
  members: [
...
{
  name: "FEditorToolbar",
  kind: "クラス(FEditorPanel 派生)", header: "gameframework/tools/inspector/EditorToolbar.h",
  summary: "editor ウィンドウ上端の<b>ツールバー</b>。Play / Pause / Step の再生制御、Scene 保存、FDebugOverlay 表示切替の 5 アクションを 1 行に並べる。再生状態を <t>EEditorState</t> 1 個で保持し、<code>FGame::SetTimeScale</code> への反映を本クラスが <code>DrawUI</code> 内で行う。Save は外部委譲 (<t>SaveSceneCallback</t> 発火のみ)、FDebugOverlay は bool flag のみ持ち実描画は外側責務。<code>editor_core::FEditorPanel</code> を継承。",
  when: "editor の中央バーから再生制御・保存・デバッグ表示を出したい時。<code>SetGame</code> で対象 <code>FGame</code> を、<code>SetOnSaveSceneCallback</code> で保存処理を注入し、毎フレーム <code>DrawUI()</code> を呼ぶ。",
  sample: "FEditorToolbar toolbar;\ntoolbar.Init();\ntoolbar.SetGame(&amp;game);\ntoolbar.SetOnSaveSceneCallback(&amp;SaveSceneToDisk, &amp;my_editor);\n// 毎フレーム:\ntoolbar.DrawUI();   // 内部で FGame::SetTimeScale(0/1) を切り替える",
  members: [
    ...
    { sig: "void SetGame(FGame* game) noexcept", ret: "—", desc: "TimeScale 反映先の <code>FGame</code> を事前 set する。nullptr のときは UI 更新のみで反映を skip。" },
    { sig: "FGame* GameRef() const noexcept", ret: "game ポインタ", desc: "現在設定中の <code>FGame</code> を返す。" },
    { sig: "const char* Title() const noexcept override", ret: "\"Editor Toolbar\"", desc: "ImGui window タイトル。" },
    { sig: "void DrawUI() noexcept override", ret: "—", desc: "Play / Pause / Step / Save / FDebugOverlay ボタン行を描画する。<code>m_Game</code> が nullptr なら TimeScale 反映を skip (UI のみ動作)。", when: "毎フレーム。" },
    { sig: "void TogglePlayPause() noexcept", ret: "—", desc: "Play ↔ Pause を切り替える。Playing→Paused で現 TimeScale を記録して 0 に、Paused→Playing で記録値を書き戻す。<code>FGame</code> への実反映は <code>DrawUI</code> 内。" },
    ...
    { sig: "void SetShowDebugOverlay(bool b) noexcept", ret: "—", desc: "FDebugOverlay の表示要望 flag を設定する。実描画は外側コードが flag を見て切り替える。" },
    { sig: "bool ShowDebugOverlay() const noexcept", ret: "表示要望", desc: "FDebugOverlay 表示要望 flag を返す。" }
  ]
},
{
  name: "FFxeditSerializer",
  kind: "クラス(static ユーティリティ)", header: "gameframework/tools/fxedit/FxeditSerializer.h",
  summary: "ParticleEditor が編集中の <code>ParticleEmitterDef</code> 群を、人間可読で <t>git diff</t> しやすい <code>.fxedit</code> テキスト形式で保存・復元する <t>シリアライザ</t>。バイナリの <code>FSaveSlot</code> と違い、<b>作業中アセットをそのまま版管理に乗せる</b>ことを優先する。全メンバ <code>static</code>、生成・コピー・ムーブはすべて <code>= delete</code>。",
  when: "emitter 定義をテキストでセーブ/ロードしたい時。1 行 1 key=value なので 1 パラメータ変更が 1 行 diff になる。ファイル I/O は <code>acs::FileSystem</code> 経由。",
  sample: "acs::game::ParticleEmitterDef defs[2] = {};\ndefs[0].lifetime_sec = 2.0f;\nconst char* names[] = {\"fire\", \"smoke\"};\nnamespace fx = acs::game::fxedit;\nfx::FFxeditSerializer::Save(L\"data/fx.fxedit\", defs, names, 2);\n\nacs::game::ParticleEmitterDef loaded[16] = {};\nchar name_buf[16 * 32] = {};\nauto r = fx::FFxeditSerializer::Load(L\"data/fx.fxedit\", loaded, name_buf, sizeof(name_buf), 16);\nif (r.IsOk()) { acs::u32 n = r.Value(); (void)n; }",
  members: [
    { sig: "static TResult&lt;void, FErrorCode&gt; Save(const wchar_t* file_path, const ParticleEmitterDef* defs, const char* const* names, u32 count) noexcept", ret: "成否", desc: "emitter 群を <code>.fxedit</code> テキストへ書き出す。既存ファイルは上書き。<code>names</code> ポインタ自体が nullptr なら <code>kSub_NullArgs</code>。count=0 はヘッダだけ書く。", when: "セーブ時。" },
    { sig: "static TResult&lt;u32, FErrorCode&gt; Load(const wchar_t* file_path, ParticleEmitterDef* out_defs, char* out_name_buffer, u32 name_buffer_capacity, u32 max_emitters) noexcept", ret: "ロードした emitter 数", desc: "<code>.fxedit</code> から emitter 群を読む。名前は <code>name0\\0name1\\0...</code> 形式で <code>out_name_buffer</code> に連結配置。成功時の値は実際にロードした個数 (&lt;= max_emitters)。", when: "ロード時。" },
    ...
    { sig: "enum SubCode : u16 { ... }", desc: "<code>FErrorCode</code> のサブコード (700〜707)。<code>kSub_NullArgs=700</code> / <code>kSub_TooManyEmitters=701</code> / <code>kSub_BufferOverflow=702</code> / <code>kSub_BadMagic=703</code> / <code>kSub_BadVersion=704</code> / <code>kSub_BadFormat=705</code> / <code>kSub_FileNotFound=706</code> / <code>kSub_IOFailure=707</code>。" }
  ]
},
{
  name: "FParticleEditorPanel",
  kind: "クラス(FEditorPanel 派生)", header: "gameframework/tools/fxedit/ParticleEditorPanel.h",
  summary: "<code>ParticleEmitterDef</code> を ImGui で実機編集するための emitter プロパティエディタ <t>パネル</t>。<code>editor_core::FEditorPanel</code> を継承し、内部に <code>TArray&lt;ParticleEmitterDef&gt;</code> を持つ。emitter handle や pool への反映 (CreateEmitter 等) は呼び出し側の責務で、本 panel は <b>def の編集のみ</b>を担う。Save / Load は callback hook に委譲。非コピー・非ムーブ・全 noexcept。",
  when: "Editor / DevTool ビルドで emitter パラメータをライブ編集したい時。<code>FEditorWorkspace::RegisterPanel(&amp;panel)</code> で登録する。",
  sample: "namespace fx = acs::game::fxedit;\nfx::FParticleEditorPanel panel;\npanel.Init();\npanel.AddEmitter();\nif (auto* def = panel.GetEmitterDefMutable(0)) {\n    def-&gt;lifetime_sec = 1.5f;\n}\nif (panel.IsDirty()) {\n    // ... 保存 ...\n    panel.ClearDirty();\n}\npanel.Shutdown();",
  members: [
    { sig: "using SaveCallback = void (*)(void* user, const ParticleEmitterDef* defs, u32 count) noexcept", desc: "Save ボタン押下時に呼ばれる関数ポインタ型。<code>user</code> は登録時に渡したポインタがそのまま戻る。" },
    { sig: "using LoadCallback = void (*)(void* user, ParticleEmitterDef* defs, u32& inout_count) noexcept", desc: "Load ボタン押下時に呼ばれる関数ポインタ型。" },
    ...
    { sig: "void SetTargetSystem(FParticleEffectSystem* system) noexcept", ret: "—", desc: "active particle 数表示用の read-only target system を set する。", when: "DrawUI 前。" },
    { sig: "FParticleEffectSystem* TargetSystem() const noexcept", ret: "system or nullptr", desc: "現在 set されている target system を返す。" },
    { sig: "const char* Title() const noexcept override", ret: "\"Particle Editor\"", desc: "FEditorPanel override: window タイトル。" },
    { sig: "void DrawUI() noexcept override", ret: "—", desc: "FEditorPanel override: <code>Begin(\"Particle Editor\")</code> から始まる単一 window レイアウトを描画。" },
    ...
    { sig: "const ParticleEmitterDef* GetEmitterDef(i32 index) const noexcept", ret: "def or nullptr", desc: "index 番目の emitter def (read-only)。範囲外は nullptr。" },
    { sig: "ParticleEmitterDef* GetEmitterDefMutable(i32 index) noexcept", ret: "def or nullptr", desc: "index 番目の emitter def (mutable)。範囲外は nullptr。書き換え後は dirty 想定。" },
...
{
  name: "FParticleEditorPreview",
  kind: "クラス", header: "gameframework/tools/fxedit/ParticleEditorPreview.h",
  summary: "編集中の <code>ParticleEmitterDef</code> を 1 個だけ instance 化して、リアルタイムに visual feedback を返す <b>preview canvas + stats</b>。外部の <code>FParticleEffectSystem</code> 上に 1 emitter を立てる方式で system は所有しない。frame budget は 60-frame の <t>リングバッファ</t>で保持。非コピー・非ムーブ・全 noexcept。",
  when: "ParticleEditor で emitter を編集しながら見た目を即時確認したい時。値変更時に <code>RecreatePreviewEmitter</code> で再生成すると即反映される。",
  sample: "namespace fx = acs::game::fxedit;\nfx::FParticleEditorPreview preview;\npreview.Init();\nacs::game::FParticleEffectSystem system;  // 別途 Init 済とする\nacs::game::ParticleEmitterDef def {};\npreview.RecreatePreviewEmitter(system, &amp;def);\npreview.Tick(0.016f, system);\npreview.DrawUI(system, &amp;def);\npreview.StopAll(system);\npreview.Shutdown();",
  members: [
    ...
    { sig: "void Tick(f32 dt, FParticleEffectSystem& system) noexcept", ret: "—", desc: "1 フレーム進める。<code>dt &lt;= 0</code> は ring に push せず無視。system 側の Tick は呼ばない (呼出側責務)。", when: "毎フレーム。" },
    { sig: "void DrawUI(FParticleEffectSystem& system, const ParticleEmitterDef* def) noexcept", ret: "—", desc: "<code>\"Particle Preview\"</code> window を描画。<code>def == nullptr</code> なら \"no def selected\" を表示。" },
    { sig: "void RecreatePreviewEmitter(FParticleEffectSystem& system, const ParticleEmitterDef* def) noexcept", ret: "—", desc: "編集中 emitter を即時再生成 (既存 handle が valid なら Destroy 後、新 def の copy で Create)。<code>def == nullptr</code> は no-op。", when: "def 変更時。" },
    { sig: "void TriggerBurst(FParticleEffectSystem& system) noexcept", ret: "—", desc: "<code>burst_count</code> に従い 1 回 Burst (handle が valid のときのみ)。" },
    { sig: "void StopAll(FParticleEffectSystem& system) noexcept", ret: "—", desc: "preview emitter を破棄 + particle pool 全消去。編集セッションをクリーンに戻す。" },
...
},
{
  ...
  summary: "<code>FLevelEditorPanel</code> のペイントブラシ種別。<t>tilemap</t> 上のマウス操作の意味を切り替える。E-prefix は ACS 規約。",
...
{
  name: "FLevelEditorPanel",
  kind: "クラス(FEditorPanel 派生)", header: "gameframework/tools/leveledit/LevelEditorPanel.h",
  summary: "<code>FTilemap</code> (multi-layer u16 FTileId grid) を対話的に編集する <b>tilemap painter パネル</b>。Paint / Erase / Fill / Pick の 4 ブラシ + アクティブレイヤ切替 + tile id picker + grid 表示 + snap-to-grid を提供。2D 用 <code>FEditorCamera</code> を内包し pan / zoom できる。tilemap は <b>非所有 (caller 所有)</b>。非コピー・非ムーブ・全 noexcept。",
  when: "tilemap を ImGui 上でペイント編集したい時。<code>SetTilemap(&amp;map)</code> で編集対象を渡し、workspace に登録する。",
  sample: "namespace lv = acs::game::leveledit;\nlv::FLevelEditorPanel panel;\npanel.Init();\nworkspace.RegisterPanel(&amp;panel);\n\nacs::game::FTilemap map;  // 別途 Init 済とする\npanel.SetTilemap(&amp;map);\npanel.SetCurrentBrush(lv::EBrushKind::Paint);\npanel.SetCurrentTileId(1);\n// ... TickAllPanels(dt) 内で DrawUI ...\nworkspace.UnregisterPanel(&amp;panel);\npanel.Shutdown();",
  members: [
    { sig: "void Init() noexcept", ret: "—", desc: "内部 state をデフォルトに (brush=Paint / tile_id=1 / active_layer=0 / show_grid=true / snap=true)、FEditorCamera を 2D mode で Init。多重 Init 可。" },
    { sig: "void Shutdown() noexcept", ret: "—", desc: "tilemap 参照を解除し FEditorCamera を Reset。tilemap 自体は破棄しない。多重 Shutdown 可。" },
    ...
    { sig: "editor_core::FEditorCamera& Camera() noexcept", ret: "camera 参照", desc: "内部 2D camera への参照。呼出側が <code>HandleMouseInput</code> / <code>Tick</code> を呼ぶ。" },
    ...
    { sig: "const char* Title() const noexcept override", ret: "\"Level Editor\"", desc: "FEditorPanel override: window タイトル。" },
    { sig: "void OnInit(editor_core::FEditorWorkspace& workspace) noexcept override", ret: "—", desc: "FEditorPanel override: workspace 登録時。基底実装 + FEditorCamera を 2D mode で Init し直す保険。" },
    { sig: "void DrawUI() noexcept override", ret: "—", desc: "FEditorPanel override: Toolbar + Canvas + Inspector を描画。tilemap 未バインドなら \"(No tilemap bound)\"。" },
...
},
{
  ...
  summary: "<code>FBehaviorTreeEditorPanel</code> のメタミラー上の node 種別。実体 BT は RTTI 無効で種別判定できないため、editor 側で明示保持する。<code>FBtSelector→Selector</code> / <code>FBtSequence→Sequence</code> / <code>FBtAction→Action</code> に 1:1 対応。",
  when: "<code>AddNode</code> で node の種別を指定する時、および tree view の色分け / アイコン判別。",
  members: [
    { sig: "Selector = 0", desc: "Selector node (FBtSelector に対応)。" },
    { sig: "Sequence = 1", desc: "Sequence node (FBtSequence に対応)。" },
    { sig: "Action = 2", desc: "Action node (FBtAction に対応)。" }
  ]
},
{
  name: "FBehaviorTreeEditorPanel",
  kind: "クラス(FEditorPanel 派生)", header: "gameframework/tools/btedit/BehaviorTreeEditorPanel.h",
  summary: "<code>FBehaviorTree</code> を <b>可視化 + step debug</b> する ImGui パネル (Unreal の Behavior Tree Debugger 相当)。実体 BT は private で覗けないため、ユーザが <code>AddNode</code> で「親 id / kind / 表示名」を push する <t>メタミラー</t>方式を採る。node 状態は <code>SetNodeStatus</code> で push。root status の履歴を 60-frame ring に保持。非コピー・非ムーブ・全 noexcept。",
  when: "実行中の BT を観察 / step 実行したい時。BT 構築直後にミラーも組み立て、tick のたびに status を push する。",
  sample: "namespace bt = acs::game::btedit;\nbt::FBehaviorTreeEditorPanel panel;\npanel.Init();\nacs::game::FBehaviorTree tree;  // 別途構築済とする\npanel.SetTree(&amp;tree);\nacs::u32 root = panel.AddNode(bt::EBtKind::Selector, \"Root\", bt::FBehaviorTreeEditorPanel::kInvalidId);\nacs::u32 act  = panel.AddNode(bt::EBtKind::Action, \"Move\", root);\npanel.SetNodeStatus(act, acs::game::EBtStatus::Running);\npanel.StepOnce();\npanel.Shutdown();",
  members: [
    { sig: "using StepCallback = void(*)(void* user, FBehaviorTree* tree, f32 dt) noexcept", desc: "「1 tick 進めたい」時に呼ばれる関数ポインタ型。登録時は panel は <code>tree-&gt;Tick</code> を直接呼ばず callback だけ呼ぶ (blackboard を渡す自由を与える)。" },
    ...
    { sig: "void SetTree(FBehaviorTree* tree) noexcept", ret: "—", desc: "観察対象 BT を差し替え (nullptr で解除、非所有)。step counter / history / status を初期化するがメタミラーは触らない。", when: "観察対象設定時。" },
    { sig: "FBehaviorTree* CurrentTree() const noexcept", ret: "tree or nullptr", desc: "現在観察中の BT。" },
    ...
    { sig: "void SetNodeStatus(u32 node_id, EBtStatus status) noexcept", ret: "—", desc: "node の last_status を更新。FBtAction の Fn から呼ぶ想定。範囲外は no-op。" },
...
},
{
  ...
  members: [
    ...
    { sig: "const char* Title() const noexcept override", ret: "\"FAnimation Curve Editor\"", desc: "FEditorPanel override: window タイトル (実体リテラルは \"FAnimation Curve Editor\")。" },
    { sig: "void DrawUI() noexcept override", ret: "—", desc: "FEditorPanel override: Toolbar + Canvas を描画。curve 未バインドなら \"(No curve bound)\"。" },
...
{
  name: "FFontEditorPanel",
  kind: "クラス(FEditorPanel 派生)", header: "gameframework/tools/fontedit/FontEditorPanel.h",
  summary: "複数 font face の <t>fallback chain</t> を ImGui で対話編集 + プレビューする <b>エディタパネル</b>。<code>FontFaceInfo</code> の順序保持配列を持ち、add / remove / reorder (MoveUp/Down) と、任意 UTF-8 テキストの擬似プレビュー (実 atlas は持たず ImGui デフォルト font で代用) を提供。非コピー・非ムーブ・全 noexcept。",
  when: "font の fallback 優先順位と各 face のメタ情報を編集したい時。編集結果を取り戻して別途 <code>Font::LoadFromFile</code> 等を呼ぶ (panel は設定エディタ、loader は外部)。",
  sample: "namespace fe = acs::game::fontedit;\nfe::FFontEditorPanel panel;\npanel.Init();\nworkspace.RegisterPanel(&amp;panel);\n\nfe::FontFaceInfo primary {};\nprimary.file_path   = L\"assets/fonts/NotoSansJP-Regular.otf\";\nprimary.family_name = \"Noto Sans JP\";\npanel.AddFontFace(primary);\npanel.SetPreviewText(\"Hello, ACS! 日本語\");\npanel.SetPreviewFontSize(28.0f);\npanel.Shutdown();",
  members: [
    ...
    { sig: "const FontFaceInfo* GetFontFace(u32 i) const noexcept", ret: "info or nullptr", desc: "i 番目の face。範囲外は nullptr。" },
    { sig: "void AddFontFace(const FontFaceInfo& info) noexcept", ret: "—", desc: "face を末尾に追加。<code>fallback_index</code> は末尾 index で上書き。<code>kMaxFontFaces</code> 到達なら no-op。", when: "追加時。" },
    ...
    { sig: "const char* Title() const noexcept override", ret: "\"Font Editor\"", desc: "FEditorPanel override: window タイトル。" },
    { sig: "void OnInit(editor_core::FEditorWorkspace& workspace) noexcept override", ret: "—", desc: "FEditorPanel override: workspace 登録時。基底実装 + preview バッファの終端 0 確認。" },
    { sig: "void DrawUI() noexcept override", ret: "—", desc: "FEditorPanel override: toolbar / list / preview / inspector + char-range strip を描画。<code>IsVisible()</code> が false なら早期 return。" },
...
},
{
  ...
  summary: "<code>FSpriteAtlasEditorPanel</code> のツールバー「Pivot toggle」用 enum。frame の <t>pivot</t> をプリセット切替する。E-prefix は ACS 規約。",
...
{
  name: "FSpriteAtlasEditorPanel",
  kind: "クラス(FEditorPanel 派生)", header: "gameframework/tools/spriteatlas/SpriteAtlasEditorPanel.h",
  summary: "<code>FSpritePack</code> の atlas texture path + 名前付き <t>SpriteFrame</t> 列を ImGui で対話編集する <b>エディタパネル</b>。toolbar (New Frame / Delete / Pivot 切替) + viewport (atlas placeholder + 矩形 overlay + 8 handle resize) + frame list + inspector の構成。FSpritePack は <b>非所有 (caller 所有)</b>。新規 frame 名は内部静的プールに書き込む。非コピー・非ムーブ・全 noexcept。",
  when: "sprite atlas の frame 矩形と pivot を編集したい時。<code>SetSpritePack(&amp;pack)</code> で注入する。",
  sample: "namespace sa = acs::game::spriteatlas;\nsa::FSpriteAtlasEditorPanel panel;\npanel.Init();\nacs::game::FSpritePack pack;  // 別途 Init 済とする\npanel.SetSpritePack(&amp;pack);\nworkspace.RegisterPanel(&amp;panel);\npanel.AddFrame();\npanel.SetPivotPreset(sa::EPivotPreset::Center);\nworkspace.UnregisterPanel(&amp;panel);\npanel.Shutdown();",
  members: [
    ...
    { sig: "const char* Title() const noexcept override", ret: "\"Sprite Atlas Editor\"", desc: "FEditorPanel override: window タイトル。" },
    { sig: "void OnInit(editor_core::FEditorWorkspace& workspace) noexcept override", ret: "—", desc: "FEditorPanel override: workspace 登録時。基底実装 + frame name pool の終端 0 確認。" },
    { sig: "void DrawUI() noexcept override", ret: "—", desc: "FEditorPanel override: toolbar / list / viewport / inspector を描画。<code>IsVisible()</code> が false なら早期 return。" },
...
},
{
  ...
  summary: "<code>FCinematicsTimelineEditorPanel</code> の編集 UI 上で扱う 5 種の <b>演出概念</b>。runtime 側の <code>FCinematicsDirector::ETimelineTrackKind</code> とは意図的に概念を分離し、オーサリング向けの要素を表現する。Play 時に <code>ToTrackKind()</code> 相当で director の TrackKind にマップされる。",
...
},
{
  ...
  summary: "<code>FCinematicsTimelineEditorPanel</code> が内部保持する keyframe (リッチ payload 付き)。runtime の <code>FCinematicsDirector::FTimelineKeyframe</code> より広い payload (FVec3 / 色 / scale / event_id) を持つ。Play 時に kind に応じた FTimelineKeyframe へ変換 (ベイク) される。active な kind に対応するフィールドのみ意味を持つ。",
  when: "panel 内部の keyframe 配列の要素として。通常は <code>AddKeyframe</code> 経由で生成され、直接構築はあまりしない。",
  sample: "acs::game::cinetimeline::EditorKeyframe kf {};\nkf.time_sec = 2.0f;\nkf.kind = acs::game::cinetimeline::ETimelineKeyKind::FadeColor;\nkf.fade_start_color = { 0.0f, 0.0f, 0.0f };\nkf.fade_end_color   = { 1.0f, 1.0f, 1.0f };",
  members: [
    ...
    { sig: "EditorKeyframe() noexcept = default", desc: "既定構築 (全フィールド上記既定値)。" }
  ]
},
{
  name: "FCinematicsTimelineEditorPanel",
  kind: "クラス(FEditorPanel 派生)", header: "gameframework/tools/cinetimeline/CinematicsTimelineEditorPanel.h",
  summary: "<code>FCinematicsDirector</code> を対話編集する <b>timeline editor パネル</b> (Unreal Sequencer の最小版)。水平タイムライン + 5 トラック (ETimelineKeyKind ごと) + keyframe marker + 再生制御 (Play/Pause/Stop/Step) + scrub slider を提供。リッチな <code>EditorKeyframe</code> を panel 所有で持ち、Play 時に director へベイクする。director は <b>非所有 (caller 所有)</b>。非コピー・非ムーブ・全 noexcept。",
  when: "cutscene の keyframe を時間軸上で配置・編集し、再生確認したい時。<code>SetCinematicsDirector(&amp;dir)</code> でバインドする。",
  sample: "namespace ct = acs::game::cinetimeline;\nct::FCinematicsTimelineEditorPanel panel;\npanel.Init();\nworkspace.RegisterPanel(&amp;panel);\nacs::game::FCinematicsDirector director;\npanel.SetCinematicsDirector(&amp;director);\npanel.AddKeyframe(ct::ETimelineKeyKind::CameraCut, 0.0f);\npanel.AddKeyframe(ct::ETimelineKeyKind::FadeColor, 2.0f);\npanel.Play();\npanel.Step(0.016f);\npanel.Shutdown();",
  members: [
    ...
    { sig: "void SetCinematicsDirector(acs::game::FCinematicsDirector* dir) noexcept", ret: "—", desc: "編集対象 director を raw 参照でセット (nullptr で解除、非所有)。selection と <code>m_CurrentTime</code> を 0 に戻す。", when: "編集対象バインド時。" },
    { sig: "acs::game::FCinematicsDirector* CurrentDirector() const noexcept", ret: "director or nullptr", desc: "現在編集対象の director (未バインドは nullptr)。" },
    ...
    { sig: "const char* Title() const noexcept override", ret: "\"Cinematics Timeline\"", desc: "FEditorPanel override: window タイトル。" },
    { sig: "void DrawUI() noexcept override", ret: "—", desc: "FEditorPanel override: Toolbar + Timeline canvas + Inspector + Ruler を描画。" },
    { sig: "static constexpr i32 kNoKeySelected = -1", desc: "「未選択」を表す sentinel (i32 戻り値で使用)。" },
...
{
  name: "FModelViewerPanel",
  kind: "クラス(FEditorPanel 派生)", header: "gameframework/tools/modelview/ModelViewerPanel.h",
  summary: "ModelViewer の<b>メインビューポート</b>パネル。<t>FEditorPanel</t> を継承し、3D モデルの表示領域と「表示パラメータ」(<t>FEditorCamera</t> / ライティング / 背景色 / グリッド / ボーン表示) を ImGui で操作する。<b>実際の 3D 描画は持たず</b>、カメラ state・光・背景・トグル群といった<b>数値パラメータの保管 + UI</b> だけを担い、描画は外部の <code>ModelViewportRenderer</code>(将来) が <code>CurrentAssetPath()</code> 等を見て行う。<t>FAssetBrowser</t> からの選択通知を受けて mesh 拡張子なら自動でモデルを切り替える。非コピー / 非ムーブ・全 noexcept。",
  when: "モデルビューアの中央ビューポートとして使う。<code>FEditorWorkspace::RegisterPanel</code> で登録し、毎フレームの <code>DrawUI</code> でビューポートと control bar を出す。カメラ行列や光パラメータを外部レンダラに渡したい時にアクセサ経由で取得する。",
  sample: "FModelViewerPanel viewer;\nviewer.Init();\nworkspace.RegisterPanel(&amp;viewer);    // FEditorPanel として登録\n\n// FAssetBrowser からの選択通知で自動 LoadModelAsset:\nworkspace.BroadcastAssetSelected(\"models/hero.mdl\");\n\nif (viewer.HasModel()) {\n    auto&amp; cam = viewer.Camera();      // renderer が ViewMatrix を取り出す\n    (void)cam;\n}",
  members: [
    { sig: "void Init() noexcept", ret: "—", desc: "内部 state をデフォルトに戻し、<t>FEditorCamera</t> を 3D mode で初期化、asset path を空に。多重呼び出し可(= 完全リセット)。", when: "panel 生成直後 / 再利用時。" },
    ...
    { sig: "editor_core::FEditorCamera& Camera() noexcept", ret: "カメラ参照", desc: "内部 <t>FEditorCamera</t> への参照。renderer / UI が <code>HandleMouseInput</code> / <code>Tick</code> / <code>ViewMatrix</code> 等を呼ぶ。寿命は panel と同一。", when: "視点操作・行列取得時。" },
    ...
    { sig: "const char* Title() const noexcept override", ret: "\"Model FViewport\"", desc: "ImGui ウインドウタイトル兼 ID。固定リテラル。", when: "基底からの呼び出し。" },
    { sig: "void OnInit(editor_core::FEditorWorkspace& workspace) noexcept override", ret: "—", desc: "Workspace 登録時に呼ばれる。基底実装で Workspace ポインタを保存し、本クラスで FEditorCamera を 3D mode で Init し直す。", when: "RegisterPanel から間接的に。" },
    { sig: "void DrawUI() noexcept override", ret: "—", desc: "ImGui::Begin \"Model FViewport\" + ビューポートプレースホルダ + control bar を描画。<code>IsVisible()</code> が false なら早期 return。実 3D 描画は外部レンダラの責務。", when: "毎フレーム。" },
    { sig: "void OnAssetSelected(const char* asset_path) noexcept override", ret: "—", desc: "FAssetBrowser からのファイル選択通知。拡張子が mesh 相当(<code>.mdl/.fbx/.gltf/.glb/.obj</code>)なら自動 <code>LoadModelAsset</code>。それ以外は無視。", when: "ブラウザでファイルを選んだ時。" },
    { sig: "static constexpr u32 kMaxAssetPathChars = 512u", ret: "定数", desc: "asset path バッファ長(wchar_t 単位、終端含む)。<code>FAssetBrowser::kMaxPathChars</code> と同値。", when: "—" },
...
},
{
  ...
  summary: "model 全体の<b>集計情報</b>を 1 インスタンスにまとめた値型。caller(<code>FModelViewerPanel</code> 等)が model load 後に集計し、<code>FModelInspectorPanel::UpdateFromModel</code> に渡す。全フィールド値型なので Mesh / Skeleton 側のリソース解放と panel 表示が完全に decouple される。",
  when: "Inspector パネルに「頂点数 / 三角形数 / submesh / material slot / ボーン / クリップ数 / バウンディング球」を渡す時に組み立てる。",
  sample: "MeshSummary s;\ns.vertex_count    = 12000;\ns.triangle_count  = 4000;\ns.submesh_count   = 3;\ns.bone_count      = 52;\ns.bounding_radius = 1.8f;",
  members: [
...
},
{
  ...
  kind: "構造体(POD)", header: "gameframework/tools/modelview/ModelInspectorPanel.h",
  summary: "Inspector が表示する 1 つの animation clip のメタ情報(名前 / 長さ / サンプル数 / ループ可否)。<code>FModelAnimationPanel</code> の <code>FAnimationClipBinding</code> とは別物(こちらは read-only 表示用)。",
...
{
  name: "FModelInspectorPanel",
  kind: "クラス(FEditorPanel 派生)", header: "gameframework/tools/modelview/ModelInspectorPanel.h",
  summary: "ModelViewer 内の<b>読み取り専用 mesh / skeleton / anim 情報ビューア</b>。<t>FEditorPanel</t> を継承し、<code>UpdateFromModel</code> で push された <code>MeshSummary</code> / submesh / bone / clip 配列を<b>値コピーでスナップショット保持</b>して ImGui に表示する(Summary / Submeshes テーブル / Bones ツリー / Animations テーブルの 4 セクション)。callback も編集も GPU リソースも持たない純粋な viewer。非コピー / 非ムーブ。Unity の Mesh Inspector 相当。",
  when: "モデルのメタ情報を確認するパネルとして使う。<code>FModelViewerPanel</code> や sample がモデル load 完了時に <code>UpdateFromModel</code> を呼んで情報を流し込む。",
  sample: "FModelInspectorPanel insp;\ninsp.Init();\nworkspace.RegisterPanel(&amp;insp);\n\nMeshSummary    sum;       // 集計を埋める\nSubmeshInfo    subs[1]  = { { \"Body\", 0, 3600, 0 } };\ninsp.UpdateFromModel(sum, subs, 1,\n                     nullptr, 0,    // bones なし\n                     nullptr, 0);   // clips なし",
  members: [
    ...
    { sig: "void UpdateFromModel(const MeshSummary& summary, const SubmeshInfo* submeshes, u32 submesh_count, const BoneInfo* bones, u32 bone_count, const FAnimationClipInfo* clips, u32 clip_count) noexcept", ret: "—", desc: "表示するモデルのスナップショットを更新する。summary は値コピー、submesh / bone / clip 各配列は <t>TArray</t> に値コピーで PushBack。各ポインタは <code>nullptr</code> 可(count==0 と整合させる)。呼び出し後 <code>HasModel()</code> が true に。", when: "モデル load 完了時に caller が呼ぶ。" },
    { sig: "void Clear() noexcept", ret: "—", desc: "表示内容を空に戻し \"(No model loaded)\" 表示に切り替える。多重呼び出し可。", when: "モデル unload / workspace clear 時。" },
    { sig: "const MeshSummary& Summary() const noexcept", ret: "summary 参照", desc: "保持中の集計情報を返す。", when: "任意" },
    { sig: "u32 SubmeshCount() const noexcept", ret: "件数", desc: "保持中の submesh 数。", when: "任意" },
    { sig: "const SubmeshInfo* Submesh(u32 i) const noexcept", ret: "要素 or nullptr", desc: "i 番目の submesh。<code>i &gt;= SubmeshCount()</code> なら <code>nullptr</code>(防衛的アクセス、UB なし)。", when: "個別 submesh を参照する時。" },
    { sig: "u32 BoneCount() const noexcept", ret: "件数", desc: "保持中の bone 数。", when: "任意" },
    { sig: "const BoneInfo* FBone(u32 i) const noexcept", ret: "要素 or nullptr", desc: "i 番目の bone。範囲外は <code>nullptr</code>。", when: "個別 bone を参照する時。" },
...
{
  name: "FModelMaterialPanel",
  kind: "クラス(FEditorPanel 派生)", header: "gameframework/tools/modelview/ModelMaterialPanel.h",
  summary: "ModelViewer 内で、現在 load されたモデルの各 material slot に対し PBR マテリアルを<b>ライブ編集</b>する ImGui パネル。<t>FEditorPanel</t> を継承し、slot ごとの <code>MaterialOverride</code> を内部 <t>TArray</t> に保持して CollapsingHeader + ColorEdit / SliderFloat で編集する。<b>編集 UI と override 値の保持だけ</b>を担い、実 shader への反映は <code>MaterialChangeCallback</code>(raw 関数ポインタ + void*) 経由で外部が行う。非コピー / 非ムーブ・全 noexcept。",
  when: "モデルのマテリアルを試行錯誤で調整するパネルとして使う。モデル load 時に <code>SetMaterialSlotCount</code> で slot 数を設定し、callback を登録して変更を shader に流す。",
  sample: "FModelMaterialPanel mat;\nmat.Init();\nmat.SetMaterialSlotCount(3);          // モデルの slot 数\nmat.SetOnMaterialChangeCallback(&amp;OnMatChanged, nullptr);\nworkspace.RegisterPanel(&amp;mat);\n\nif (MaterialOverride* ov = mat.GetOverrideMutable(0)) {\n    ov-&gt;roughness = 0.2f;\n    ov-&gt;is_overridden = true;          // panel 自身は変更検知しない\n}",
  members: [
    { sig: "using MaterialChangeCallback = void (*)(void* user, u32 slot_index, const MaterialOverride& override) noexcept", ret: "型", desc: "override 1 件分の変更通知 callback 型。<code>user</code> は登録時のポインタがそのまま戻る。<code>override</code> は panel が保持する最新コピー。", when: "—" },
    ...
    { sig: "const MaterialOverride* GetOverride(u32 slot_index) const noexcept", ret: "要素 or nullptr", desc: "指定 slot の override(const)。範囲外 / 未 Init は <code>nullptr</code>。", when: "値を読む時。" },
    { sig: "MaterialOverride* GetOverrideMutable(u32 slot_index) noexcept", ret: "要素 or nullptr", desc: "指定 slot の override(mutable)。範囲外 / 未 Init は <code>nullptr</code>。外部から書き換える際は <code>is_overridden=true</code> を立てるか手動で callback を呼ぶ責務がある(panel は変更検知しない)。", when: "外部から値を書く時。" },
...
},
{
  ...
  summary: "<code>FModelAnimationPanel</code> の<b>再生状態</b>を表す 3 状態 enum。Phase 19a の E-prefix 規約(<code>enum class E*</code> + 基底 <code>u8</code>)に従う。",
...
},
{
  ...
  members: [
    ...
    { sig: "const char* Title() const noexcept override", ret: "\"FAnimation\"", desc: "ImGui ウインドウタイトル兼 ID。固定リテラル。", when: "基底からの呼び出し。" },
    { sig: "void DrawUI() noexcept override", ret: "—", desc: "ImGui::Begin \"FAnimation\" + ClipCombo + Time slider + Play / Pause / Stop + Loop checkbox + Speed slider + BlendWeight slider を描画。<code>IsVisible()</code> が false なら早期 return。", when: "毎フレーム。" },
...
"ギズモ": "viewport 上で選択 Node の Transform(移動 / 回転 / 拡縮)を直接ドラッグ操作するハンドル。ACS では FEditorGizmo が実装する。",
"ワークスペース": "複数のエディタ panel を 1 つの editor アプリとしてまとめ、ライフサイクル・メインループ・レイアウト永続化・イベント broadcast を統括する中央 hub。ACS では FEditorWorkspace。",
...
"payload": "ImGui のドラッグ &amp; ドロップで運ばれるデータ本体。ACS の FAssetBrowser では \"ASSET_PATH\" id で wchar_t* を渡す。",
"drawer": "inspector 上で 1 つの property を描画する関数。ACS では DrawerFn(関数ポインタ)で表現し PropertyDrawerRegistry に登録する。",
...
"merge(コマンド)": "連続 drag のような細かい操作を 1 件の undo エントリにまとめること。FEditorCommand の CanMerge / MergeWith で判定・統合する。",
"FieldChangeCallback": "FInspectorPanel が field 編集時に外部へ変更を通知する raw 関数ポインタ型 (undo / 永続化への委譲口)。",
"SaveSceneCallback": "FEditorToolbar の Save ボタン押下で発火する raw 関数ポインタ型。serialize 先は user 側で決める。",
"EEditorState": "FEditorToolbar が持つ editor の再生状態列挙 (Playing / Paused / Stepping)。",
"FInspectorSeam": "Node から InspectableObject / InspectableField 配列を解決する seam。FInspectorPanel が provider 解決元として消費する。",
"fallback chain": "複数の font face を優先順位付きで並べ、ある文字が前の face に無ければ次の face で補う仕組み。CJK や emoji を別 face で埋める用途に使う。",
"メタミラー": "実体オブジェクト (ここでは BehaviorTree) を直接覗けない時に、ユーザが構造情報 (親 id / 種別 / 名前) を別途 push して作る描画用のミラー (写し)。BehaviorTree エディタが RTTI 無効下で tree を可視化するために使う。",
"SpriteFrame": "sprite atlas 内の 1 枚分の矩形領域 (x/y/w/h + pivot + 名前) を表す単位。FSpritePack が名前付きで複数保持する。",
...
"FEditorPanel": "ACS の editor_core が提供するエディタパネルの抽象基底クラス。Title / DrawUI / OnInit などを override し、FEditorWorkspace に登録して使う。",
"FEditorCamera": "editor_core のエディタ用カメラ。3D orbit / pan / dolly 操作と ViewMatrix / ProjectionMatrix の取得を提供する。各パネルが値メンバとして内包する Unity SceneView 風モデル。",
"FAssetBrowser": "editor_core のアセットブラウザパネル。ファイル選択を OnAssetSelected で各パネルに通知し、拡張子で mesh / texture などに分類する。"
});
