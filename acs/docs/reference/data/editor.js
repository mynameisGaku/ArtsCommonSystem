  ...
  types: [
{
  ...
  members: [
    ...
    { sig: "virtual void OnSelectionChanged(inspector::FSelectionService&amp; selection) noexcept", ret: "—", desc: "FScene 内 ANode の選択が変わった時に呼ばれる(Phase 20 <code>FSelectionService</code> 統合)。既定 no-op。", when: "Inspector 等 node 系 panel。" },
...
},
{
  ...
  when: "<code>FEditorGizmo::HotAxis()</code> や <code>FGizmoState::hot_axis</code> で現在 hot な軸を読む時。",
  members: [
...
},
{
  ...
  summary: "<b>drag 終了時に外部へ delta を通知する callback 型</b>。drag 完了時に 1 度だけ呼ばれ、<code>FUndoStack</code> に <code>FMoveNodeCommand</code> 等を push する適切なタイミングになる。<code>delta</code> はモード依存(Translate=移動量 / Rotate=euler 差分[radians] / Scale=倍率差分)。<code>user</code> は <code>SetOnManipulateCallback</code> の第二引数がそのまま戻る。",
...
},
{
  ...
  summary: "選択中の ANode に紐づく <t>FInspectorSeam</t> 経由の <code>FInspectableField</code> 配列を、ImGui の field widget (Checkbox / InputInt / SliderFloat / DragFloatN / ColorEdit / InputText / Combo 等) として描画・編集する<b>プロパティエディタ</b>パネル。値の<b>永続化や undo は行わず</b>、変更を <t>FieldChangeCallback</t> で外部へ通知するだけ。<code>editor_core::FEditorPanel</code> を継承し、ヘッダから ImGui 依存は漏らさない。<b>ランタイム側の同名 FInspectorSeam とは別レイヤの UI 部品</b>。",
  when: "<code>FHierarchyPanel</code> 等で選択された ANode の各フィールドを GUI で編集させたい時。<code>SetInspectorSeam</code> で provider 解決元を、<code>SetSelectionService</code> で選択取得元を注入し、毎フレーム <code>DrawUI()</code> を呼ぶ。",
  ...
  members: [
    ...
    { sig: "void SetInspectorSeam(FInspectorSeam* seam) noexcept", ret: "—", desc: "ANode → Provider 解決に使う <code>FInspectorSeam</code> を事前 set する。<code>DrawUI</code> 内で <code>seam.GetProvider(node_id)</code> を呼ぶ。" },
...
},
{
  ...
  members: [
    ...
    { sig: "FNodeId SelectedNodeId() const noexcept", ret: "選択 id", desc: "現選択 ANode の <code>FNodeId</code>。<code>FSelectionService</code> 注入時はそちら経由、なければ内部値。未選択は <code>FNodeId{}</code> (packed==0)。" },
    { sig: "void SelectNode(ANode* node) noexcept", ret: "—", desc: "選択を <code>node</code> に切り替える。nullptr で選択解除。<code>FSelectionService</code> 注入時はそちらにも反映する。" },
...
},
{
  ...
  summary: "<code>FParticleEmitterDef</code> を ImGui で実機編集するための emitter プロパティエディタ <t>パネル</t>。<code>editor_core::FEditorPanel</code> を継承し、内部に <code>TArray&lt;FParticleEmitterDef&gt;</code> を持つ。emitter handle や pool への反映 (CreateEmitter 等) は呼び出し側の責務で、本 panel は <b>def の編集のみ</b>を担う。Save / Load は callback hook に委譲。非コピー・非ムーブ・全 noexcept。",
...
},
{
  ...
  summary: "編集中の <code>FParticleEmitterDef</code> を 1 個だけ instance 化して、リアルタイムに visual feedback を返す <b>preview canvas + stats</b>。外部の <code>FParticleEffectSystem</code> 上に 1 emitter を立てる方式で system は所有しない。frame budget は 60-frame の <t>リングバッファ</t>で保持。非コピー・非ムーブ・全 noexcept。",
  ...
  members: [
    ...
    { sig: "void DrawUI(FParticleEffectSystem& system, const FParticleEmitterDef* def) noexcept", ret: "—", desc: "<code>\"Particle Preview\"</code> window を描画。<code>def == nullptr</code> なら \"no def selected\" を表示。" },
    { sig: "void RecreatePreviewEmitter(FParticleEffectSystem& system, const FParticleEmitterDef* def) noexcept", ret: "—", desc: "編集中 emitter を即時再生成 (既存 handle が valid なら Destroy 後、新 def の copy で Create)。<code>def == nullptr</code> は no-op。", when: "def 変更時。" },
...
},
{
  ...
  members: [
    ...
    { sig: "const char* Title() const noexcept override", ret: "\"Animation Curve Editor\"", desc: "FEditorPanel override: window タイトル (実体リテラルは \"Animation Curve Editor\")。" },
...
},
{
  ...
  summary: "<code>FCinematicsDirector</code> を対話編集する <b>timeline editor パネル</b> (Unreal Sequencer の最小版)。水平タイムライン + 5 トラック (ETimelineKeyKind ごと) + keyframe marker + 再生制御 (Play/Pause/Stop/Step) + scrub slider を提供。リッチな <code>FEditorKeyframe</code> を panel 所有で持ち、Play 時に director へベイクする。director は <b>非所有 (caller 所有)</b>。非コピー・非ムーブ・全 noexcept。",
...
},
{
  ...
  summary: "ModelViewer 内で、現在 load されたモデルの各 material slot に対し PBR マテリアルを<b>ライブ編集</b>する ImGui パネル。<t>FEditorPanel</t> を継承し、slot ごとの <code>FMaterialOverride</code> を内部 <t>TArray</t> に保持して CollapsingHeader + ColorEdit / SliderFloat で編集する。<b>編集 UI と override 値の保持だけ</b>を担い、実 shader への反映は <code>MaterialChangeCallback</code>(raw 関数ポインタ + void*) 経由で外部が行う。非コピー / 非ムーブ・全 noexcept。",
...
"ギズモ": "viewport 上で選択 ANode の Transform(移動 / 回転 / 拡縮)を直接ドラッグ操作するハンドル。ACS では FEditorGizmo が実装する。",
...
"drawer": "inspector 上で 1 つの property を描画する関数。ACS では DrawerFn(関数ポインタ)で表現し FPropertyDrawerRegistry に登録する。",
...
"FInspectorSeam": "ANode から FInspectableObject / FInspectableField 配列を解決する seam。FInspectorPanel が provider 解決元として消費する。",
"fallback chain": "複数の font face を優先順位付きで並べ、ある文字が前の face に無ければ次の face で補う仕組み。CJK や emoji を別 face で埋める用途に使う。",
"メタミラー": "実体オブジェクト (ここでは FBehaviorTree) を直接覗けない時に、ユーザが構造情報 (親 id / 種別 / 名前) を別途 push して作る描画用のミラー (写し)。FBehaviorTree エディタが RTTI 無効下で tree を可視化するために使う。",
