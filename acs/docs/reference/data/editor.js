...
},
{
  ...
  kind: "クラス(AEditorPanel 派生)", header: "gameframework/tools/cinetimeline/ACinematicsTimelineEditorPanel.h",
  ...
  members: [
    ...
    { sig: "void Step(f32 dt) noexcept", ret: "—", desc: "再生中に有限で正の dt だけ進める。非有限値と加算結果が非有限になる値は無視し、director.Tick(dt) と m_CurrentTime を変更しない。", when: "再生中の毎フレーム。" },
    { sig: "bool IsPlaying() const noexcept", ret: "bool", desc: "再生中か。" },
    { sig: "f32 CurrentTimeSec() const noexcept / void SetCurrentTimeSec(f32 t) noexcept", ret: "秒", desc: "タイムカーソルの現在時刻。有限値は [0, Duration] にクランプし、非有限値は無視する。" },
    { sig: "f32 DurationSec() const noexcept / void SetDurationSec(f32 d) noexcept", ret: "秒", desc: "タイムライン全体の長さ。有限値は最低 <code>kMinDurationSec</code> (0.1s) にクランプし、非有限値は無視する。" },
    ...
    { sig: "void AddKeyframe(ETimelineKeyKind kind, f32 time_sec) noexcept", ret: "—", desc: "有限の time_sec で新規 keyframe を追加。有限値は [0, Duration] にクランプし、配列は time 昇順、追加後は新 keyframe を selection にする。非有限値は panel と director を変更しない。", when: "キー追加時。" },
