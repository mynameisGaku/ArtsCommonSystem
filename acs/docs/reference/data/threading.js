  ...
  types: [
    ...
    },
    {
      ...
      members: [
        ...
        { sig: "bool CompareExchange(T& expected, T desired)", ret: "成功したか", desc: "現在値が <code>expected</code> と一致すれば <code>desired</code> に書き換え true。失敗時は <code>expected</code> に実際の値が入り false。<t>CAS</t>。", when: "『今が X なら Y にする』というロックフリーな更新ループを書く時。"},
    ...
    },
    {
      ...
      members: [
        ...
        { sig: "static TResult<void> ParallelFor(u32 begin, u32 end, u32 grain, body, void* user)", ret: "成否", desc: "<code>[begin, end)</code> を <code>grain</code> 個ずつに分割して並列実行し、全部終わるまで待つ。", when: "配列を範囲分割して並列処理したい時の定番。"},
