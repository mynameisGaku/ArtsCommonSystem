...
ACS_REF.modules.push({
  ...
  types: [
    ...
    },
    {
      ...
      members: [
        { sig: "static TResult<FThread> Spawn(ThreadEntry entry, void* user, const ThreadConfig& cfg = {})", ret: "スレッド or エラー", desc: "<code>entry(user)</code> を別スレッドで起動する。失敗時は Err。", when: "スレッドを 1 本立ち上げる時。" },
        ...
        { sig: "ThreadId Id() const", ret: "スレッド ID", desc: "このスレッドの <code>ThreadId</code>。" }
      ]
    },
    {
      name: "ThreadEntry / ThreadConfig",
      kind: "型エイリアス / 構造体", header: "threading/Thread.h",
      summary: "<b>ThreadEntry</b>=スレッドの入口関数の型 <code>void(*)(void* user)</code>。<b>ThreadConfig</b>=スレッド起動時の設定(名前・スタックサイズ・優先度・CPU アフィニティ)。",
      when: "<code>FThread::Spawn</code> に渡す。名前を付けるとデバッガで識別しやすい。",
      sample: "ThreadConfig cfg;\ncfg.name        = L\"AssetLoader\";  // デバッガ表示名\ncfg.stack_bytes = 0;               // 0 = OS 既定\ncfg.priority    = 0;               // -2..+2\nFThread::Spawn(&Work, &ctx, cfg);",
      members: [
    ...
    },
    {
      ...
      sample: "ThreadId me = CurrentThreadId();\nif (me == g_MainThread) {\n    // メインスレッドからの呼び出し\n}",
      members: [
        { sig: "u32 raw", desc: "OS の DWORD と等価な生の ID。" },
        { sig: "bool operator==(ThreadId) / operator!=(ThreadId)", desc: "ID 同士の一致/不一致を比べる。" }
      ]
    },
    {
      name: "CurrentThreadId()",
      kind: "関数", header: "threading/ThreadId.h",
      summary: "今このコードを実行している<t>スレッド</t>の <code>ThreadId</code> を返す(GetCurrentThreadId のラッパ)。",
      when: "スレッド固有の処理分岐や、後述の <code>ACS_THREAD_AFFINITY_*</code> の検証に使われる。",
      sample: "ThreadId tid = CurrentThreadId();"
    },
    {
    ...
    },
    {
      ...
      summary: "<t>FThreadPool</t> に投げる 1 件の仕事。<b>TaskFn</b>=タスク本体の関数型 <code>void(*)(void* user, u32 worker_index)</code>。<b>Task</b>=その関数・ユーザーデータ・完了通知先をまとめた <t>POD</t>。",
      ...
      members: [
        ...
        { sig: "CompletionCounter* counter", desc: "完了通知先(任意・null 可)。Submit 時に自動で 1 加算され、タスク終了時に減算される。" }
      ]
    },
    {
      ...
      sample: "CompletionCounter c;\nfor (auto& job : jobs)\n    FThreadPool::Submit(Task{ job.fn, job.user, &c }); // 自動 Add\nFThreadPool::Wait(c);  // 全部終わるまで(自分も手伝いつつ)待つ",
      members: [
        { sig: "explicit CompletionCounter(u32 initial)", desc: "初期残数を指定して作る(既定は 0)。" },
    ...
    },
    {
      ...
      members: [
        { sig: "JobHandle Add(JobFn fn, void* user)", ret: "ジョブのハンドル", desc: "ジョブを 1 件追加する。<code>Submit</code> より前にだけ呼べる。", when: "グラフを組み立てる段階。" },
        { sig: "void AddDependency(JobHandle upstream, JobHandle downstream)", desc: "<code>upstream</code> が終わるまで <code>downstream</code> を走らせない依存を張る(<code>JobHandle::DependOn</code> と同じ)。" },
    ...
    {
      name: "JobHandle / JobFn",
      kind: "構造体 / 型エイリアス", header: "threading/JobGraph.h",
      summary: "<b>JobHandle</b>=<t>FJobGraph</t> に追加したジョブを指す軽い参照で、これ越しに依存関係を張る。<b>JobFn</b>=ジョブ本体の関数型 <code>void(*)(void* user, u32 worker_index)</code>(<code>TaskFn</code> と同形式)。",
      ...
      members: [
        ...
        { sig: "void DependOn(JobHandle upstream)", desc: "<code>upstream</code> が完了するまでこのジョブを走らせない、という依存を張る。" }
      ]
    }
  ]
});
...
Object.assign(ACS_REF.glossary, {
  ...
  "RwLock": "読み手は同時に複数、書き手は 1 人だけ通すロック(std::shared_mutex 相当)。",
