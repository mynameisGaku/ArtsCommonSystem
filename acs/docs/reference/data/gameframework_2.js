  ...
  types: [
    {
      ...
      members: [
        ...
        { sig: "using FCharacterCustomizer = CCharacterCustomizer", desc: "旧名を使う既存コード向けの互換別名。新しいコードでは <code>CCharacterCustomizer</code> を使う。" }
      ]
    },
    {
      name: "FCosmeticItem",
    ...
    },
    {
      ...
      members: [
        ...
        { sig: "bool ActivateCheckpoint(const char* checkpoint_id)", ret: "切替できたか", desc: "指定 checkpoint を「現在地点」にする。one_way 違反 / 未 unlock / 未登録は false。同 id が既に active なら true (再発火なし)。", when: "プレイヤーがチェックポイントトリガに触れた時。"},
        ...
        { sig: "bool TriggerRespawn(FVec2& out_pos, u32& out_level_index) const", ret: "active があったか", desc: "死亡時の復活先座標と level index を書き出す。active が無ければ false。成功時に RespawnCallback 発火。", when: "プレイヤー死亡時。"},
        ...
        { sig: "using FCheckpointSystem = CCheckpointSystem", desc: "旧名を使う既存コード向けの互換別名。新しいコードでは <code>CCheckpointSystem</code> を使う。" }
      ]
    },
    {
      name: "FCheckpointId",
    ...
    },
    {
      ...
      members: [
        { sig: "bool TryAddKeyframe(const FTimelineKeyframe& kf)", ret: "追加可否", desc: "有限時刻の keyframe を time_sec 昇順 (同時刻は登録順) で追加する。再生中・進行後・確保失敗・非有限時刻では追加せず false。負値は 0 秒へ丸める。", when: "停止状態の初期設定時。" },
        { sig: "void AddKeyframe(const FTimelineKeyframe& kf)", desc: "TryAddKeyframe の結果を返さず、受け入れ可能な keyframe だけを追加する。", when: "シーンに入った時にまとめて keyframe を積む。" },
        ...
        { sig: "void Tick(f32 dt)", desc: "有限で正の dt だけ時刻を進め、到達した keyframe を時刻順に一度発火する。再生中でない、非有限、0 以下、または加算結果が非有限なら何もしない。", when: "毎フレーム呼ぶ。" },
        ...
        { sig: "using FCinematicsDirector = CCinematicsDirector", desc: "旧名を使う既存コード向けの互換別名。新しいコードでは <code>CCinematicsDirector</code> を使う。" }
      ]
    },
    {
    ...
    },
    {
      ...
      members: [
        ...
        { sig: "void SetTimeScale(f32 s)", desc: "倍率を設定。0=フリーズ、1=通常、>1=早送り、<1=スロー。負値は 0 にクランプ。"},
        { sig: "void Reset()", desc: "時間積算とフレーム数だけリセット (pause / time_scale は維持)。" },
        { sig: "using FSceneClock = CSceneClock", desc: "旧名を使う既存コード向けの互換別名。新しいコードでは <code>CSceneClock</code> を使う。" }
      ]
    },
    {
      name: "CCollisionWorld2D",
      ...
      members: [
        ...
        { sig: "bool Raycast(const FRay2& ray, f32 max_t, FRayHit2& out_hit, FShapeId& out_id, u32 mask = kAllLayers)", ret: "当たったか", desc: "最も近い形状を 1 つ返す。out_hit に交点 / 法線 / 距離、out_id にどの形状か。", when: "視線判定・射撃・地面検出など。"},
        ...
        { sig: "using FCollisionWorld2D = CCollisionWorld2D", desc: "旧名を使う既存コード向けの互換別名。新しいコードでは <code>CCollisionWorld2D</code> を使う。" }
      ]
    },
    {
      name: "FShapeId",
      kind: "構造体", header: "gameframework/CollisionWorld2D.h",
      summary: "<code>CCollisionWorld2D</code> 内の形状を指す<t>ハンドル</t>。24bit index + 8bit generation の packed u32。削除済み slot を再利用しても古い handle は無効化される。0 は invalid。",
    ...
    },
    {
      ...
      when: "戦況に応じて BGM や環境演出を切り替えたい時。状態が変わったら callback で CMusicDirector 等に通知する。",
      members: [
        ...
        { sig: "using FCombatStateMachine = CCombatStateMachine", desc: "旧名を使う既存コード向けの互換別名。新しいコードでは <code>CCombatStateMachine</code> を使う。" }
      ]
    },
    {
      name: "ECombatState",
    ...
    },
    {
      ...
      summary: "1 体の敵の認識情報。enemy_id・awareness_level [0,1] (1=完全検出) ・is_engaged を持つ。<code>CCombatStateMachine</code> が内部で配列管理する。",
    ...
    },
    {
      ...
      members: [
        ...
        { sig: "using FContentModeratorStub = CContentModeratorStub", desc: "旧名を使う既存コード向けの互換別名。新しいコードでは <code>CContentModeratorStub</code> を使う。" }
      ]
    },
    {
    ...
    },
    {
      ...
      members: [
        ...
        { sig: "bool TryUse(FCooldownId id)", ret: "使えたか", desc: "charged なら remaining=duration にして true。リロード中は false。", when: "スキルを発動しようとする時。"},
        ...
        { sig: "using FCooldownTimer = CCooldownTimer", desc: "旧名を使う既存コード向けの互換別名。新しいコードでは <code>CCooldownTimer</code> を使う。" }
      ]
    },
    {
      name: "FCooldownId",
    ...
    },
    {
      ...
      members: [
        ...
        { sig: "using FCraftingSystem = CCraftingSystem", desc: "旧名を使う既存コード向けの互換別名。新しいコードでは <code>CCraftingSystem</code> を使う。" }
      ]
    },
    {
      name: "FCraftRecipe",
    ...
    {
      name: "CCrashReporterStub / GetCrashStub / GetDefaultCrashReporter",
      kind: "クラス / 関数", header: "gameframework/CrashReporter.h",
      summary: "<code>CCrashReporterStub</code> は全 API が NotImplemented を返す<t>null-object</t> 実装。<code>GetCrashStub()</code> で共有 stub を取得。<code>GetDefaultCrashReporter()</code> は実 backend が登録済ならそれを、未登録なら stub を返す。<code>SetCrashReporterProvider()</code> で実 backend を結線する。",
      when: "実 backend が無い環境でもリンクを通したい時、または backend 非依存に既定 reporter を取りたい時。",
      members: [
        { sig: "using FCrashReporterStub = CCrashReporterStub", desc: "旧名を使う既存コード向けの互換別名。新しいコードでは <code>CCrashReporterStub</code> を使う。" }
      ]
    },
    {
      name: "CCrashHandler",
      ...
      members: [
        ...
        { sig: "using FCrashHandler = CCrashHandler", desc: "旧名を使う既存コード向けの互換別名。新しいコードでは <code>CCrashHandler</code> を使う。" }
      ]
    },
    {
      name: "FCrashContext",
    ...
    },
    {
      ...
      members: [
        ...
        { sig: "using FDamageFeedback = CDamageFeedback", desc: "旧名を使う既存コード向けの互換別名。新しいコードでは <code>CDamageFeedback</code> を使う。" }
      ]
    },
    {
      name: "CDebugDraw",
      ...
      members: [
        { sig: "bool TryDrawLine(FVec2 a, FVec2 b, FVec4 color)", ret: "全線分を追加できたか", desc: "任意 2 点間の線分を一括追加する。無効値・容量不足では蓄積を変更しない。" },
        ...
        { sig: "bool TryDrawArrow(FVec2 a, FVec2 b, FVec4 color, f32 head_len = 0.0f)", ret: "全線分を追加できたか", desc: "軸と矢じりを一括追加する。同一点なら軸 1 本だけを追加し、無効値・容量不足では蓄積を変更しない。" },
        { sig: "void DrawArrow(FVec2 a, FVec2 b, FVec4 color, f32 head_len = 0.0f)", desc: "TryDrawArrow の既存 void 互換入口。失敗時は蓄積を変更しない。" },
        ...
        { sig: "using FDebugDraw = CDebugDraw", desc: "旧名を使う既存コード向けの互換別名。新しいコードでは <code>CDebugDraw</code> を使う。" }
      ]
    },
    {
      name: "CDebugOverlay",
      ...
      members: [
        ...
        { sig: "using FDebugOverlay = CDebugOverlay", desc: "旧名を使う既存コード向けの互換別名。新しいコードでは <code>CDebugOverlay</code> を使う。" }
      ]
    },
    {
      name: "CDeckSystem",
      ...
      members: [
        ...
        { sig: "using FDeckSystem = CDeckSystem", desc: "旧名を使う既存コード向けの互換別名。新しいコードでは <code>CDeckSystem</code> を使う。" }
      ]
    },
    {
      name: "FCardDef",
    ...
    },
    {
      ...
      members: [
        ...
        { sig: "void Open() / Close() / Toggle()", desc: "コンソールの開閉。"},
        { sig: "bool IsOpen() const", desc: "今開いているか。" },
        { sig: "void Clear()", desc: "履歴とログを破棄する (コマンド登録は保持)。" },
        { sig: "u32 CommandCount() const", desc: "登録済みコマンド数。" },
        { sig: "using FDevConsole = CDevConsole", desc: "旧名を使う既存コード向けの互換別名。新しいコードでは <code>CDevConsole</code> を使う。" }
      ]
    },
    {
...
Object.assign(ACS_REF.glossary, {
  ...
  "pay-to-win": "課金で性能 (戦闘力) を直接買えてしまう設計。<t>CCharacterCustomizer</t> は見た目のみ扱い、性能をいじらないことで構造的にこれを避ける。",
  "有限オートマトン": "決められた状態のどれか 1 つにいて、入力で別の状態へ遷移する仕組み (state machine)。戦闘フェーズ管理などに使う。",
  "time_scale": "時間の流れる倍率。1=通常、0=停止、>1=早送り、<1=スロー。<t>CSceneClock</t> がこれを掛けて scaled な時間を作る。",
  ...
  "immediate-mode": "状態を保持せず、毎フレーム「今出したいもの」をその場で積み上げる API スタイル。<t>CDebugDraw</t> がこの方式。",
