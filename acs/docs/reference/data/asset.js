...
ACS_REF.modules.push({
  ...
  types: [
    ...
    },
    {
      ...
      summary: "アセットファイル不要で<b>立方体・球・平面を生成</b>する便利関数。<t>FMeshAsset</t> を <t>TSharedPtr</t> で返す。プロトタイピングやサンプルに便利。",
      when: "モデルを用意せずにとりあえず形を描きたい/動作確認したい時。",
      sample: "TSharedPtr&lt;FMeshAsset&gt; cube   = Primitive::MakeCube();\nTSharedPtr&lt;FMeshAsset&gt; sphere = Primitive::MakeSphere(1.0f, 32, 16);\nTSharedPtr&lt;FMeshAsset&gt; plane  = Primitive::MakePlane(10.0f, 10.0f);",
      members: [
        { sig: "TSharedPtr<FMeshAsset> MakeCube(f32 size = 1.0f)", ret: "立方体", desc: "中心原点の立方体。各面 4 頂点で法線と UV 0..1 を持つ。" },
        { sig: "TSharedPtr<FMeshAsset> MakeSphere(f32 radius = 0.5f, u32 segments = 32, u32 rings = 16)", ret: "球", desc: "UV 球(半径・横分割・縦分割)。" },
        { sig: "TSharedPtr<FMeshAsset> MakePlane(f32 width = 1.0f, f32 depth = 1.0f)", ret: "平面", desc: "XZ 平面(Y=0、法線 +Y)。" }
      ]
    },
    {
    ...
    },
    {
      ...
      sample: "FAnimationPlayer ap;\nap.SetMesh(&amp;mesh);\nap.Play(0, /*loop=*/true);\nap.Update(dt);\nFMat4 palette[64];\nu32 count = ap.WritePalette(palette, 64);\nshader.SetBonePalette(palette, count);",
      members: [
        { sig: "void SetMesh(const FSkinnedMeshAsset* mesh)", desc: "再生対象のメッシュを設定する(アニメ・時刻はリセット)。" },
    ...
    {
      name: "FTextAssetLoader",
      kind: "クラス(IAssetLoader 派生)", header: "asset/TextAsset.h",
      summary: "テキストファイルの<t>ローダ</t>。バイト列を <t>FTextAsset</t> にする。主要拡張子は \"txt\"。",
      when: "標準ローダ登録で自動使用。",
      sample: "FTextAssetLoader txt;\nreg.RegisterLoader(&amp;txt);"
    },
    {
      name: "FBinaryAsset",
...
Object.assign(ACS_REF.glossary, {
  "レジストリ": "読み込んだアセットをキー(<t>FAssetId</t>)で保持・共有し、再利用する管理表。ACS では <t>FAssetRegistry</t>。",
  "FAssetId": "パス文字列をハッシュした 64-bit のアセット識別子。同じパスなら必ず同じ ID。",
  "FAssetRegistry": "パスからアセットを読み込み、<t>TSharedPtr</t> でキャッシュ共有する中心クラス。",
  ...
  "FImageAsset": "画像 1 枚分の CPU 側ピクセルデータを持つアセット。",
  "FAudioAsset": "統一 PCM で音声を保持するアセット。",
  "FMeshAsset": "頂点+インデックス+サブメッシュからなる単一メッシュのアセット。",
  "FTextAsset": "UTF-8 テキストを保持するアセット。",
  "FBinaryAsset": "生バイト列をそのまま持つ最小アセット(フォールバック)。",
