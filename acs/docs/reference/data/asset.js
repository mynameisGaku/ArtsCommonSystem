  ...
  types: [
    ...
    },
    {
      ...
      members: [
        ...
        { sig: "TResult<TSharedPtr<AAsset>> Load(const wchar_t* path)", ret: "アセット or エラー", desc: "<b>同期</b>でファイル読み込み + ローダ呼び出し。キャッシュ済みなら即返却。<t>Result</t> で成否を返す。"},
        { sig: "FAssetFuture LoadAsync(const wchar_t* path)", ret: "完了ハンドル", desc: "<b>非同期</b>で <t>CThreadPool</t> ワーカーに読み込みを投げ、<t>FAssetFuture</t> を返す。キャッシュ済みなら即完了状態の future。", when: "大きいモデル等をフレームを止めずに読みたい時。"},
    ...
    },
    {
      ...
      summary: "アセットファイル不要で<b>立方体・球・平面を生成</b>する便利関数。<t>AMeshAsset</t> を <t>TSharedPtr</t> で返す。プロトタイピングや簡易形状確認に便利。",
