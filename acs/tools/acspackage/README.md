...
`EMPTY3D`, `CMP3D`, `CPROP3D`, `PLY3D`, `SPR3D`, `PFAB3D`, `PINS3D`, `POVR3D`, and `SEL3D`. `PLY3D`
...
32-character stable instance identity, and `POVR3D` preserves its root Visible/Enabled/Color
override mask without adding a dependency. Unknown directives, invalid reflected components, and standalone
