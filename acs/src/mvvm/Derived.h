...
public:
    ...
    Derived(Fn fn, DepRefs&... deps) noexcept
    {
        ...
        usize i = 0;
        // 初期値計算 + Subscribe
        ((_deps[i] = static_cast<Observable<D>*>(&deps), ++i), ...);
    ...
    Observable<T>& AsObservable() noexcept { return _out; }

    ObservableHandle Subscribe(typename Observable<T>::Listener cb, void* user) noexcept {
        return _out.Subscribe(cb, user);
    }
    bool Unsubscribe(ObservableHandle h) noexcept { return _out.Unsubscribe(h); }
...
private:
    ...
    Observable<D>*    _deps[kMaxDeps]    = {};
    ObservableHandle  _handles[kMaxDeps] = {};
    usize             _dep_count         = 0;
