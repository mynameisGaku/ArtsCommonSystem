...
template<>
void BindFormat<f32>(const char* fmt, const Observable<f32>& v) noexcept {
    ImGui::Text(fmt, v.Get());
}

/** f64 Observable の値を fmt で整形してテキスト表示する。 */
template<>
void BindFormat<f64>(const char* fmt, const Observable<f64>& v) noexcept {
    ImGui::Text(fmt, v.Get());
}

/** i32 Observable の値を fmt で整形してテキスト表示する。 */
template<>
void BindFormat<i32>(const char* fmt, const Observable<i32>& v) noexcept {
    ImGui::Text(fmt, v.Get());
}

/** u32 Observable の値を fmt で整形してテキスト表示する。 */
template<>
void BindFormat<u32>(const char* fmt, const Observable<u32>& v) noexcept {
    ImGui::Text(fmt, v.Get());
}

/** Command をボタン描画し、実行可能ならクリックで Execute、不可なら grayout する。 */
...
bool BindCommand(const char*, Command&) noexcept { return false; }
