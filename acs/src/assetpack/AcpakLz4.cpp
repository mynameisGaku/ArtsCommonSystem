...
// =============================================================================
// ACS AssetPack — CAcpakLz4 実装 (自前 LZ4 block format)
// -----------------------------------------------------------------------------
...
u32 CAcpakLz4::MaxCompressedSize(u32 input_size) noexcept {
...
TResult<u32> CAcpakLz4::Compress(const u8* src,
                               u32       src_size,
                               u8*       dst,
    ...
    if (dst == nullptr || dst_capacity == 0) {
        return ACS_ERR(Asset, kAcpakSubLz4BadInput,
                       "CAcpakLz4::Compress: dst is null / empty");
    }
    if (src == nullptr && src_size > 0) {
        return ACS_ERR(Asset, kAcpakSubLz4BadInput,
                       "CAcpakLz4::Compress: src is null but size > 0");
    }
    if (src_size == 0) {
    ...
    if (src_size < kMinMatch + kMatchFindLimit) {
        ...
        if (!check_dst(1u + ext_bytes + lit_len)) {
            return ACS_ERR(Asset, kAcpakSubLz4DstOverflow,
                           "CAcpakLz4::Compress: dst overflow (short input)");
        }
    ...
    while (ip < match_end) {
        ...
        if (!check_dst(need)) {
            return ACS_ERR(Asset, kAcpakSubLz4DstOverflow,
                           "CAcpakLz4::Compress: dst overflow (mid sequence)");
        }
    ...
    const u32 trailing = src_size - anchor;
    {
        ...
        if (!check_dst(need)) {
            return ACS_ERR(Asset, kAcpakSubLz4DstOverflow,
                           "CAcpakLz4::Compress: dst overflow (trailing literals)");
        }
...
TResult<u32> CAcpakLz4::Decompress(const u8* src,
                                 u32       src_size,
                                 u8*       dst,
                                 u32       dst_capacity) noexcept {
    if (src_size == 0) {
    ...
    if (src == nullptr || dst == nullptr) {
        return ACS_ERR(Asset, kAcpakSubLz4BadInput,
                       "CAcpakLz4::Decompress: src/dst is null");
    }
    if (dst_capacity == 0) {
        return ACS_ERR(Asset, kAcpakSubLz4DstOverflow,
                       "CAcpakLz4::Decompress: dst_capacity == 0");
    }
    ...
    while (ip < src_end) {
            ...
            while (true) {
                if (ip >= src_end) {
                    return ACS_ERR(Asset, kAcpakSubLz4SrcOverflow,
                                   "CAcpakLz4::Decompress: truncated literal_length");
                }
                const u8 b = *ip++;
                lit_len += b;
                if (b != 0xFF) break;
                ...
                if (lit_len > 0x10000000u) {
                    return ACS_ERR(Asset, kAcpakSubLz4SrcOverflow,
                                   "CAcpakLz4::Decompress: literal_length overflow");
                }
            }
        }
        ...
        if (lit_len > 0) {
            ...
            if (static_cast<usize>(src_end - ip) < lit_len) {
                return ACS_ERR(Asset, kAcpakSubLz4SrcOverflow,
                               "CAcpakLz4::Decompress: src exhausted in literals");
            }
            if (static_cast<usize>(dst_end - op) < lit_len) {
                return ACS_ERR(Asset, kAcpakSubLz4DstOverflow,
                               "CAcpakLz4::Decompress: dst exhausted in literals");
            }
        ...
        if (mat_len == 15u) {
            while (true) {
                if (ip >= src_end) {
                    return ACS_ERR(Asset, kAcpakSubLz4SrcOverflow,
                                   "CAcpakLz4::Decompress: truncated match_length");
                }
                const u8 b = *ip++;
                mat_len += b;
                if (b != 0xFF) break;
                if (mat_len > 0x10000000u) {
                    return ACS_ERR(Asset, kAcpakSubLz4SrcOverflow,
                                   "CAcpakLz4::Decompress: match_length overflow");
                }
            }
        }
        ...
        if (static_cast<usize>(dst_end - op) < full_match) {
            return ACS_ERR(Asset, kAcpakSubLz4DstOverflow,
                           "CAcpakLz4::Decompress: dst exhausted in match");
        }
