// =============================================================================
// ACS Math — Quat (クォータニオン)
// -----------------------------------------------------------------------------
// 16 バイト整列の Vec4 同様のレイアウト。回転を表現するのに使う。
// 内部は DirectXMath 委譲。
// =============================================================================
#pragma once
...
struct alignas(16) Quat {
    ...
    // ---- ファクトリ ----
    static Quat AxisAngle(Vec3 axis, f32 rad) noexcept;             // 軸 + 角度から構築
    static Quat Euler(f32 pitch_rad, f32 yaw_rad, f32 roll_rad) noexcept;  // オイラー角から
    static Quat FromMatrix(const Mat4& m) noexcept;                  // 回転行列から
};
