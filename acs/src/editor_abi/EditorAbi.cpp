...
struct FEditorHost {
    ...
    FGpuMesh      gm_water_plane;                 // camera近傍へ再配置する96x96水面格子
...
TSharedPtr<FMeshAsset> MakeEditorWaterGrid(u32 cells = 96u) noexcept {
...
bool AdvanceEnsure3D(FEditorHost& h) noexcept {
    ...
    auto water_grid_upload =
        FWaterSurface3D::CreateAdaptivePlaneMesh(
            *dev, h.gm_water_plane, 96u);
...
void DrawInteractiveWater3DPass(
    ...
    for (u32 i = 0u; i < nodes.Size(); ++i) {
        ...
        const u64 surface_id = static_cast<u64>(
        ...
        if (mesh == &host.gm_water_plane) {
            host.water3d.DrawAdaptivePlane(
                ...
                authored_normal_map, authored_normal_strength);
        } else {
