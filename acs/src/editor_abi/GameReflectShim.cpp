...
#include "gameframework/ANode.h"           // 実シーングラフ
#include "gameframework/AComponent.h"      // AComponent::ReflectName
...
// =============================================================================
// インプロセス Play: «DLL 側» が実シーン (real ANode + real components) を所有・tick する。
...
GR_API void* acs_game_scene_create() noexcept {
    ...
    auto* s = static_cast<FGamePlayScene*>(AcsConstruct<FGamePlayScene>());
    if (s != nullptr) {
        s->root = NewObject<ANode>();
        ...
        if (s->root && s->services) s->root->ManagementAccess().SetSceneServices(s->services.Get());
    }
    return s;
}
...
GR_API int acs_game_scene_add_component(void* scene, int idx, const char* type_name) noexcept {
    ...
    n->AttachComponent(static_cast<TUniquePtr<AComponent>&&>(comp));
