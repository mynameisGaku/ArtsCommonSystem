...
FArenaAllocator::FPage* FArenaAllocator::AllocPage(usize size) noexcept {
    usize total = sizeof(FPage) + size + 64;
    void* raw = _backing->Alloc(total, alignof(FPage), FSourceLoc::Current());
    if (!raw) return nullptr;
    auto* p = static_cast<FPage*>(raw);
    p->next = nullptr;
    // データ領域はヘッダ直後を 64B 整列した位置から始める（SIMD 安全）
    p->base = reinterpret_cast<u8*>(AlignUp(static_cast<u8*>(raw) + sizeof(FPage), 64));
    p->size = size;
...
void* FArenaAllocator::Alloc(usize size, usize alignment, FSourceLoc /*loc*/) noexcept {
    ...
    while (true) {
        FPage* p = _current.Load(EMemoryOrder::Acquire);
        if (p) {
            // 現在ページに収まるか確認
...
void FArenaAllocator::Reset(bool release_pages) noexcept {
    ...
    if (release_pages) {
        ...
        // ページを保持してカーソルだけ巻き戻し
        for (FPage* p = _pages; p; p = p->next) p->used.Store(0, EMemoryOrder::Release);
