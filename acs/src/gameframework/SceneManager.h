...
public:
    ...
    // フレーム頭で pending 遷移を適用する。退場 FScene は GPU が直前フレームで
    ...
    // top のシーンを描画する。FRenderContext は呼び出し側が用意。
    ...
    // 退場 FScene retain frames (= フレームインフライト 2 + 1)。
...
private:
    ...
    TArray<TUniquePtr<FScene>> _stack;          // top = Back()
    Op                      _pending_op   = Op::None;
    TUniquePtr<FScene>        _pending_arg;    // Change/Push の next、Pop では未使用
    TUniquePtr<FScene>        _retired[kRetireRingSize];  // GPU 遅延削除 ring buffer
