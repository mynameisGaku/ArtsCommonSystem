...
//
// シーンの中身を表す唯一のノードクラス (2D 専用、抽象 FNode 基底は作らない)。
...
//   ・**transform**: `_local` を真値、`FWorld()` は親をたどってオンザフライ計算
...
//
// 範囲外 (Phase 5+ で):
//   ・FComponent2D (FSprite/FAnimation/Collider など)
...
public:
    ...
    FTransform2D FWorld() const noexcept;
    ...
    // ノード単位に振られる generational handle。FScene 内で唯一であることは
    ...
    // 固定刻み update を subtree に propagate (Pillar A polish)。FScene の
