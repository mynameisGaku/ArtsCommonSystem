...
// GameFramework Pillar O/G — FInventorySystem 実装
//
// 設計上のポイント (ヘッダの設計コメントと対応):
//   ・item_id は const char* per-byte 線形検索 (Entitlement / FEconomyDirector と同設計)。
...
//   ・WARN は FEconomyDirector / Entitlement と同じ Log.h 経由。
...
void FInventorySystem::NotifyChange(u32 slot_index) noexcept {
    ...
    const FInventorySlot& s = _slots[slot_index];
...
u32 FInventorySystem::AddItem(const char* item_id, u32 count) noexcept {
    ...
    // FItemDef::id をリテラル参照として保存する (slot.item_id は _items[].id を指す)。
    const FItemDef& def     = _items[def_idx];
...
bool FInventorySystem::HasItem(const char* item_id, u32 min_count) const noexcept {
...
u32 FInventorySystem::ItemTotal(const char* item_id) const noexcept {
    ...
    for (u32 i = 0; i < n_slots; ++i) {
        const FInventorySlot& s = _slots[i];
        if (s.item_id == nullptr) continue;
        if (!StrEq(s.item_id, item_id)) continue;
...
bool FInventorySystem::MoveSlot(u32 from_index, u32 to_index) noexcept {
    ...
    FInventorySlot& from = _slots[from_index];
    FInventorySlot& to   = _slots[to_index];
    ...
    if (StrEq(from.item_id, to.item_id)) {
        ...
        // max_stack は FItemDef から引く必要がある。未登録 (= 移行中の異常 slot) なら 1 扱い。
