...
// ---- FStatConfig: stat 種別ごとの設定 (全 survivor で共通) -------------------
...
// ---- FStatState: ある survivor の 1 stat の実状態 ----------------------------
// current     : 現在値 ([0, max] でクランプ)。
// max         : 上限値 (FStatConfig::max_value のコピー。SetMaxStat 系は将来拡張)。
...
// ---- FSurvivorId: 24bit index + 8bit gen を packed した opaque handle -------
...
struct FSurvivorId {
    ...
    bool operator==(FSurvivorId o) const noexcept { return _packed == o._packed; }
    bool operator!=(FSurvivorId o) const noexcept { return _packed != o._packed; }
};
...
public:
    ...
    // 非コピー・非ムーブ: 内部 TArray<FSurvivorSlot> + その中の TArray<FStatState>。
    ...
    // ---- 初期化 -----------------------------------------------------------
    // 7 stat 分の FStatConfig をデフォルト値で初期化する。コンストラクタでは
    ...
    // 既存 survivor の FStatState には影響しない (= 既に走っている survivor の
...
private:
    // ---- 内部 survivor slot ----------------------------------------------
    // 各 survivor の FStatState を 7 個固定で持つ。`in_use=false` の slot は
    ...
    // ---- 検索 -----------------------------------------------------------
    // survivor handle → FSurvivorSlot* (gen 一致 + in_use + 範囲チェック)。
    ...
    // ---- 状態 -----------------------------------------------------------
    TArray<FStatConfig>    _configs   {};   // 7 stat の共通 config
    TArray<FSurvivorSlot>  _survivors {};   // FSurvivorSlot 配列 (generational)
