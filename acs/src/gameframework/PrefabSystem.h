...
struct PrefabId {
    ...
    /// 0 = invalid。「登録済 slot に存在するか」は PrefabSystem 側で別途検証する。
...
public:
    ...
    PrefabSystem(PrefabSystem&&)                 = delete;
    PrefabSystem& operator=(PrefabSystem&&)      = delete;

    /// 新規 Prefab を登録して `PrefabId` を返す。
    /// `name` は永続文字列を渡すこと (string literal か別バッファ管理)、
    /// PrefabSystem は複製せずポインタを保管する。`nullptr` や空文字、
    ...
    /// 名前で検索 (登録順で最初に一致したもの)。
    /// 一致しない / `name == nullptr` の時は invalid PrefabId を返す。
