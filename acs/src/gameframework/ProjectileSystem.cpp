...
//   ・固定容量 FSlot pool の Acquire / Release (generational handle)
//   ・def_id 名前引きでの FProjectileDef 取得
...
// Init
// -----------------------------------------------------------------------------
// pool 容量を確定し、FSlot 配列と AllAlive 用 snapshot buffer を default 構築する。
...
FProjectileSystem::FSlot* FProjectileSystem::FindSlot(FProjectileId id) noexcept {
    ...
    FSlot& s = _slots[static_cast<usize>(idx)];
