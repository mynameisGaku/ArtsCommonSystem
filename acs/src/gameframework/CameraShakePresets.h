...
// GameFramework Pillar E Phase 3 — CameraShakePresets
//
// Camera2D (= IShakeTarget) に対して「爆発 / 地震 / 着弾」等のジャンル別
...
//     Eiserloh 流の挙動を維持する。
//   ・**duration_hint は CameraShakePresets 自身は使わない**: 単なる "この
...
//   ・directional shake (現状は等方ノイズ、方向ベクトルは Camera2D 拡張で)
...
// duration_hint は Camera2D 側 API には現状反映されない (frequency は
// Camera2D 実装 (Tick 内 25.0f hard-coded) を将来 SetShakeFrequency 化する
...
struct ShakeParams {
    ...
    f32 frequency      = 25.0f;  // 主要振動周波数 (Camera2D 拡張予約)。
    f32 duration_hint  = 1.0f;   // 約何秒で減衰しきるかの目安 = trauma / decay_rate。
};

// ---- IShakeTarget seam ----------------------------------------------------
// CameraShakePresets が trauma 値を流し込む先の純粋仮想インターフェース。
// Camera2D は Phase 3+ でこれを派生する (現状の AddShake / SetShakeAmplitude
...
// ---- CameraShakePresets ---------------------------------------------------
class CameraShakePresets {
public:
    CameraShakePresets()  noexcept = default;
    ~CameraShakePresets() noexcept = default;

    CameraShakePresets(const CameraShakePresets&)            = delete;
    CameraShakePresets& operator=(const CameraShakePresets&) = delete;
    CameraShakePresets(CameraShakePresets&&)                 = delete;
    CameraShakePresets& operator=(CameraShakePresets&&)      = delete;
