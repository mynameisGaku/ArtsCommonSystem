...
//     - FPickupSystem は「FCircle 形状の pickup を専用に高速処理する」軽量サブセット。
...
struct FPickupId {
    ...
    constexpr bool operator==(FPickupId o) const noexcept { return _packed == o._packed; }
    constexpr bool operator!=(FPickupId o) const noexcept { return _packed != o._packed; }
};

// ---- FPickupInfo: pickup 1 件の定義 -----------------------------------------
...
public:
    ...
    //   id      : 拾われた FPickupId (この時点で Despawn 済み)
    //   kind    : pickup 種別
    ...
    //   id   : 消滅した FPickupId (この時点で Despawn 済み)
    ...
    // pickup を世界に登録。戻り値は新規 FPickupId (失敗時 invalid)。
    ...
    // FPickupInfo の生ポインタ取得 (Despawn / Spawn / ClearAll で無効化される)。
    // 無効 id / 既消滅は nullptr。
