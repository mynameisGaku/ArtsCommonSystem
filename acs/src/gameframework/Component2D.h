...
//     最初の一致を返す (線形探索)。
//   ・**lifecycle**: AddComponent 即時 `OnAttach`、FNode 破棄時に `OnDetach`
//     (compact pattern で OnDespawn と一緒に呼ぶ)。OnUpdate/OnDraw は FNode の
//     対応フックの後に呼ばれる。
#pragma once
...
public:
    ...
    // 固定刻み update。FGame の fixed-step accumulator から FScene 経由で呼ばれ、
