...
void PhysicsBody2D::SyncShapeIfRegistered() noexcept {
    ...
    const Vec2 pos = Owner().Local().position;
    if (_kind == ShapeKind::Circle) {
...
bool PhysicsBody2D::WouldBlockAt(Vec2 pos) noexcept {
...
void PhysicsBody2D::OnUpdate(f32 dt) noexcept {
    ...
    // X 軸
    const Vec2 try_x{pos.x + velocity.x * dt, pos.y};
    ...
    // Y 軸 (X 試行後の位置から)
    const Vec2 try_y{next.x, next.y + velocity.y * dt};
