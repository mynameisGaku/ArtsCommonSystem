...
// GameFramework Pillar O/G — InventorySystem (アイテムスロット + stack 管理)
//
...
// サポートする。EconomyDirector が「通貨で買って消える」即時取引を担うのに対し、
// InventorySystem は「拾った / 報酬で得たアイテムを、プレイヤーが持ち歩く」状態を
// 保持する責務を持つ。
//
// 想定する位置付け:
//   ・Pillar O EconomyDirector との違い:
//     - EconomyDirector は「ゲーム内通貨 ↔ shop 商品」の即時購入トランザクション。
//     - InventorySystem は「プレイヤーが保持するアイテム」の slot ベース在庫。
//       購入完了後に AddItem() を呼んでもらう橋渡し設計 (本クラスはストア非依存)。
//   ・Pillar O CharacterCustomizer との違い:
//     - CharacterCustomizer は cosmetic (見た目装着) — slot 単位で最大 1 つ、stack なし。
//     - InventorySystem は使用アイテム / 素材 / クエスト系 — stack あり、slot 数固定。
//   ・Pillar O Entitlement との違い:
//     - Entitlement は「DLC / SeasonPass 等の永続権利フラグ」(持っているかの真偽)。
//     - InventorySystem は「個数を持つ消費可能アイテム」(stack 数を持つ)。
//
// 使い方:
//   InventorySystem inv;
...
//     (アセット二重ロード保護)。EconomyDirector / Entitlement と同じパターン。
...
//   ・craft / 合成 / 消費レシピ — 別 Manager (CraftingSystem 等)。
...
public:
    ...
    InventorySystem()  noexcept = default;
    ~InventorySystem() noexcept = default;
