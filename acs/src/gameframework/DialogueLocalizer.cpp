...
// GameFramework 完成度システム — FDialogueLocalizer 実装
//
// 責務:
//   ・行 / 選択肢メタ (key のみ) の蓄積
//   ・StartFromLine で FLocalizationDirector::Get を 2 回 (speaker / text) 叩いて
//     callback に流すだけのシンプルな bridge
//
// 状態は持たない (進行状態は FDialogueSystem 側で管理)。
...
const char* ResolveKey(const FLocalizationDirector* loc, const char* key) noexcept {
    ...
    if (key == nullptr) return "";
    // FLocalizationDirector::Get は未翻訳時に key 自身、key==nullptr で "" を返す。
...
void FDialogueLocalizer::StartFromLine(u32 line_index, BindCallback cb, void* user) noexcept {
    ...
    // 範囲外 index は no-op (= FDialogueChoice::next_line_index で UINT32_MAX 等が
    // 渡されて終了扱いになるケースをここで吸収)。
    if (line_index >= static_cast<u32>(_lines.Size())) return;

    const FLocalizedDialogueLine& line = _lines[line_index];
