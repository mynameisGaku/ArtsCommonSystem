 ...
 *
 * @param a 比較文字列 1 (nullptr なら false)。
 * @param b 比較文字列 2 (nullptr なら false)。
...
/**
 * 「ワークベンチ不要」を意味する文字列かを判定する。
 *
 * @param s 判定対象文字列。
 * @return nullptr または空文字なら true。
 */
...
bool CCraftingSystem::CanCraft(const char* recipe_id,
    ...
    const FCraftRecipe& r = m_Recipes[slot];
...
void CCraftingSystem::CancelCraft() noexcept {
    ...
    const FCraftRecipe& r = *m_CurrentRecipe;
