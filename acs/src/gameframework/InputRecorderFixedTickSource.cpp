...
CInputRecorderFixedTickSource::CInputRecorderFixedTickSource(const CInputRecorder& recorder,
                                                             FInputSampleKeyDecoder key_decoder) noexcept
    : m_Recorder(recorder), m_KeyDecoder(key_decoder)
{
}

void CInputRecorderFixedTickSource::Reset() noexcept
{
...
bool CInputRecorderFixedTickSource::TryBuildSnapshot_Internal(u32 fixed_tick, FReplayState& state,
                                                              FInputStateSnapshot& output) const noexcept
{
    ...
    const CInputRecorder::FReplayReadAdapter reader = m_Recorder.ReplayReadAccess();
...
bool CInputRecorderFixedTickSource::TryCaptureFixedTickInput(u64 fixed_tick, FInputStateSnapshot& output) noexcept
{
    if (fixed_tick > static_cast<u64>(~u32{0}) || m_KeyDecoder == nullptr) return false;

    FReplayState staged_state{};
    if (m_HasLastFixedTick && m_LastFixedTick < ~u64{0} && fixed_tick == m_LastFixedTick + 1u) staged_state = m_State;
