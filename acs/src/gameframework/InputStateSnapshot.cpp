...
/** ゲームパッドボタンの保持状態と今回の変化を検証して設定する。 */
bool FInputStateSnapshot::TrySetGamepadButtonState(u32 player_index, EGamepadButton button, bool down, bool pressed, bool released) noexcept
{
    /** 設定するゲームパッドボタンの配列位置。 */
...
/** ゲームパッド軸の正規化済み値を検証して設定する。 */
bool FInputStateSnapshot::TrySetGamepadAxis(u32 player_index, EGamepadAxis axis, f32 value) noexcept
{
    /** 設定するゲームパッド軸の配列位置。 */
    usize axis_index = 0;
    if (player_index >= kGamepadPlayerCount || !TryGamepadAxisIndex(axis, axis_index) || !std::isfinite(value)) return false;
    /** トリガーにだけ適用する片方向の値域判定。 */
    const bool is_trigger = axis == EGamepadAxis::LeftTrigger || axis == EGamepadAxis::RightTrigger;
    if ((is_trigger && (value < 0.0f || value > 1.0f)) || (!is_trigger && (value < -1.0f || value > 1.0f))) return false;
...
{
    usize button_index = 0;
    return player_index < kGamepadPlayerCount && TryGamepadButtonIndex(button, button_index) && m_GamepadButtonsDown[player_index][button_index];
}
...
{
    usize button_index = 0;
    return player_index < kGamepadPlayerCount && TryGamepadButtonIndex(button, button_index) && m_GamepadButtonsPressed[player_index][button_index];
}
...
{
    usize button_index = 0;
    return player_index < kGamepadPlayerCount && TryGamepadButtonIndex(button, button_index) && m_GamepadButtonsReleased[player_index][button_index];
}
...
{
    usize axis_index = 0;
    return player_index < kGamepadPlayerCount && TryGamepadAxisIndex(axis, axis_index) ? m_GamepadAxes[player_index][axis_index] : 0.0f;
}
