...
//
// 設計方針:
//   ・シーン死亡で bundle 廃棄 → 内部 TRc<FAsset> が drop → FAssetRegistry の refcount
...
public:
    ...
    // bundle は FScene にメンバとして埋め込む想定。コピー禁止 (内部 TArray 規約)。
    ...
    // (デストラクタに任せても良いが、FScene::OnExit で明示的に呼ぶのが推奨)。
