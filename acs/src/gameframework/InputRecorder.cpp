...
//     Capture 側は値コピーのみ。FInputSample の sizeof は環境依存 (FVec2 = 8 B,
//     padding 込みで 28〜32 B) だが、本クラスは内部で sizeof に依存しないため
//     ABI 変動に耐える。Phase D-2 の file I/O 時は明示的に field を 1 つずつ
//     little-endian で書き出す予定 (`FInputSample` 自体の memcpy には頼らない)。
...
bool FInputRecorder::ConsumeSample(u32 tick, FInputSample& out) noexcept {
    ...
    for (usize i = _cursor; i < n; ++i) {
        const FInputSample& s = _samples[i];
        if (s.tick == tick) {
            out = s;
...
// -----------------------------------------------------------------------------
// Phase D-2 の擬似コード:
//   1. required = 16 (header) + sample_count * sizeof_on_disk(FInputSample) + 4 (footer)
...
//   4. sample_count * sizeof_on_disk(FInputSample) + 20 が size と一致しなければ kSub_BadSize
