...
//   ・WARN ログは Entitlement / FAchievementManager と同じ Log.h 経由。
...
bool FSeasonPass::IsRewardClaimed(u32 tier_index, bool premium) const noexcept {
    ...
    const FClaimState& cs = _claims[slot];
...
bool FSeasonPass::ClaimReward(u32 tier_index, bool premium) noexcept {
    ...
    const FTier& tier = _tiers[slot];
    FClaimState& cs   = _claims[slot];
...
u32 FSeasonPass::ClaimableCount() const noexcept {
    ...
    const usize n = _tiers.Size();
    for (usize i = 0; i < n; ++i) {
        const FTier&       tier = _tiers[i];
        const FClaimState& cs   = _claims[i];
