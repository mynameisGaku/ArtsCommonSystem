...
// エディタ用パネル。Unity の Hierarchy / Godot の SceneTree / UE の FWorld Outliner
// 相当の役割で、retail ビルドからは #ifdef で消す前提。
//
// 機能:
//   ・root_node 配下を再帰 TreeNode で描画 ("FScene Hierarchy" タイトルの ImGui window)
...
public:
    ...
    // 非コピー・非ムーブ: 内部 TArray<FCollapsedEntry> + raw pointer の所有を
    // 曖昧にしない。
    ...
    // メイン ImGui window 描画。`Begin("FScene Hierarchy")` 1 window で完結。
    ...
    const char* Title() const noexcept override { return "FScene Hierarchy"; }
...
private:
    // 折りたたみ状態 1 エントリ。TArray<FCollapsedEntry> を FNodeId で linear search
    ...
    TArray<FCollapsedEntry>     _collapsed_map      {};
