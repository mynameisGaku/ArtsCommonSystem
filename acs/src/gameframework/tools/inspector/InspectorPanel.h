...
// (Phase 20-1 SceneHierarchyPanel) で選択された FNode に対して、紐づく provider
// の field 配列を表示する。
//
// 役割分担:
...
public:
    ...
    // 所有するリテラル文字列 (FInspectableField::name)。
    ...
    // ---- 公開定数 -------------------------------------------------------
    // F32 SliderFloat の既定 min/max。FInspectableField に min/max metadata が
    // 追加されるまでの暫定値。
