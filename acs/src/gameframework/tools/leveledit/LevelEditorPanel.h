...
// GameFramework Pillar — leveledit / LevelEditorPanel (Phase 22)
//
// `acs::game::Tilemap` (multi-layer u16 TileId grid) を **対話的に編集する
...
//
// 役割:
//   ・ImGui canvas 上に Tilemap を矩形描画 (= ImDrawList::AddRectFilled の
...
//   ・中央 viewport: 2D camera (`EditorCamera` Mode2D 内包) で pan / zoom
//   ・マウス操作:
//       - LMB drag (Paint)  : `Tilemap::SetTile(x, y, current_tile_id, layer)`
//       - LMB click (Erase) : `Tilemap::SetTile(x, y, TileId{0}, layer)`
...
//
// 役割分担:
//   ・本 panel は「**tilemap の編集 UI** だけ」を担当。実際の Tilemap データは
//     caller 所有 (= `SetTilemap(&tm)` で raw 参照を渡す、寿命は caller 責任)。
//     本 panel が tilemap を生成 / 破棄しない (AnimCurveEditorPanel が
//     AnimationCurve を non-owning で受けるのと同形)。
//   ・Tilemap の描画 (本物テクスチャ atlas) は **Phase 2 以降の責務**。Phase 22
...
//   map.FillRect(0, 0, 31, 0, TileId{1}, 0);  // 床
...
//     DrawUI override。
//   ・**tilemap は raw pointer の非所有保持**: caller 所有 (AnimCurveEditorPanel
...
//     Erase=消し (TileId{0} 強制)、Fill=flood-fill (連結成分塗替)、Pick=スポイト
...
//     EditorPanel と同パターン (ImGui Demo の Canvas example と同形)。
//   ・**flood-fill の上限 sentinel**: Tilemap 全 cell 数 (= 32*32=1024 程度)
...
//     ModelViewerPanel / InspectorPanel) と同形。
//
...
//   ・タイル単位 flip / rotate (= TileId 自体は純粋 id のまま、flag は別配列が必要)
#pragma once
...
namespace acs::game {
// 編集対象の Tilemap は本ヘッダから forward-decl のみで受ける。
// `<gameframework/Tilemap.h>` を include しないことで、本 panel を利用する側が
// ヘッダ依存を最小化できる (= Tilemap 自体の変更で不要な再ビルドを避ける)。
class Tilemap;
} // namespace acs::game
...
// Erase=消し (TileId{0} を SetTile、drag 対応)
...
// ---------------------------------------------------------------------------
// LevelEditorPanel — Tilemap (multi-layer u16 TileId grid) 編集 UI panel
// ---------------------------------------------------------------------------
class LevelEditorPanel : public acs::game::editor_core::EditorPanel {
public:
    LevelEditorPanel() noexcept = default;
    ~LevelEditorPanel() noexcept override = default;

    // 非コピー / 非ムーブ: 基底 EditorPanel と同規約 + 内部 EditorCamera
    // (非コピー / 非ムーブ) + tilemap 参照 / brush state の所有を曖昧にしない
    // (ACS 規約)。
    LevelEditorPanel(const LevelEditorPanel&)            = delete;
    LevelEditorPanel& operator=(const LevelEditorPanel&) = delete;
    LevelEditorPanel(LevelEditorPanel&&)                 = delete;
    LevelEditorPanel& operator=(LevelEditorPanel&&)      = delete;

    // ----- 初期化 / 解放 ---------------------------------------------------

    // 内部 state をデフォルトに、EditorCamera を 2D mode で Init、tilemap 参照を
    // nullptr、brush=Paint、tile_id=1、active_layer=0、show_grid=true、
    // snap_to_grid=true、selected coord を unset に。多重 Init 可 (= 完全リセット)。
    void Init() noexcept;

    // 内部 state を全解放 (= tilemap 参照を解除、EditorCamera を Reset)。
    // tilemap 自体は caller 所有なので本 panel が破棄しない。多重 Shutdown 可。
    void Shutdown() noexcept;

    // ----- tilemap バインド ------------------------------------------------

    // 編集対象の Tilemap を raw 参照でセット (nullptr で解除)。
    ...
    void SetTilemap(class Tilemap* tm) noexcept;

    // 現在編集対象の Tilemap (未バインド時は nullptr)。
    class Tilemap* CurrentTilemap() const noexcept;
    ...
    // アクティブレイヤ番号 (`Tilemap::LayerCount()` 未満)。
    ...
    // 本クラスでは追加初期化 (EditorCamera を 2D mode で Init し直す保険) を行う。
    ...
    // flood-fill (Brush=Fill) の最大処理セル数。Tilemap 全 cell を超える深さの
...
private:
    ...
    // 編集対象 Tilemap (caller 所有、本 panel は非所有)。
    class Tilemap* _tilemap = nullptr;
