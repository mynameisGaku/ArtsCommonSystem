...
//   ・上限超は最古 1 件を捨てる (Add 後にしか発生しないので超過量 = 1)。
...
void CUndoStack::Push(TUniquePtr<AEditorCommand> owned) noexcept
{
    ...
    if (!m_UndoStack.IsEmpty()) {
        AEditorCommand* prev = m_UndoStack.Last().Get();
    ...
    // 4. merge しなかった場合のみ m_UndoStack に Add。
    //    merged のときは owned が scope 抜けで自動 delete される。
    if (!merged) {
        m_UndoStack.Add(Move(owned));
    }
    ...
    if (m_Cb != nullptr && !m_UndoStack.IsEmpty()) {
        m_Cb(m_CbUser, m_UndoStack.Last().Get(), /*is_redo=*/false);
    }
}
...
bool CUndoStack::Undo() noexcept {
    ...
    // top を取り出す。Pop は破棄してしまうので、先に Move で別 TUniquePtr に
    // 移してから Pop する。Move 後の Back() は moved-from 状態 (= 内部
    // pointer が nullptr) なので、Pop の dtor は何もしない。
    TUniquePtr<AEditorCommand> cmd = Move(m_UndoStack.Last());
    m_UndoStack.Pop();
...
bool CUndoStack::Redo() noexcept {
    ...
    TUniquePtr<AEditorCommand> cmd = Move(m_RedoStack.Last());
    m_RedoStack.Pop();
...
u32 CUndoStack::UndoCount() const noexcept {
    return static_cast<u32>(m_UndoStack.Num());
}

/** redo stack に積まれている件数を返す。 */
u32 CUndoStack::RedoCount() const noexcept {
    return static_cast<u32>(m_RedoStack.Num());
}
...
const char* CUndoStack::UndoDescription() const noexcept {
    ...
    const AEditorCommand* top = m_UndoStack.Last().Get();
    if (top == nullptr) {
        return "";
    }
...
const char* CUndoStack::RedoDescription() const noexcept {
    ...
    const AEditorCommand* top = m_RedoStack.Last().Get();
    if (top == nullptr) {
        return "";
    }
    const char* desc = top->Description();
...
/** 上限超過時に最古 1 件を捨てる (Add 直後にのみ呼ぶ、超過量は高々 1)。 */
void CUndoStack::DropOldestIfOverflow() noexcept {
    // m_UndoStack のみが上限管理対象 (redo は Push 時に Clear されるため自然に
    // 上限以下になる)。
    // 超過件数は最大でも 1 (= 直前の Add 1 件分のみ) を想定する。
    while (static_cast<u32>(m_UndoStack.Num()) > m_MaxHistory) {
        // 最古 = index 0 を捨てる。TArray は RemoveAt(0) を持たないので、
        // 自前で 1 つずつ前にずらしてから Pop する。
        // ・要素は TUniquePtr<AEditorCommand> なのでムーブ可能。
        // ・Pop 前に index 0 が末尾要素まで移送されていれば、Pop の
        //   破棄で最古 cmd が dtor 経由で delete される。
        const usize n = m_UndoStack.Num();
        for (usize i = 1; i < n; ++i) {
