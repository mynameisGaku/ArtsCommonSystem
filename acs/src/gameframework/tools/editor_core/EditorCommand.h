...
//   する **undo/redo の原子単位**。FCommand パターンの GoF 古典に沿い、
...
//     "Undo Move FNode" 表示) に直接渡す想定。動的文字列が要る派生クラスは
...
public:
    ...
    // undo history UI 表示用の短い人間可読ラベル ("Move FNode", "Add Component" 等)。
...
// =============================================================================
// FMoveNodeCommand — FNode2D の position を変更する FEditorCommand 派生サンプル
// -----------------------------------------------------------------------------
...
//   acs::TUniquePtr<FMoveNodeCommand> c = acs::MakeUnique<FMoveNodeCommand>(
//       &node, old_pos, new_pos);
//   undo_stack.Push(c.Release());
//
// マージ規約:
//   ・同じ `_target` (raw pointer 一致) かつ同じ kind (= FMoveNodeCommand)
//     なら merge OK。
...
public:
    FMoveNodeCommand(FNode2D* target, FVec2 old_pos, FVec2 new_pos) noexcept
    ...
    // 同一 target に対する連続 FMoveNodeCommand のみ merge を許す。
