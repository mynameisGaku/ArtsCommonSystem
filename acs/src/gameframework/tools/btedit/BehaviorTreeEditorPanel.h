...
//   │ └───────────────────────────────────────────────────────────┘ │
//   │ ┌── left: Tree view ─────┐  ┌── right: FNode Inspector ────┐ │
...
public:
    ...
    // 非コピー / 非ムーブ: 内部 `TArray<FNodeMeta>` + `TArray<u8>` の所有を曖昧に
    ...
    // メタミラー (FNodeMeta 配列) と autorun フラグは触らない。
    void Reset() noexcept;
...
private:
    ...
    // 配列内 index == id == FNodeMeta::id (= 三位一体で常に等しい)。
    struct FNodeMeta {
