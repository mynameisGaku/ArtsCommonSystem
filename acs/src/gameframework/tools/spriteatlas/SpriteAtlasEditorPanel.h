...
// GameFramework Pillar — spriteatlas / SpriteAtlasEditorPanel (Phase 22 第一弾)
//
// `acs::game::SpritePack` (gameframework/SpritePack.h) が保持する atlas メタ情報
// と名前付き frame 矩形のリストを、ImGui ベースで対話的に編集する **エディタ
// パネル**。Phase 21a で導入された editor 共通基盤
// (`editor_core::EditorPanel` / `EditorWorkspace`) を継承し、ModelViewer
...
//   ・SpritePack そのものの所有はしない (= caller 所有、本 panel は raw pointer
//     を非所有で保持する。SpritePack は非コピー / 非ムーブなので、参照保持しか
//     できない設計とも整合)。
//
// 役割分担 (Phase 22 SpriteAtlasEditor 全体):
//   ・本 panel (SpriteAtlasEditorPanel) : ImGui UI + frame rect 編集
...
//   // ... 既存 frame を AddFrame ...
//
//   spriteatlas::SpriteAtlasEditorPanel panel;
//   panel.Init();
...
//
// 設計選択 (Phase 22 第一弾):
//   ・**EditorPanel 継承**: Phase 21a 共通基盤を dogfood。Title / DrawUI /
//     OnInit を override。OnInit では基底実装を必ず呼ぶ。ModelViewerPanel と
//     同形 (Phase 21b 確立済パターン)。
//   ・**SpritePack は raw pointer 非所有保持**: SpritePack は非コピー / 非ムーブ
//     なので参照保持しか選択肢が無く、それを noexcept ABI に乗せるため
//     `class SpritePack*` (forward decl + .h からは触らない実装)。caller が
...
//   ・**選択は単一 frame index (i32)**: -1 = 未選択。ParticleEditorPanel と
...
//   ・animation timeline 連携 (SpriteAnimator との結合は Phase 22+ で別 panel)
...
namespace acs::game {
// SpritePack の forward-decl。SpritePack.h は .cpp 側でのみ include する
// (= ヘッダ依存最小化、利用側が SpritePack の API 変更で再ビルドさせられない)。
class SpritePack;
} // namespace acs::game
...
// ---------------------------------------------------------------------------
// SpriteAtlasEditorPanel — atlas texture path + 名前付き frame 列を編集
// ---------------------------------------------------------------------------
class SpriteAtlasEditorPanel : public acs::game::editor_core::EditorPanel {
public:
    SpriteAtlasEditorPanel() noexcept = default;
    ~SpriteAtlasEditorPanel() noexcept override = default;

    // 非コピー・非ムーブ: 内部 frame name pool (静的配列) と SpritePack* の
    // 参照関係を曖昧にしない (ACS 規約 + 他 panel 群と同形)。
    SpriteAtlasEditorPanel(const SpriteAtlasEditorPanel&)            = delete;
    SpriteAtlasEditorPanel& operator=(const SpriteAtlasEditorPanel&) = delete;
    SpriteAtlasEditorPanel(SpriteAtlasEditorPanel&&)                 = delete;
    SpriteAtlasEditorPanel& operator=(SpriteAtlasEditorPanel&&)      = delete;

    // ----- 初期化 / 解放 ---------------------------------------------------

    // 内部 state をデフォルトに、SpritePack 参照を nullptr、selection を解除、
    // zoom = 1.0、pivot preset = Center に。Workspace 登録は別途
    // `EditorWorkspace::RegisterPanel(&this)` で行う (= OnInit が呼ばれる)。
    // 多重 Init 可 (= 完全リセット)。
    void Init() noexcept;

    // 内部 state を解放 (frame name pool / selection / SpritePack* をクリア)。
    ...
    // 編集対象 SpritePack を注入する (raw pointer / 非所有)。
    // `nullptr` を渡すと「pack 未接続」状態になり、UI は gracefully 退化する
    // (Add/Delete/Pivot ボタンが disabled、説明文 "(No SpritePack attached)" を表示)。
    // selection は 0 にリセットされる (= 新しい pack の最初の frame を選ぶ意図)。
    void SetSpritePack(class acs::game::SpritePack* pack) noexcept;

    // 現在注入されている SpritePack。未注入時は nullptr。
    class acs::game::SpritePack* CurrentPack() const noexcept;
    ...
    // SpritePack に const char* として渡す。追加後 selection を新規 frame に移す。
    // pack 未注入 or 上限到達 (= kMaxOwnedFrames) なら no-op。
    void AddFrame() noexcept;

    // 選択中 frame を SpritePack から削除する。
    // 削除は SpritePack::RemoveFrame で行うが、SpritePack の RemoveFrame は
    ...
    // no-op。SpritePack は順序非保持 swap remove のため、削除後の index 整合は
    // panel 側で SpritePack::AllFrames を再走査して取り直す (= state 同期)。
...
private:
    // ---- 注入された編集対象 SpritePack (raw / 非所有) ----
    class acs::game::SpritePack* _pack = nullptr;
    ...
    // SpritePack に const char* を渡すため、panel 寿命中に確実に生存する
