...
//
// ImGui レイアウト (DrawUI):
//   ┌────────────── "FParticle Editor" window ───────────────────┐
//   │ ┌─ left column ──┐  ┌─ right column ──────────────────┐   │
//   │ │ [FEmitter list] │  │ Selected: idx=N                  │   │
//   │ │  - FEmitter 0   │  │ lifetime_sec     [slider]        │   │
//   │ │  - FEmitter 1   │  │ emit_rate_per_sec[slider]        │   │
//   │ │  ...           │  │ burst_count      [slider]        │   │
...
public:
    ...
    // 非コピー・非ムーブ: 内部 TArray<FParticleEmitterDef> の所有を曖昧にしない。
    ...
    const char* Title() const noexcept override { return "FParticle Editor"; }

    // FEditorPanel override: メイン ImGui window 描画。target system が set されて
    // いれば active particle 数を表示。`Begin("FParticle Editor")` から始まる
    // 単一 window レイアウト。
...
private:
    TArray<FParticleEmitterDef> _emitters {};
    // editor 専用の「将来 FParticleEmitterDef に追加される予定」の値。
    // 現状 FParticleEmitterDef に spread_radians フィールドが無いので、
