...
//
// Phase 1: FSceneManager 駆動 + FRenderContext 配線。
// Phase 2 (本フェーズ): 固定タイムステップ accumulator + AppState 型消去
//   永続状態 + FScene への dt は time_scale 乗算済を渡す。
...
public:
    ...
    FRenderContext& GetRenderCtx()  noexcept { return _render_ctx; }

    // 時間スケール。FScene::OnUpdate / OnFixedUpdate に渡る dt に乗算される。
...
protected:
    // 派生クラス実装必須: 最初に push される FScene を返す。
