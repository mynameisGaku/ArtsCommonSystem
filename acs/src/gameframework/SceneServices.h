...
//           return ESvc::Default2D;  // FClock | Tweens | Sequences | FInput
//       }
...
//           // dt は FClock 経由 scaled (services 有効時)。Tweens/Sequences は
...
//   ・**2 phase tick**: PreUpdate (FClock 進行) → scene.OnUpdate → PostUpdate
//     (Tweens/Sequences tick)。新規スケジュールは次フレーム頭から進行 (predictable)。
//   ・**自動 pause**: シーンが下位に追いやられた間は OnUpdate が呼ばれず、
//     FClock も tick されないので tween/seq は自然に止まる。明示的 Pause 不要。
//
// 範囲外 (Phase 8 では):
...
enum class ESvc : u32 {
    ...
    Default2D  = FClock | Tweens | Sequences | FInput,
};
...
public:
    ...
    FSceneClock&          FClock()     noexcept;
    FTweenManager&        Tweens()    noexcept;
    FSequenceRunner&      Sequences() noexcept;
    FInputMap&            FInput()     noexcept;
    acs::game::FCamera2D& FCamera()    noexcept;
    FCollisionWorld2D&    Physics()   noexcept;

    // FGame/FSceneManager driver (利用者は触らない)。
    //   PreUpdate: FClock.Tick(raw_dt) で時間を進める (= scaled dt が確定)
    ...
    // PreUpdate 後に scene の OnUpdate に渡す dt。FClock 未要求なら raw_dt をそのまま返す。
