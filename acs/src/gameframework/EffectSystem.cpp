...
// GameFramework Pillar I — EffectSystem 実装
//
...
// 打ち消されない)。負値はノーオプ。
// =============================================================================
void EffectSystem::HitStop(f32 duration) noexcept {
...
// 上書きされない)。Camera2D が次フレーム頭で ConsumeShake する想定。
// duration_hint は将来 EffectSystem 内で trauma decay の倍率に使いたい場合の
// 予約 (現状は無視)。
// =============================================================================
void EffectSystem::TriggerShake(f32 trauma, f32 /*duration_hint*/) noexcept {
    if (trauma <= 0.0f) return;
    // pending は max-of-frame (Camera2D の AddShake が累積側を担う)
...
//   ・HitStop timer: 線形減衰。
//   ・Pending shake: ここでは消さない (Camera2D が ConsumeShake する責務)。
// dt が負 (= clock 巻き戻し等) のときは 0 扱いにして state を破壊しない。
// =============================================================================
void EffectSystem::Tick(f32 dt) noexcept {
    if (dt < 0.0f) dt = 0.0f;
...
// =============================================================================
f32 EffectSystem::FlashIntensity() const noexcept {
