...
FAudioDirector::FAudioDirector() noexcept {
    // SFX ring を固定容量で予約。Resize で FSfxEntry をデフォルト構築 (active=false)。
...
FAudioVoiceHandle FAudioDirector::PlayBgmClip(const FAudioClipDesc& clip,
                                            f32 fade_in_sec,
                                            bool loop) noexcept {
    if (_backend == nullptr) {
    ...
    const FAudioVoiceHandle handle = _backend->PlayLooped(clip, 0.0f, 1.0f);
    ...
    if (!loop) {
        ...
        const FAudioVoiceHandle one = _backend->PlayOneShot(clip, 0.0f, 1.0f);
...
FAudioVoiceHandle FAudioDirector::PlaySfxClip(const FAudioClipDesc& clip,
                                            f32 volume_scale,
                                            f32 pitch) noexcept {
