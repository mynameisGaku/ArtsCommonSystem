...
public:
    ...
    f32  Remaining(CooldownId id) const noexcept; // 0.0f if stale / charged
    f32  Progress (CooldownId id) const noexcept; // [0,1]、1.0 = charged

    // ----- 直接操作 -----
    void ForceReady (CooldownId id) noexcept;             // 即時 charged (ReadyCallback 発火)
    void Reset      (CooldownId id) noexcept;             // 即時 duration にリセット (charged=false)
