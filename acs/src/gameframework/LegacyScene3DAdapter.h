...
struct FScene3DClouds {
     ...
     *
     * @details
     * **1.0 でも «等倍» ではない。** 雲は常に画面の 1/4 の寸法でレイマーチし、時間方向の
     * 再構成で埋める作りで、この値はその方策への倍率 (1.0 で 1/4、0.75 で約 1/5.3、
     * 0.5 以下で 1/8)。**下げるとドット感が出る**ので既定は 1.0。
     *
     * 本当に等倍で見たいときは `bReferenceMode`。
     */
