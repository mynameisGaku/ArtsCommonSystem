...
//     FAchievementManager / Entitlement と同じ StrEq を anonymous namespace に
...
//   ・timestamp は FClock::MillisSinceStartup()。Pillar S の Achievement と同じ
...
void FTelemetryDirector::Flush() noexcept {
    ...
    for (usize i = _pending.Size(); i-- > 0;) {
        const FTelemetryEvent& e = _pending[i];
