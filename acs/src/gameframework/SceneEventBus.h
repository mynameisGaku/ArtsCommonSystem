 ...
 *
 * @details
 * 同一 Scene のオブジェクト同士が「相手の存在」を知らずに通知をやり取りするための薄い
 * メッセージング層。EventId (compile-time FNV-1a ハッシュ) で event を識別し、ハンドラは
 * 関数ポインタ + void* user で type-erase する。Subscribe ごとにユニークな u32 handle を
 * 払い出し、Unsubscribe は Entry を mark-inactive するのみで物理削除しないため、Publish 中の
 * Subscribe / Unsubscribe を安全に行える。Scene に埋め込む前提の非コピー・非ムーブ型。
 */
...
public:
     ...
     * 指定 event にハンドラを登録する。
     *
     * @details 既存の inactive Entry を再利用するか末尾に追加し、毎回ユニークな handle を払い出す。
     ...
     *
     * @details Entry を mark-inactive するのみで物理削除しないため、Publish 中に呼んでも安全。
    ...
    /** 全 subscription を破棄する (Scene::OnExit 等で使う)。 */
...
private:
     ...
     * 1 件の購読登録を表すエントリ。
     *
     * @details active=false の Entry は論理削除済みで、再利用または Publish 走査でスキップされる。
     */
    struct Entry {
        /** 購読対象のイベント識別子。 */
        EventId   id;
