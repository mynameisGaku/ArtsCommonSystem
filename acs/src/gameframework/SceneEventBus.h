...
//           _sub = FScene()->Events().Subscribe(
//               FEventId("PlayerDied"), &OnPlayerDied, this);
//       }
//       void OnExit() noexcept override {
//           FScene()->Events().Unsubscribe(_sub);
//       }
...
//   FScene()->Events().Publish(FEventId("PlayerDied"), &p, sizeof(p));
//
// 設計選択 (Phase 1 polish):
...
//   ・cross-scene broadcast (FScene を越えるのは GlobalEventBus が出来てから)
...
public:
    ...
    // 非コピー・非ムーブ (FScene にメンバとして埋め込む前提)
    ...
    // Publish 中に呼んでも安全 (FEntry を物理削除しない)。
    ...
    // 全 subscription を破棄。FScene::OnExit 等で使う。
