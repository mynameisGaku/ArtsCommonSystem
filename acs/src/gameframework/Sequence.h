...
//   ・Tween は FSequence 内蔵 (FTweenManager に委譲しない) — sequence の進行
...
public:
    ...
    const TArray<FSeqAction>& Actions() const noexcept { return _actions; }
    u32 LoopCount() const noexcept { return _loop_count; }

private:
    TArray<FSeqAction> _actions;
    u32              _loop_count = 1;
};
