...
// GameFramework Pillar B Phase 3 — NodeId (シーングラフ用 generational handle)
//
// シーングラフ内の Node2D / Node を一意に識別する 32bit パック handle。
// 1 個の `u32` に **24bit index + 8bit generation** を pack する設計で、
// `ShapeId` (CollisionWorld2D.h) と完全に同じパターンを採用している。
//
// 使い方:
//   NodeId id = scene.CreateNode();          // 何らかのファクトリ経由で取得
//   if (id.IsValid()) {
...
//     呼び出し側 (Scene 等) が別 array で持つ責務。NodeId は純粋に「どの slot
...
struct NodeId {
    ...
    constexpr NodeId(u32 index, u8 gen) noexcept
    ...
    constexpr bool operator==(NodeId o) const noexcept { return _packed == o._packed; }
    constexpr bool operator!=(NodeId o) const noexcept { return _packed != o._packed; }
};
