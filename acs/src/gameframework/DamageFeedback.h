...
//      のみ更新。指定なし (= Vec2{}) なら矢印は触らない (例: 環境ダメージ /
...
//       DamageFeedback _dmg;
//       void OnPlayerHit(f32 amount, Vec2 attacker_pos) noexcept {
//           const Vec2 to_player = player.Position() - attacker_pos;
//           _dmg.TakeDamage(amount, to_player);
//           if (player.HP() <= 0.0f) {
//               _dmg.TriggerDeathCam(Vec3{attacker_pos.x, 0, attacker_pos.y});
//           }
//       }
...
public:
    ...
    void TakeDamage(f32 amount, Vec2 source_direction = Vec2{0.0f, 0.0f}) noexcept;
...
private:
    ...
    Vec3 _killer_pos       = Vec3{0.0f, 0.0f, 0.0f};
