...
// ビルドでも常に使える `FSteamworksBridgeStub` のみを実装する。実 SDK を使う
...
//     保証されているため、追加同期は不要。
//   ・文字列リテラルは static storage duration なので、FPlayerIdentity に
//     そのまま入れても寿命は永続。
...
TResult<void> FSteamworksBridgeStub::UnlockAchievement(const char* ach_id) noexcept {
    ...
    if (!_initialized) {
        return ACS_ERR(Generic, kSubSteamworksNotInitialized,
                       "FSteamworksBridgeStub::UnlockAchievement called before Init()");
    }
    return ACS_ERR(Generic, kSubSteamworksNotImplemented,
                   "FSteamworksBridgeStub: UnlockAchievement is not implemented (link real SDK)");
}
...
TResult<void> FSteamworksBridgeStub::SetLeaderboardScore(const char* board_id, i64 score) noexcept {
    ...
    if (!_initialized) {
        return ACS_ERR(Generic, kSubSteamworksNotInitialized,
                       "FSteamworksBridgeStub::SetLeaderboardScore called before Init()");
    }
    return ACS_ERR(Generic, kSubSteamworksNotImplemented,
                   "FSteamworksBridgeStub: SetLeaderboardScore is not implemented (link real SDK)");
}
...
TResult<i64> FSteamworksBridgeStub::GetLeaderboardScore(const char* board_id) noexcept {
    ...
    if (!_initialized) {
        return TResult<i64>(ACS_ERR(Generic, kSubSteamworksNotInitialized,
                                   "FSteamworksBridgeStub::GetLeaderboardScore called before Init()"));
    }
    return TResult<i64>(ACS_ERR(Generic, kSubSteamworksNotImplemented,
                               "FSteamworksBridgeStub: GetLeaderboardScore is not implemented (link real SDK)"));
}
...
FSteamworksBridgeStub& FSteamworksBridgeStub::GetStub() noexcept {
    ...
    static FSteamworksBridgeStub _instance;
    return _instance;
}
