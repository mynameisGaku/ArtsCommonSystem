...
//  ・Flush 中のコールバックが新規 Enqueue を呼んだ場合、m_Records の Add で
...
//  ・Flush 後の物理削除は「実行済 one_shot を後ろから順に Pop または RemoveAtSwap」
...
void CSceneCommandQueue::Cancel(const char* label) noexcept {
    ...
    while (m_Records.Num() > w) m_Records.Pop();
}
...
void CSceneCommandQueue::Flush() noexcept {
    ...
    //    再エントランシ安全性パターン。
    const usize snapshot_size = m_Records.Num();
    ...
    //    安定順序を維持するため、新 TArray で書き戻す方式を採用 (Pop/Swap は
    //    順序破壊が起き得るため避ける)。
    ...
    const usize now_size = m_Records.Num();
    for (usize i = snapshot_size; i < now_size; ++i) {
        retained.Add(m_Records[i]);
    }
