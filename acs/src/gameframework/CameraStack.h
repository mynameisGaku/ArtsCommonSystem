...
// GameFramework Pillar E — CameraStack (Phase 2)
//
// 複数 `Camera2D` を **virtual camera スタック**として保持し、最上層 (= top) を
...
//       acs::game::Camera2D  _follow_cam;     // プレイヤー追従カメラ
//       acs::game::Camera2D  _cinematic_cam;  // 演出用カメラ (固定 or 別追従)
//       acs::game::CameraStack _stack;
...
//   ・**virtual camera スタック**: Camera2D は user が own。CameraStack は
...
//     [-π, π] に正規化してから lerp。
//   ・**Tick**: 上から順に **active な 2 層**だけ Camera2D::Tick を呼ぶ
...
class CameraStack {
public:
    CameraStack() noexcept = default;
    ~CameraStack() noexcept = default;

    CameraStack(const CameraStack&)            = delete;
    CameraStack& operator=(const CameraStack&) = delete;
    CameraStack(CameraStack&&)                 = delete;
    CameraStack& operator=(CameraStack&&)      = delete;
    ...
    void PushCamera(Camera2D& cam, f32 blend_duration = 0.5f) noexcept;
    ...
    // ----- 状態取得 -----
    Camera2D* Active() const noexcept;          // 現在の top (= 最上層)。空なら nullptr。
    ...
    // ----- driver -----
    // blend timer 進行 + active な 2 層の Camera2D::Tick 呼び出し。pop の
