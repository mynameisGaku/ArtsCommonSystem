...
// GameFramework meta — PerfBudget 実装
//
// state holder のみ。計測 (タイマ / Allocator hook) は呼出し側責務。詳細は
// PerfBudget.h 参照。
...
usize PerfBudget::FindCategoryIndex(const char* category) const noexcept {
...
void PerfBudget::SetFrameBudget(f32 ms) noexcept {
...
void PerfBudget::DefineCategory(const char* category, f32 budget_ms, u32 budget_bytes) noexcept {
...
void PerfBudget::RecordTimeMs(const char* category, f32 elapsed_ms) noexcept {
    if (category == nullptr) return;
...
void PerfBudget::RecordMemoryAlloc(const char* category, u32 bytes) noexcept {
...
void PerfBudget::RecordMemoryFree(const char* category, u32 bytes) noexcept {
    if (category == nullptr || bytes == 0u) return;
    const usize idx = FindCategoryIndex(category);
    if (idx >= _categories.Size()) return;
    BudgetEntry& e = _categories[idx];
...
void PerfBudget::BeginFrame() noexcept {
...
void PerfBudget::EndFrame() noexcept {
    ...
    // 循環バッファ書き込み (DebugOverlay と同じパターン)。
...
bool PerfBudget::IsOverBudget(const char* category) const noexcept {
...
f32 PerfBudget::AverageFrameMs() const noexcept {
...
u32 PerfBudget::CategoryCount() const noexcept {
    return static_cast<u32>(_categories.Size());
}

const BudgetEntry* PerfBudget::AllCategories(u32& out_count) const noexcept {
