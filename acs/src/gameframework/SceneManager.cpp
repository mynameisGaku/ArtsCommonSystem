...
void FSceneManager::DoPopInternal(bool resume_new) noexcept {
    ...
    _stack.Back()->OnExit();
    // 退場 FScene を ring buffer の現在ヘッドに格納 (3 フレーム保持)。
...
void FSceneManager::_ApplyPending(FGame& game) noexcept {
    ...
    // 解放する (= 3 フレーム前に退場した FScene を今ここで破棄)。フレーム
    ...
    Op op = _pending_op;
    _pending_op = Op::None;
    TUniquePtr<FScene> arg = Move(_pending_arg);
...
void FSceneManager::_Update(f32 dt) noexcept {
    FScene* top = Top();
    if (!top) return;
    // Phase 8: services 2 phase tick — PreUpdate (FClock 進行) → scene OnUpdate
    // → PostUpdate (Tweens/Sequences tick)。FClock 未要求なら raw dt をそのまま OnUpdate へ。
