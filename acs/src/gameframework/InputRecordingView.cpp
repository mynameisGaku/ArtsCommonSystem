...
TResult<FInputRecordingView> FInputRecordingView::Decode(TSpan<const u8> bytes) noexcept
{
    if (bytes.Data() == nullptr) {
        return ACS_ERR(IO, FInputRecorder::kSub_NullBuffer, "FInputRecordingView::Decode: buffer is null");
    }
    if (bytes.Size() < input_recording_detail::kHeaderBytes + input_recording_detail::kFooterBytes) {
        return ACS_ERR(IO, FInputRecorder::kSub_BadSize, "FInputRecordingView::Decode: buffer is smaller than header and footer");
    }

    /** 検証対象の固定長 header。 */
    TSpan<const u8> header;
    if (!bytes.TrySubSpan(0u, input_recording_detail::kHeaderBytes, header)) {
        return ACS_ERR(IO, FInputRecorder::kSub_BadSize, "FInputRecordingView::Decode: header range is invalid");
    }
    if (input_recording_detail::ReadU32(header.Data()) != input_recording_detail::kMagic) {
        return ACS_ERR(IO, FInputRecorder::kSub_BadMagic, "FInputRecordingView::Decode: magic mismatch");
    }
    if (input_recording_detail::ReadU32(header.Data() + 4u) != input_recording_detail::kVersion) {
        return ACS_ERR(IO, FInputRecorder::kSub_BadVersion, "FInputRecordingView::Decode: version mismatch");
    }
    ...
    if (tick_rate_hz == 0u || tick_rate_hz > kInputRecorderMaximumTickRateHz || sample_count > kInputRecorderMaximumSamples) {
        return ACS_ERR(IO, FInputRecorder::kSub_LimitExceeded, "FInputRecordingView::Decode: metadata exceeds product limits");
    }
    ...
    if (expected_bytes != static_cast<u64>(bytes.Size())) {
        return ACS_ERR(IO, FInputRecorder::kSub_BadSize, "FInputRecordingView::Decode: sample count and size mismatch");
    }
    ...
    if (!bytes.TrySubSpan(input_recording_detail::kHeaderBytes, static_cast<usize>(sample_bytes_u64), samples)) {
        return ACS_ERR(IO, FInputRecorder::kSub_BadSize, "FInputRecordingView::Decode: sample range is invalid");
    }
    /** CRC footer 領域。 */
    ...
    if (!bytes.TrySubSpan(input_recording_detail::kHeaderBytes + samples.Size(), input_recording_detail::kFooterBytes, footer)) {
        return ACS_ERR(IO, FInputRecorder::kSub_BadSize, "FInputRecordingView::Decode: footer range is invalid");
    }
    if (input_recording_detail::ComputeCrc32(samples) != input_recording_detail::ReadU32(footer.Data())) {
        return ACS_ERR(IO, FInputRecorder::kSub_BadCrc, "FInputRecordingView::Decode: CRC32 mismatch");
    }
    ...
    for (u32 index = 0u; index < sample_count; ++index) {
        ...
        if (!input_recording_detail::HasFiniteMousePosition(sample)) {
            return ACS_ERR(IO, FInputRecorder::kSub_BadValue, "FInputRecordingView::Decode: non-finite mouse position");
        }
    }
