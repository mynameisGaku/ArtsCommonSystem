...
{
    if (bytes.GetData() == nullptr) {
        return ACS_ERR(IO, CInputRecorder::kSub_NullBuffer, "FInputRecordingView::Decode: buffer is null");
    }
    if (bytes.Num() < input_recording_detail::kHeaderBytes + input_recording_detail::kFooterBytes) {
    ...
    if (input_recording_detail::ReadU32(header.GetData()) != input_recording_detail::kMagic) {
        return ACS_ERR(IO, CInputRecorder::kSub_BadMagic, "FInputRecordingView::Decode: magic mismatch");
    }
    if (input_recording_detail::ReadU32(header.GetData() + 4u) != input_recording_detail::kVersion) {
        return ACS_ERR(IO, CInputRecorder::kSub_BadVersion, "FInputRecordingView::Decode: version mismatch");
    }

    /** header に記録された tick rate。 */
    const u32 tick_rate_hz = input_recording_detail::ReadU32(header.GetData() + 8u);
    /** header に記録された sample 件数。 */
    const u32 sample_count = input_recording_detail::ReadU32(header.GetData() + 12u);
    ...
    if (expected_bytes != static_cast<u64>(bytes.Num())) {
    ...
    if (!bytes.TrySubSpan(input_recording_detail::kHeaderBytes + samples.Num(), input_recording_detail::kFooterBytes, footer)) {
        return ACS_ERR(IO, CInputRecorder::kSub_BadSize, "FInputRecordingView::Decode: footer range is invalid");
    }
    if (input_recording_detail::ComputeCrc32(samples) != input_recording_detail::ReadU32(footer.GetData())) {
    ...
    for (u32 index = 0u; index < sample_count; ++index) {
        /** 現在検査する sample の先頭。 */
        const u8* sample = samples.GetData() + static_cast<usize>(index) * input_recording_detail::kSampleBytes;
...
bool FInputRecordingView::DecodeSample(u32 index, FInputSample& out) const noexcept
{
    ...
    input_recording_detail::ReadSample(sample.GetData(), decoded);
    out = decoded;
    return true;
}
