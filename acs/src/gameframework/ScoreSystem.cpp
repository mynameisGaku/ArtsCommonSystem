...
// GameFramework Pillar R/O — FScoreSystem 実装
//
// 設計上のポイント (ヘッダの設計コメントと対応):
//   ・combo→倍率は f32 で計算するが、FScoreEntry には ×100 整数化して記録する
...
void FScoreSystem::Reset() noexcept {
...
void FScoreSystem::ClearAll() noexcept {
...
void FScoreSystem::NotifyMiss() noexcept {
    _combo_count = 0;
    _combo_timer = 0.0f;
}

void FScoreSystem::Tick(f32 dt) noexcept {
...
void FScoreSystem::SetComboDuration(f32 sec) noexcept {
    // 負値 / NaN は 0 に丸める (= コンボ無効化相当の挙動: NotifyHit 直後の
    // Tick で即リセットされる)。
    _combo_duration = (sec > 0.0f) ? sec : 0.0f;
}

f32 FScoreSystem::ComboMultiplier() const noexcept {
...
void FScoreSystem::SetMultiplierFn(MultiplierFn fn) noexcept {
...
void FScoreSystem::SetOnMilestoneCallback(MilestoneCallback cb, void* user) noexcept {
    _on_milestone      = cb;
    _on_milestone_user = user;
}

void FScoreSystem::CheckMilestones() noexcept {
