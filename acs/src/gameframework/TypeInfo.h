...
//   ・ヘッダオンリ。`FTypeInfo<T>` の特殊化に static const FFieldInfo[] を持ち、
...
struct FTypeInfoBase {
    ...
    const FFieldInfo* fields;        // FFieldInfo 配列 (field_count 要素)
...
// -----------------------------------------------------------------------------
// FReflectedFields<T> — 型 T の static FFieldInfo 配列ホルダ
//
// ACS_GAME_REFLECT macro が特殊化を生成する。
// default 特殊化は空 (fields == nullptr / count == 0)。
// -----------------------------------------------------------------------------
template<typename T>
struct FReflectedFields {
...
//   — FFieldInfo の構造体初期化子を返す。ACS_GAME_REFLECT の引数として使う。
// =============================================================================
...
//   — 型 T を反射する。`FReflectedFields<T>` と `FTypeInfo<T>` の特殊化を生成。
...
#define ACS_GAME_REFLECT(T, ...)                                                 \
    namespace acs::game {                                                        \
    template<>                                                                   \
    struct FReflectedFields<T> {                                                  \
        static const ::acs::game::FFieldInfo* Fields() noexcept {                 \
            static const ::acs::game::FFieldInfo s_fields[] = { __VA_ARGS__ };    \
            return s_fields;                                                     \
        }                                                                        \
    ...
    struct FTypeInfo<T> {                                                         \
        static const ::acs::game::FTypeInfoBase& Get() noexcept {                 \
            static const ::acs::game::FTypeInfoBase s_info {                      \
                ...
                /* size        */ sizeof(T),                                     \
                /* alignment   */ alignof(T),                                    \
                /* fields      */ FReflectedFields<T>::Fields(),                  \
                /* field_count */ FReflectedFields<T>::Count(),                   \
