...
// に委譲、Exists/Delete は acs::FFileSystem に委譲)。
//
// 設計の流れ:
...
//   Exists → FFileSystem::Exists
//   Delete → CheckInitialized → FFileSystem::Exists (べき等) → FFileSystem::Delete
//
...
//   3. FFileSystem::Rename(tmp_path, file_path)
// に書き換える (Phase 2 候補)。
...
// -----------------------------------------------------------------------------
// SaveSlot_Exists — FFileSystem::Exists 委譲
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// SaveSlot_Delete — FFileSystem::Delete 委譲 (べき等)
// -----------------------------------------------------------------------------
...
TResult<void> SaveSlot_Delete(const wchar_t* file_path) noexcept {
    ...
    return FFileSystem::Delete(file_path);
}
