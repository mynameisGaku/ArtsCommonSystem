...
u8 MatchGrid::PickColorAvoidingMatch(u32 x, u32 y, Random& rng) const noexcept {
    ...
    const u8 left2 = (x >= 2u) ? _cells[Idx(x - 2u, y)].color : 0u;
    const u8 up1   = (y >= 1u) ? _cells[Idx(x, y - 1u)].color : 0u;
    const u8 up2   = (y >= 2u) ? _cells[Idx(x, y - 2u)].color : 0u;

    const bool ban_horiz = (left1 != 0u && left1 == left2);
    const bool ban_vert  = (up1   != 0u && up1   == up2);

    // 通常の rolling: 1..color_count から 1 個。被ったら別色にずらす。
    u8 c = static_cast<u8>(1u + (rng.NextU32() % _color_count));
    // 最悪でも color_count 個試せば被らない色が見つかる (color_count >= 2 のとき)。
    // color_count == 1 では何も避けられないのでそのまま返す。
    if (_color_count >= 2u) {
        for (u32 guard = 0; guard < _color_count; ++guard) {
            ...
            c = static_cast<u8>(1u + (c % _color_count));
        }
    }
    return c;
}
...
void MatchGrid::ClearOne(u32 x, u32 y, TArray<u8>& visited) noexcept {
    ...
    ++_total_cleared;
    if (_on_clear != nullptr) {
        _on_clear(_on_clear_user, x, y, color, sp);
    }
...
void MatchGrid::ApplySpecialEffect(u32 x, u32 y, u8 color, ESpecialKind sp,
    ...
    case ESpecialKind::Bomb: {
        ...
        const u32 x1 = (x + 1u < _width)  ? (x + 1u) : (_width  - 1u);
        const u32 y1 = (y + 1u < _height) ? (y + 1u) : (_height - 1u);
...
u32 MatchGrid::ResolveAllMatches() noexcept {
    ...
    const u32 safety_limit = (_width * _height) + 8u;
    ...
    return _total_cleared - cleared_before;
}
