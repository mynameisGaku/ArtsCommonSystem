...
{
    /** 操作軸の宣言順に並べた検証対象。 */
    const FActionId actions[]{move_forward_action, move_right_action, move_up_action, look_yaw_action, look_pitch_action};
...
/** 明示入力状態の5actionを評価し、成功時だけcamera入力へ反映する。 */
bool FOrbitCameraInputActionSet3D::TryEvaluate(const FInputMap& input_map, const IInputStateView& input, COrbitCameraController3D::FOrbitCameraInput3D& output) const noexcept
{
    ...
    if (!std::isfinite(candidate.move_forward) || !std::isfinite(candidate.move_right) || !std::isfinite(candidate.move_up) || !std::isfinite(candidate.look_yaw) || !std::isfinite(candidate.look_pitch)) return false;
    output = candidate;
    return true;
}
