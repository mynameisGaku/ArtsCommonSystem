...
//     FAchievementProgress と同じ Def/FState 分離設計を踏襲。
...
private:
    // claim 状態。FTier と並行 TArray で 1:1 対応。
    struct FClaimState {
        u32  tier_index      = 0;     // FTier::tier_index と一致 (検索冗長化)
    ...
    // FTier 定義 + claim 状態 (同 index で 1:1 対応)。
    TArray<FTier>       _tiers;
    TArray<FClaimState> _claims;
};
