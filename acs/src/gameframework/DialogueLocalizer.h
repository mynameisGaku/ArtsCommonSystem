...
// GameFramework 完成度システム — DialogueLocalizer (Dialogue × Localization 橋渡し)
//
// DialogueSystem は本文 (`DialogueLine::text`) を const char* で参照のみ持つ。
// 多言語タイトルでは「同じシナリオ行を locale ごとに別文字列で出したい」
// = ロケール切替で本文 / 選択肢の表示を差し替えたい、という要件が必ず出る。
//
// LocalizationDirector は (locale, key) → 翻訳済み const char* の単純な辞書で、
// 一方 DialogueSystem は (line_index, speaker, text, cps) の state holder。
// この二者を直接結合すると DialogueSystem に locale の概念が漏れて
// 設計が崩れる (= DialogueSystem の単一責務原則違反)。
//
// DialogueLocalizer は両者を**疎結合**で繋ぐ橋渡し helper:
...
//       // → caller 側で DialogueSystem::AddLine 等に流す
//   }, nullptr);
//
// 範囲外 (将来 phase で):
//   ・format 引数展開 ("{0} HP" の {0} 置換) → LocalizationDirector 側 Phase 2 範疇
...
// LocalizationDirector から取得する。speaker_id も翻訳キー扱い。
...
public:
    ...
    // speaker / text とも常に非 nullptr (LocalizationDirector::Get の契約により
    ...
    DialogueLocalizer()  noexcept = default;
    ~DialogueLocalizer() noexcept = default;

    DialogueLocalizer(const DialogueLocalizer&)            = delete;
    DialogueLocalizer& operator=(const DialogueLocalizer&) = delete;
    DialogueLocalizer(DialogueLocalizer&&)                 = delete;
    DialogueLocalizer& operator=(DialogueLocalizer&&)      = delete;
    ...
    // (DialogueSystem::AddChoices と同契約)
    ...
    // 所有しないので寿命は呼び出し側保証。
    void SetLocalizer(LocalizationDirector* loc) noexcept;
