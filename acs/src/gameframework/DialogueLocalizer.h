...
// GameFramework 完成度システム — FDialogueLocalizer (Dialogue × FLocalization 橋渡し)
//
...
struct FLocalizedDialogueLine {
    ...
    f32         type_speed_cps = 30.0f;    // タイプ速度 (FDialogueLine と同義、<=0 で瞬時)
};

// 1 つの選択肢メタ。本文 (表示テキスト) は翻訳 key 経由。
// next_line_index が範囲外 (>= LineCount) なら「ダイアログ終了」を表現する
// (FDialogueChoice::next_line_index と同契約)。
...
private:
    ...
    // (FDialogueSystem::FChoicesAt と同じ形)
    struct FChoicesAt {
        u32 line_index   = 0;
        u32 choice_start = 0;
        u32 choice_count = 0;
    ...
    TArray<FLocalizedDialogueLine>   _lines;
    TArray<FChoicesAt>               _choices_at;     // line_index 昇順想定 (線形検索)
    TArray<FLocalizedDialogueChoice> _all_choices;    // 全 choice をフラットに保持
