...
public:
     ...
     * 円盤は各視線の地球遮蔽と透過を適用するが、輝度の尺度は旧方式を保持している。
     * 円盤の光量校正・画素面積の光量保存・HDR保存前の露出整合は未対応。
     * 半径0は円盤だけを非表示にし、太陽による散乱光は維持する。
...
struct FVolumetricCloudLighting {
     ...
     * 層内高さから求めた周囲散乱源の確率をそのまま使う。高さは疎な輪郭の判定だけに使い、
     * 密な雲底を一律に暗くしない。一次散乱は現在の密度標本で既に制限されるため、
     * この値は二次以降の散乱だけへ適用する。
     */
