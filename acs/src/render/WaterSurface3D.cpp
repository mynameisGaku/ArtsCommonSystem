...
cbuffer WaterObject : register(b1) {
    ...
    float4 adaptive_lod;        // xy=カメラ焦点、zw=軸ごとの指数密度率
};
...
float WarpAdaptiveCoordinate(float coordinate, float focus, float rate) {
    ...
    float warped_distance =
        (exp(rate * normalized_distance) - 1.0) / denominator;
    return focus + (delta < 0.0 ? -1.0 : 1.0)
         * side_extent * warped_distance;
}

void EvaluateAmbientWaves(float2 p, float time,
                          out float height, out float2 gradient,
                          out float2 horizontal_offset) {
    float2 flow = normalize(flow_params.xy + float2(1e-5, 0.0));
    float direction_angles[16] = {
         0.00,  0.16, -0.24,  0.39,
        -0.55,  0.71, -0.88,  1.05,
        -1.21,  1.42, -1.60,  1.82,
        -2.05,  2.28, -2.52,  2.83
    ...
    float frequencies[16] = {
         ...
         0.58,  0.78,  1.05,  1.41,
         1.90,  2.55,  3.42,  4.59,
         6.16,  8.26, 11.10, 14.90
    };
    float amplitudes[16] = {
        0.170, 0.145, 0.120, 0.100,
        0.085, 0.072, 0.061, 0.052,
        0.044, 0.038, 0.032, 0.027,
...
void EvaluateAmbientWavesPixel(float2 p, float time,
                               out float height, out float2 gradient) {
    float2 flow = normalize(flow_params.xy + float2(1e-5, 0.0));
    float direction_angles[16] = {
         0.00,  0.16, -0.24,  0.39,
...
void EvaluateRipples(float2 p, out float height,
    ...
    for (int i = 0; i < kMaxRipples; ++i) {
        ...
        float breaking_energy = max(
            abs(submitted_amplitude) - amplitude_limit, 0.0)
            * envelope * 0.18;
...
VSOut VSMain(VSIn input) {
    ...
    if (adaptive_lod.z > 1e-4 || adaptive_lod.w > 1e-4) {
        local_position.x = WarpAdaptiveCoordinate(
            local_position.x, adaptive_lod.x, adaptive_lod.z);
        local_position.z = WarpAdaptiveCoordinate(
            local_position.z, adaptive_lod.y, adaptive_lod.w);
        authored_uv = float2(
            local_position.x + 0.5, 0.5 - local_position.z);
    }
...
float4 PSMain(VSOut input) : SV_TARGET {
    ...
    EvaluateAmbientWavesPixel(
        input.surface_position, camera_time.w,
        ambient_height, ambient_gradient);
    ...
    float3 geometric_surface_normal = normalize(
        input.world_normal
        - tangent * ambient_gradient.x
        - bitangent * ambient_gradient.y);
    ...
    float normal_variance = min(
        max(dot(ddx(normal), ddx(normal)),
            dot(ddy(normal), ddy(normal))),
        1.0);
    float surface_roughness = clamp(
        sqrt(base_roughness * base_roughness
             + normal_variance * 0.18),
        0.035, 0.72);
    ...
    float sun_disk_power = lerp(1400.0, 180.0,
                                saturate(surface_roughness * 2.2));
    ...
    float roughness = clamp(surface_roughness
...
FWaterSurfaceFrame BuildWaterSurfaceFrame(const FMat4& model) noexcept {
    ...
    const f64 transformed_tangent_length = std::hypot(
        static_cast<f64>(transformed_tangent.x),
        static_cast<f64>(transformed_tangent.y),
        static_cast<f64>(transformed_tangent.z));
    if (!IsFinite(transformed_tangent)
        || !std::isfinite(transformed_tangent_length)
        || transformed_tangent_length <= 1e-8) {
    ...
    const f32 bitangent_span =
        std::abs(Dot(result.bitangent, transformed_bitangent));
 ...
 */
f32 AdaptivePlaneWarpRate_Internal(
    f32 world_span, u32 cells,
    ...
    const f32 target_cell = std::max(
        0.0025f,
        std::min(shortest_macro_wavelength * 0.20f,
                 ripple_wavelength * 0.25f));
    /** 等間隔格子での一cell距離。 */
    ...
    return std::min(
        14.0f,
        logarithmic_ratio + std::log(logarithmic_ratio + 1.0f));
}
...
u32 AdaptivePlaneCellCount_Internal(const FGpuMesh& mesh) noexcept {
    ...
    /** 正方格子と仮定して求める一軸の分割数。 */
    const u32 cells = static_cast<u32>(
        std::sqrt(static_cast<f64>(quad_count)));
    if (cells < 2u
        || static_cast<u64>(cells) * cells != quad_count) {
        return 0u;
    }
    /** 一軸の頂点数。 */
 ...
 * @param params 最短波長を決める水面設定。
 * @param enabled 連続密度格子を使う場合はtrue。
 * @return xyに格子焦点、zwに軸ごとの指数密度率を持つ値。
 */
FVec4 BuildAdaptivePlaneLod_Internal(
    const FWaterSurfaceFrame& surface, FVec3 camera_position,
    const FWaterSurface3DParams& params, u32 cells) noexcept {
    if (cells < 2u) return FVec4{};
    /** 水面原点を基準にしたcamera位置。 */
    const FVec3 relative_camera = camera_position - surface.origin;
    /** 水面X軸上で端へ制限した焦点。 */
    const f32 focus_x = ClampFinite(
        Dot(relative_camera, surface.tangent) / surface.tangent_span,
        0.0f, -0.5f, 0.5f);
    /** 水面Z軸上で端へ制限した焦点。 */
    const f32 focus_z = ClampFinite(
        Dot(relative_camera, surface.bitangent) / surface.bitangent_span,
        0.0f, -0.5f, 0.5f);
    return FVec4{
        focus_x, focus_z,
        AdaptivePlaneWarpRate_Internal(
 ...
 */
bool TryBuildAdaptivePlaneMesh_Internal(
    u32 cells, FMeshAsset& output) noexcept {
    /** 一行あたりの頂点数。 */
    const usize row = static_cast<usize>(cells) + 1u;
    /** 格子全体の頂点数。 */
    const usize vertex_count = row * row;
    /** 全四角形を二つのtriangleへ分割したindex数。 */
    const usize index_count = static_cast<usize>(cells)
                            * static_cast<usize>(cells) * 6u;
    ...
    if (!vertices.TryResize(vertex_count)
        || !indices.TryResize(index_count)
        || !submeshes.TryResize(1u)) {
        return false;
    }
    ...
    for (u32 z = 0u; z < cells; ++z) {
        for (u32 x = 0u; x < cells; ++x) {
            /** 現在四角形の左上頂点。 */
            const u32 top_left = static_cast<u32>(
                static_cast<usize>(z) * row + x);
            /** 現在四角形の右上頂点。 */
 ...
 */
TResult<void> FWaterSurface3D::CreateAdaptivePlaneMesh(
    IRhiDevice& device, FGpuMesh& output, u32 cells) noexcept {
    if (cells < 2u || cells > 512u) {
        return ACS_ERR(
            Render, 716,
            "adaptive water plane cell count must be between 2 and 512");
    }
    /** 転送前の正規化XZ格子。 */
    FMeshAsset cpu_mesh;
    if (!TryBuildAdaptivePlaneMesh_Internal(cells, cpu_mesh)) {
        return ACS_ERR(
            Render, 717,
            "adaptive water plane mesh allocation failed");
    }
...
u32 FWaterSurface3D::AddWakeSegmentForSurface(
    ...
    if (sample_count > 1u) {
        ...
        /** 一つの波束が主に影響する進行方向の幅。 */
        const f64 packet_span = static_cast<f64>(
            m_Params.ripple_wavelength) * 2.20;
        overlap = std::max(
            1.0, packet_span / std::max(actual_spacing, 1e-5));
    }
    /** 重なりを除いて各sampleへ渡す強さ。 */
    const f32 sample_strength = static_cast<f32>(
        static_cast<f64>(strength) / overlap);
...
void FWaterSurface3D::DrawMesh(IRhiCommandList& command_list,
    ...
    DrawMesh_Internal(
        command_list, mesh, model, scene_color, scene_depth,
        screen_reflection, surface_id, hardware_depth_bound,
        authored_normal_map, authored_normal_strength, false);
}

/** 正規化格子をcamera近傍へ連続再配置する共通描画本体へ渡す。 */
void FWaterSurface3D::DrawAdaptivePlane(
    ...
    f32 authored_normal_strength) noexcept {
    DrawMesh_Internal(
        command_list, plane_mesh, model, scene_color, scene_depth,
        screen_reflection, surface_id, hardware_depth_bound,
        authored_normal_map, authored_normal_strength, true);
}

/** 通常meshと連続密度平面で共有する描画処理を実行する。 */
void FWaterSurface3D::DrawMesh_Internal(
    ...
    object.adaptive_lod = BuildAdaptivePlaneLod_Internal(
        surface, m_CameraPos, m_Params, adaptive_cells);
