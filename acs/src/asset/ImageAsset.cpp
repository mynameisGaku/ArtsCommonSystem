...
TResult<TRc<FAsset>> FImageAssetLoader::LoadFromBytes(FAssetId id, const TArray<byte>& bytes) noexcept {
    ...
    if (is_hdr) {
        ...
        if (!px) return ACS_ERR(FAsset, 100, "stbi_loadf_from_memory failed");
        ...
        return TResult<TRc<FAsset>>(OkInit, TRc<FAsset>(Move(a)));
    }
    ...
    stbi_uc* px = ::stbi_load_from_memory(
        reinterpret_cast<const stbi_uc*>(bytes.Data()),
        static_cast<int>(bytes.Size()), &w, &h, &channels, 4);
    if (!px) return ACS_ERR(FAsset, 101, "stbi_load_from_memory failed");
