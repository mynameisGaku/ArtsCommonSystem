...
/** 球メッシュの経度方向に許可する最大分割数。 */
inline constexpr u32 kMaximumSphereSegments = 1024u;

/** 球メッシュの緯度方向に許可する最大分割数。 */
inline constexpr u32 kMaximumSphereRings = 1024u;
 ...
 * @param output 成功時に生成結果を受け取る共有ポインタ。失敗時は変更しない。
 * @return 生成できた場合は true。上限超過、入力不正または確保失敗なら false。
 */
 ...
 * @return 生成結果。上限超過、入力不正または確保失敗時は空。
 */
