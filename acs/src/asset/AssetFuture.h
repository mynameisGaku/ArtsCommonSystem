...
struct FAsyncLoadState {
    FCompletionCounter counter{1};   // 1 → 0 で完了
    TRc<FAsset>         result;
    FErrorCode         error{};
...
public:
    ...
    explicit FAssetFuture(TRc<FAsyncLoadState> s) noexcept : _state(Move(s)) {}
    ...
    TResult<TRc<FAsset>> Get() noexcept {
        if (!_state.Get()) return ACS_ERR(FAsset, 10, "FAssetFuture: empty");
        FThreadPool::Wait(_state->counter);
        if (_state->has_error) return TResult<TRc<FAsset>>(_state->error);
        return TResult<TRc<FAsset>>(OkInit, _state->result);
    }

    // 別名: Wait() = Get() （直感的に使えるよう）
    TResult<TRc<FAsset>> Wait() noexcept { return Get(); }

private:
    TRc<FAsyncLoadState> _state;
};
