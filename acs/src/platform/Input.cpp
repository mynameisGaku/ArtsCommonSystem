...
struct FInputState {
    ...
    /** このフレームに押下開始を受け取ったキー。 */
    bool keys_pressed[(usize)EKey::_Count]{};

    /** このフレームに解放を受け取ったキー。 */
    bool keys_released[(usize)EKey::_Count]{};

    /** 現フレームのマウスボタン押下状態 (EMouseButton で添字)。 */
    bool mb_now[(usize)EMouseButton::_Count]{};

    /** このフレームに押下開始を受け取ったマウスボタン。 */
    bool mb_pressed[(usize)EMouseButton::_Count]{};

    /** このフレームに解放を受け取ったマウスボタン。 */
    bool mb_released[(usize)EMouseButton::_Count]{};
...
/** 配列位置が実在するキーを指すか調べる。 */
constexpr bool IsValidKeyIndex(usize key_index) noexcept
{
    return key_index > static_cast<usize>(EKey::Unknown) && key_index < static_cast<usize>(EKey::_Count);
}
...
{
    // キーボード / マウス: 前フレームの押下・解放エッジを消去
    for (usize i = 0; i < (usize)EKey::_Count; ++i) {
        g_input.keys_pressed[i] = false;
        g_input.keys_released[i] = false;
    }
    for (usize i = 0; i < (usize)EMouseButton::_Count; ++i) {
        g_input.mb_pressed[i] = false;
        g_input.mb_released[i] = false;
    }
...
void FInput::OnEvent(const FEvent& e) noexcept
{
    ...
    case EEventType::KeyPressed: {
        const usize key_index = static_cast<usize>(e.key.key);
        if (IsValidKeyIndex(key_index)) {
            if (!g_input.keys_now[key_index]) g_input.keys_pressed[key_index] = true;
            g_input.keys_now[key_index] = true;
        }
        break;
    }
    case EEventType::KeyRepeat: {
        const usize key_index = static_cast<usize>(e.key.key);
        if (IsValidKeyIndex(key_index)) g_input.keys_now[key_index] = true;
        break;
    }
    case EEventType::KeyReleased: {
        const usize key_index = static_cast<usize>(e.key.key);
        if (IsValidKeyIndex(key_index)) {
            if (g_input.keys_now[key_index]) g_input.keys_released[key_index] = true;
            g_input.keys_now[key_index] = false;
        }
        break;
    }
    case EEventType::MouseButtonPressed: {
        const usize button_index = static_cast<usize>(e.mouse_button.button);
        if (button_index < static_cast<usize>(EMouseButton::_Count)) {
            if (!g_input.mb_now[button_index]) g_input.mb_pressed[button_index] = true;
            g_input.mb_now[button_index] = true;
        }
        break;
    }
    case EEventType::MouseButtonReleased: {
        const usize button_index = static_cast<usize>(e.mouse_button.button);
        if (button_index < static_cast<usize>(EMouseButton::_Count)) {
            if (g_input.mb_now[button_index]) g_input.mb_released[button_index] = true;
            g_input.mb_now[button_index] = false;
        }
        break;
    }
    ...
    case EEventType::CharInput: {
        ...
        if (cp >= 0xD800 && cp <= 0xDBFF) { // UTF-16 上位サロゲート
            ...
            if (g_input.hi_surrogate) {
                cp = 0x10000u + ((g_input.hi_surrogate - 0xD800u) << 10) + (cp - 0xDC00u);
    ...
    case EEventType::WindowLostFocus:
        for (usize key_index = 0; key_index < static_cast<usize>(EKey::_Count); ++key_index) {
            if (g_input.keys_now[key_index]) g_input.keys_released[key_index] = true;
            g_input.keys_now[key_index] = false;
        }
...
bool FInput::IsKeyDown(EKey k) noexcept
{
    const usize key_index = static_cast<usize>(k);
    return IsValidKeyIndex(key_index) && g_input.keys_now[key_index];
}
bool FInput::IsKeyPressed(EKey k) noexcept
{
    const usize key_index = static_cast<usize>(k);
    return IsValidKeyIndex(key_index) && g_input.keys_pressed[key_index];
}
bool FInput::IsKeyReleased(EKey k) noexcept
{
    const usize key_index = static_cast<usize>(k);
    return IsValidKeyIndex(key_index) && g_input.keys_released[key_index];
}

bool FInput::IsMouseButtonDown(EMouseButton b) noexcept
{
    const usize button_index = static_cast<usize>(b);
    return button_index < static_cast<usize>(EMouseButton::_Count) && g_input.mb_now[button_index];
}
bool FInput::IsMouseButtonPressed(EMouseButton b) noexcept
{
    const usize button_index = static_cast<usize>(b);
    return button_index < static_cast<usize>(EMouseButton::_Count) && g_input.mb_pressed[button_index];
}
bool FInput::IsMouseButtonReleased(EMouseButton b) noexcept
{
    const usize button_index = static_cast<usize>(b);
    return button_index < static_cast<usize>(EMouseButton::_Count) && g_input.mb_released[button_index];
}

FVec2 FInput::MousePos() noexcept
{
    return FVec2(g_input.mouse_x, g_input.mouse_y);
}
FVec2 FInput::MouseDelta() noexcept
{
    return FVec2(g_input.mouse_x - g_input.mouse_x_prev, g_input.mouse_y - g_input.mouse_y_prev);
}
f32 FInput::MouseWheel() noexcept
{
    return g_input.wheel_frame;
}

const char* FInput::TextInput() noexcept
{
    return g_input.text_utf8;
}
...
f32 FInput::GamepadAxisValue(u32 idx, EGamepadAxis axis) noexcept
{
    ...
    case EGamepadAxis::LeftX:
        return NormalizeStick(g.sThumbLX, XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE);
    case EGamepadAxis::LeftY:
        return NormalizeStick(g.sThumbLY, XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE);
    case EGamepadAxis::RightX:
        return NormalizeStick(g.sThumbRX, XINPUT_GAMEPAD_RIGHT_THUMB_DEADZONE);
    case EGamepadAxis::RightY:
        return NormalizeStick(g.sThumbRY, XINPUT_GAMEPAD_RIGHT_THUMB_DEADZONE);
    case EGamepadAxis::LeftTrigger:
        return NormalizeTrigger(g.bLeftTrigger);
    case EGamepadAxis::RightTrigger:
        return NormalizeTrigger(g.bRightTrigger);
    default:
        return 0.0f;
    }
}
