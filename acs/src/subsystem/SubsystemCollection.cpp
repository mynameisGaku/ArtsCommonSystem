                                         ...
                                         FSubsystemOwner owner) noexcept
{
...

    {
        ...
        for (usize Index = 0u; Index < m_Subsystems.Num(); ++Index) {
            m_Subsystems[Index].instance->SetOwnerDescriptor_Internal(owner);
        }
        for (usize Index = 0u; Index < m_Subsystems.Num(); ++Index) {
...
void CSubsystemCollection::ClearOwners() noexcept
{
    for (usize Index = 0u; Index < m_Subsystems.Num(); ++Index) {
        if (m_Subsystems[Index].instance) {
            m_Subsystems[Index].instance->SetOwnerDescriptor_Internal(FSubsystemOwner{});
        }
    }
}
...
void CSubsystemCollection::TeardownVisibleEntries() noexcept
{
    ...
    while (m_VisibleCount > 0u) {
        ...
        if (Entry.instance) {
            ...
            Entry.instance->SetOwnerDescriptor_Internal(FSubsystemOwner{});
        }
        --m_VisibleCount;
    }
    ClearOwners();
