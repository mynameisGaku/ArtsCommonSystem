...
protected:
    ...
    void EnsureSparseCapacity(u32 entity_index) noexcept {
        if (entity_index >= m_Sparse.Size()) {
            usize old = m_Sparse.Size();
            m_Sparse.Resize(entity_index + 1);
            for (usize i = old; i <= entity_index; ++i) m_Sparse[i] = kInvalid;
        }
    }

    TArray<u32> m_Sparse;   // entity_index -> dense_index
    TArray<u32> m_Dense;    // dense_index -> entity_index
};
...
public:
    ...
    void Add(u32 entity_index, T value) noexcept {
        ...
        u32 idx = m_Sparse[entity_index];
        if (idx != kInvalid) {
            m_Data[idx] = Move(value);  // 上書き
            return;
        }
        u32 new_idx = static_cast<u32>(m_Dense.Size());
    ...
    void Remove(u32 entity_index) noexcept {
        ...
        u32 last = static_cast<u32>(m_Dense.Size() - 1);
        if (idx != last) {
            // 末尾要素を idx 位置に移動
            u32 last_entity = m_Dense[last];
