...
//   HelloShadowsApp.{h,cpp}   - CApplication 派生クラス (resource 所有 + 入力)
//
// 動作:
//   ・ライト視点の depth-only パスでシャドウマップを焼く
//   ・主パスでメッシュを CStandardShader + シャドウサンプリングで描画
//   ・複数のキューブ + 球が地面に影を落とす
//
// 操作:
//   WASD  : 移動
...
//
// 学習ポイント:
//   ・CShadowMap::SetDirectionalLight でライト VP を計算
//   ・cl->BeginShadowPass / EndShadowPass の使い方
//   ・CStandardShader::SetShadowMap で主パスにシャドウを通知
//   ・PCSS (Fernando 2005) で物理的に正しいソフトシャドウ
//
// ACS_DEFINE_MAIN は HelloShadowsApp を main エントリに登録 (CApplication 派生 →
// `int WINAPI WinMain` / `int main` 両方の通常 main を裏で生成)。
#include "app/EntryPoint.h"
#include "HelloShadowsApp.h"

ACS_DEFINE_MAIN(helloshadows::CHelloShadowsApp)
