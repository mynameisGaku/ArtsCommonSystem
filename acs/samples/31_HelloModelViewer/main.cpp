...
//   ModelViewerApp.{h,cpp}        - CGame 派生クラス (ImGui lifecycle ラッパ)
...
//
// 動作:
//   ・editor_core の CEditorWorkspace / CAssetBrowser / CEditorTheme と、modelview/
//     配下 4 panel (AModelViewerPanel / AModelInspectorPanel / AModelMaterialPanel /
//     AModelAnimationPanel) を 1 個の Workspace に集約。
//   ・サンプル 17_HelloMesh と同等の "頂点+色 cube" を回転表示する 3D viewport を
//     ImGui の外側 (= フレームバッファ直書き) に同時描画。viewport カメラは
//     `AModelViewerPanel::Camera()` (= editor_core::CEditorCamera) の view/proj を
...
//     Layout は editor_core::CEditorWorkspace::SaveLayout/LoadLayout を呼ぶ。
//     Theme は CEditorTheme::DrawThemeSettingsUI() を toggle で開閉。
//   ・Esc で終了。
//
...
//   ・animation frame 適用 — AModelAnimationPanel::Tick は時間だけ進める。
//   ・gizmo 操作 (translate/rotate/scale)。
//   ・複数モデルの同時表示 (現状 1 cube 固定)。
#include "ModelViewerApp.h"

ACS_GAME_MAIN(hellomv::CModelViewerApp)
