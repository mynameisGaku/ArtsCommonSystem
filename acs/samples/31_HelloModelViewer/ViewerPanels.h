...
//   この変更点を Scene 本体から物理的に分離することで、Scene は「3D + UI」の
...
public:
    ...
    // EditorWorkspace::RegisterPanel は内部で `panel->OnInit(*this)` を呼ぶので、
    // 個別に panel.OnInit を呼ぶと二重初期化になる (= 呼ばない)。
    void Init(const wchar_t* asset_root) noexcept;

    // 逆順 shutdown。EditorWorkspace::Shutdown は登録済み panel に OnShutdown を
    // 1 度ずつ呼んでから list を Clear するので、個別 UnregisterPanel は不要。
    // EditorTheme::Shutdown は API として存在しない (Dtor で十分)。
    ...
    acs::game::editor_core::EditorTheme&       Theme()     noexcept { return _theme; }
