...
void CViewerPanels::Init(const wchar_t* asset_root) noexcept {
    ...
    // CAssetBrowser: 引数で受けた asset_root を起点に列挙。
    ...
    // CAssetBrowser は AEditorPanel 派生ではないので Workspace 登録対象外。
    // 1 フレームの DrawUI() は Draw() 内で直接呼ぶ。AEditorPanel ラッパが追加
    // されたら m_Workspace.RegisterPanel(&m_AssetBrowserPanel) に置き換える。
}

void CViewerPanels::Shutdown() noexcept {
    m_Workspace.Shutdown();
    m_AssetBrowser.Shutdown();
    // CEditorTheme は明示 Shutdown が無い (Dtor で十分)。
}

void CViewerPanels::Draw(f32 dt) noexcept {
    ...
    // CAssetBrowser を直接描画 (Workspace 登録対象外)。
