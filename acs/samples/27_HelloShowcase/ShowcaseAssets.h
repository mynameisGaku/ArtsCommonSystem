...
//
// CPostProcess / CSky / IBL / PBR / SSR / SSAO / CHiZ / Motion / Refraction / CBlit /
// CSpriteBatch / FFont / 球 + 床メッシュ / 屈折 background RT を 1 つの struct に
// まとめる。pass 系 helper (PbrPass / RefractionPass / SsrPass / ...) はこの
// FAssets を const & または & で受け取って動く free function 群。
//
// CShowcaseApp は FAssets を 1 メンバとして保持し、OnStart で Initialize() を
...
struct FAssets {
    acs::CPostProcess        post;
    acs::CImageBasedLighting ibl;
    acs::CSky                sky;
    acs::CPbrShader          pbr;
    acs::CSsr                ssr;
    acs::CSsao               ssao;
    acs::CHiZ                hiz;            // SSR の skip-ahead 用 coarse min-depth
    acs::CMotionVector       motion;
    acs::CRefractionShader   refr;
    acs::CBlit               blit;
...
// CSky を Day preset に、sphere/plane メッシュを生成し、SSR/SSAO/CHiZ/Motion/
// Refraction/CBlit/CSpriteBatch を順次セットアップする。
//
//   color_fmt / depth_fmt : swapchain の format (CPostProcess / CSky / Refraction に渡す)
//
