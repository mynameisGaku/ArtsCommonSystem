...
void CHelloLocalizationApp::OnStart() noexcept
{
    ...
    ACS_SAMPLE_INIT(m_Loc.LoadFallbackBytes(reinterpret_cast<const u8*>(kLangEn), std::strlen(kLangEn)));
    SwitchTo(0); // 起動時は日本語
...
void CHelloLocalizationApp::OnRender() noexcept
{
    ...
    m_Batch.DrawRect(0, 0, static_cast<f32>(sw), static_cast<f32>(sh), FVec4{0.10f, 0.13f, 0.20f, 1});
    m_Batch.DrawRect(40, 40, static_cast<f32>(sw - 80), static_cast<f32>(sh - 80), FVec4{0.16f, 0.20f, 0.30f, 1});

    if (m_FontBig.AtlasTexture()) {
        m_Batch.DrawString(m_FontBig, m_Loc.Tr("title"), 80, 80, FVec4{1, 0.85f, 0.4f, 1});
    }
    if (m_FontSmall.AtlasTexture()) {
        m_Batch.DrawString(m_FontSmall, m_Loc.Tr("greeting"), 80, 150, FVec4{0.95f, 0.95f, 1, 1});

        const char* lang_name = (m_Lang == 0) ? "(日本語 / Japanese)" : (m_Lang == 1) ? "(English)" : "(Français)";
        m_Batch.DrawString(m_FontSmall, lang_name, 80, 180, FVec4{0.6f, 0.7f, 0.85f, 1});
        ...
        m_Batch.DrawString(m_FontSmall, m_Loc.Tr("note"), 80, static_cast<f32>(sh - 130), FVec4{0.7f, 0.7f, 0.8f, 1});
        m_Batch.DrawString(m_FontSmall, m_Loc.Tr("hint"), 80, static_cast<f32>(sh - 90), FVec4{0.6f, 0.7f, 0.95f, 1});
    }
    m_Batch.End();
}
