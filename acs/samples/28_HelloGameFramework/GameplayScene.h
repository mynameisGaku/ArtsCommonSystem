...
private:
    ...
    // ANode ツリー (root → wheel → spoke[0/1])。
    acs::game::ANode        m_Root;
    ARotatingNode*            m_Wheel    = nullptr;
    ARotatingNode*            m_Spoke[2] = {nullptr, nullptr};
    // composition 版 (プレーン ANode + ARotateComponent attach)。
    acs::game::ANode*       m_Rotator  = nullptr;
    // spoke[0/1] の world pos に毎フレーム追従させる CollisionWorld 上の円。
    acs::game::FShapeId       m_SpokeShape[2];
    // 重力で落下する ball (弱参照、ツリーの所有) と静的 ground。
    acs::game::ANode*       m_Ball         = nullptr;
