...
// HelloFullGame — FPlayer モジュール。
// 自機 ANode + FHealthId + 円形コリジョン + 入力ハンドリング + 射撃クールダウン。
//
// 設計メモ: FPlayer は FGameplayScene に所有させ、scene が tick / draw を呼ぶ。
// 当たり判定形状は Services().Physics() に登録するため、FGameplayScene の Services()
// を介して触る。
#pragma once
...
// FPlayer モジュール。Node + Health + Shape + 発射 CD をひとまとめにする。
class FPlayer {
public:
    // OnEnter で呼ぶ。root に子として player node を生やし、Health/Shape も登録する。
    void Init(FGameplayScene& scene, acs::game::ANode& root,
    ...
    acs::FVec2 UpdateMovement(FGameplayScene& scene, acs::f32 dt) noexcept;

    // 発射ボタンが押されていれば cooldown を見て撃つ。
    void UpdateFire(FGameplayScene& scene, acs::f32 dt) noexcept;

    // 敵との接触ダメージを 1 体ぶん適用しようとする。
    // 戻り値: true なら致死 (シーンが GameOver 遷移する)。
    bool TryTakeContactDamage(FGameplayScene& scene,
