...
FScriptValue Number(f64 n) noexcept {
    FScriptValue v{};
...
void HostAdd(IScriptVm&, FScriptCallFrame& frame, void*) noexcept {
...
bool CScriptDemoApp::DemoCallFunction(IScriptVm& vm) noexcept {
    ...
    FScriptValue args[2] = { Number(2.0), Number(3.0) };
    FScriptValue ret{};
    const bool ok_add = vm.CallFunction("add", args, 2, &ret).IsOk()
                        && ret.kind == EScriptValueKind::Number;

    FScriptValue fib_arg[1] = { Number(10.0) };
    FScriptValue fib_ret{};
