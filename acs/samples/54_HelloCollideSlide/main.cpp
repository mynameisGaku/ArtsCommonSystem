...
int main() {
    ...
    // ---- (0) ResolveCircle 直接: 床にめり込んだ円は上に押し出される ----
    {
        ...
        w.AddAabb(Aabb2{ FVec2{0, 10}, FVec2{100, 10} });         // 上面 y=0 (床は y∈[0,20]=画面下)
    ...
    // ---- (1) 円 body: 重力で落下 → 床で静止しつつ右へスライド ----
    {
        ...
        FNode2D node;
        node.Local().position = FVec2{ 0, -5 };                   // 床より上 = 小さい Y
    ...
    {
        FCollisionWorld2D w; w.Init(8.0f);
        w.AddAabb(Aabb2{ FVec2{0, 10}, FVec2{100, 10} });        // 床は y∈[0,20]=画面下
        FNode2D node;
        node.Local().position = FVec2{ 0, -5 };                  // 床より上 = 小さい Y
        auto& body = node.AddComponent<FPhysicsBody2D>(w);
