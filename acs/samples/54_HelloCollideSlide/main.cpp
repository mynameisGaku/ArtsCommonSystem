...
// HelloCollideSlide — FPhysicsBody2D の collide-and-slide を検証するコンソール。
...
int main() {
    ...
    // ---- (0) ResolveCircle 直接: 床にめり込んだ円は上に押し出される ----
    {
        ...
        const FVec2 push = w.ResolveCircle(Circle{ FVec2{0, 0.5f}, 1.0f });  // 中心 y=0.5 → 0.5 めり込み
    ...
    // ---- (1) 円 body: 重力で落下 → 床で静止しつつ右へスライド ----
    {
        ...
        const FVec2 p = node.Local().position;
        const bool rest  = Near(p.y, 1.0f, 0.25f);                // 床上 (中心 = top + r)
        const bool slid  = p.x > 3.0f;                            // 水平に滑った
        const bool nopen = p.y > 0.7f;                            // 貫通なし
    ...
    {
        FCollisionWorld2D w; w.Init(8.0f);
        w.AddAabb(Aabb2{ FVec2{0, -10}, FVec2{100, 10} });
        FNode2D node;
        node.Local().position = FVec2{ 0, 5 };
        auto& body = node.AddComponent<FPhysicsBody2D>(w);
        ...
        for (int i = 0; i < 90; ++i) node.UpdateTree(1.0f / 60.0f);
        const FVec2 p = node.Local().position;
        const bool rest  = Near(p.y, 1.0f, 0.3f);                 // box 下端 = 0 → 中心 y=1
