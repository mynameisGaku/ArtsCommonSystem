...
void DrawFloor(HelloIblApp& app) noexcept {
    ...
    app._pbr.SetExtParams(0.0f, 0.08f, 0.0f, Vec3{1, 0, 0});
    app._pbr.SetEmissive(Vec3{0, 0, 0}, 0.0f);
...
void DrawSphereGrid(HelloIblApp& app) noexcept {
    ...
    for (u32 y = 0; y < kGrid; ++y) {
        ...
        for (u32 x = 0; x < kGrid; ++x) {
            ...
            app._pbr.SetSheen(Vec3{0.95f, 0.90f, 0.78f}, cloth_row ? frac : 0.0f, 0.4f);
            ...
            app._pbr.SetSubsurface(Vec3{0.90f, 0.30f, 0.18f}, sss_row ? frac : 0.0f);
    ...
    // 後続描画 (オーブ等) のため reset
    app._pbr.SetSheen(Vec3{0, 0, 0}, 0.0f, 0.3f);
    app._pbr.SetIridescence(0.0f, 400.0f, 1.4f);
    app._pbr.SetSubsurface(Vec3{0, 0, 0}, 0.0f);
}
