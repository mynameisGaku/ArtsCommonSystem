...
//   1) BuildEquirectFromSky: 現在の FSky procedural を CPU で評価して equirect float bitmap
//      に焼く。ProcSky (FIbl.cpp の HLSL) と同じ式を C++ で再現したもの。SH9 計算用。
...
namespace acs { class FSky; }

namespace helloibl {

// 現 FSky preset (Day / Sunset / Night) の procedural を CPU で評価して
// equirect HDR float buffer に焼く。SH9 計算で使う。
void BuildEquirectFromSky(const acs::FSky& sky, acs::TArray<acs::f32>& buf) noexcept;

// 4 灯 Studio HDR equirect を合成 (Studio preset、FSky 評価は不要)。
