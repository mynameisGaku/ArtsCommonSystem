...
void HelloIblApp::OnStart() noexcept {
    ...
    // 実装は IblLightmapBaker に分離。Sphere-Ray analytical hit で覆い、各
    // plane texel で hemisphere sampling して visibility を求める。
    {
...
bool HelloIblApp::OnCustomFrame() noexcept {
    ...
    if (_need_studio_hdr) {
        dev->WaitIdle();
        BuildStudioHdrEquirect(_equirect_rgba);
