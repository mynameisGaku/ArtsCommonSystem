...
void UpdateExposureControls(HelloIblApp& app, f32 dt) noexcept {
    if (FInput::IsKeyPressed(EKey::U)) app._use_auto_exposure = !app._use_auto_exposure;

    if (app._use_auto_exposure) {
        // 露出は GPU が実測輝度から算出。Q/E は目標平均輝度 (key) を動かして
        // 全体の明暗を補正する (EV compensation 相当)。
        if (FInput::IsKeyDown(EKey::E)) app._auto_key += dt * 0.3f;
        if (FInput::IsKeyDown(EKey::Q)) app._auto_key -= dt * 0.3f;
        ...
        if (FInput::IsKeyDown(EKey::E)) app._exposure_target += dt * 0.5f;
        if (FInput::IsKeyDown(EKey::Q)) app._exposure_target -= dt * 0.5f;
