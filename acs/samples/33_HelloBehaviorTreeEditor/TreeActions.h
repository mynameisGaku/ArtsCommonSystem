...
//
// Action Fn は `acs::game::FBtAction::Fn`
// (= EBtStatus(*)(void*, f32) noexcept) のシグネチャに合わせる。実 BT の
// `FBtAction` から呼ばれ、blackboard を `BtEditorBb*` に restore して
// panel.SetNodeStatus に status を push する。
//
// Tree 構築側 (TreeBuilder) と FScene 側 (BtEditorScene) の両方から関数
...
// Action Fn 群 — 全て FBtAction::Fn シグネチャ。
...
void StepCallbackFn(void* user, acs::game::FBehaviorTree* tree, acs::f32 dt) noexcept;
