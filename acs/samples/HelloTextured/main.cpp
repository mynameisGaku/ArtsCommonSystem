...
public:
    void OnStart() noexcept override {
        ...
        if (auto r = CreateRhiBuffer(*dev, vb); r.IsErr()) { Quit(); return; }
        ...
        if (auto r = CreateRhiBuffer(*dev, ib); r.IsErr()) { Quit(); return; }
        else _ib = Move(r.Value());

        BufferDesc cb_d{};
        cb_d.size = 256; cb_d.usage = BufferUsage::Uniform; cb_d.cpu_writable = true;
        if (auto r = CreateRhiBuffer(*dev, cb_d); r.IsErr()) { Quit(); return; }
