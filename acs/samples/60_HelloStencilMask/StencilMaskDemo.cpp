...
public:
    ...
    void OnTick(f32 dt) noexcept override {
        ...
        FVec2 c;
        if (m_FollowMouse) {
            c = ScreenToWorld(FInput::MousePos());   // owner は原点なので world = 相対
        } else {
        ...
        // [Tab] 円 ⇔ 矩形。
        const bool tab = FInput::IsKeyPressed(EKey::Tab);
        if (tab && !m_TabPrev) m_RectShape = !m_RectShape;
        m_TabPrev = tab;

        // ホイール代わりに上下キーで窓サイズ調整。
        if (FInput::IsKeyDown(EKey::Up))   m_Radius += 0.04f;
        if (FInput::IsKeyDown(EKey::Down)) m_Radius -= 0.04f;
