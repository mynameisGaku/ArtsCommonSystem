...
使わず、関数を呼ぶだけで 2D ゲームが書けます。`01_` 以降は「`CApplication` を
...
シェーダ（HLSL）を一切書きません。`CSpriteBatch` などの高レベル API だけで
2D ゲームに必要な要素を学びます。

| # | サンプル | 学べること |
|---|---|---|
| 01 | `01_HelloWindow` | ウィンドウ・入力・`CApplication` のライフサイクル（最初の 1 本、73 行） |
| 02 | `02_HelloSprite` | 2D スプライト・矩形・α ブレンド（`CSpriteBatch`） |
| 03 | `03_HelloText` | TTF フォント・UTF-8・漢字テキスト描画 |
| 04 | `04_HelloECS` | ECS（`CWorld` / `TQueryView`）・並列イテレーション・`CMessageBroker`・`CTimerManager` |
...
### (2) 3D 入門 — CStandardShader　`10`〜`15`
ここも HLSL を書きません。`CStandardShader` が「光が当たった 3D」を
シェーダ記述なしで描いてくれます。

| # | サンプル | 学べること |
|---|---|---|
| 10 | `10_HelloModel` | `CStandardShader` で 3D ライティング描画・手続きプリミティブ・非同期ロード（最初の 3D） |
...
**ここからは発展編**です。`CStandardShader` を使わず、シェーダ（HLSL）と
...
| # | サンプル | 学べること |
|---|---|---|
| 19 | `19_HelloUI` | ACS 純正 UI フレームワーク（AWidget + TObservable バインディング） |
| 20 | `20_HelloMVVM` | MVVM データバインディング（TObservable / Binder / FCommand） |
| 21 | `21_HelloImGui` | Dear ImGui 統合（デバッグ UI） |
| 22 | `22_HelloNet` | TCP echo（`CApplication` を使わない素の例） |
...
> `24`〜`27` は `CApplication::OnCustomFrame()` を override し、フレーム描画を
...
3. `main.cpp` で `CApplication` を継承し、`OnStart` / `OnUpdate` / `OnRender` /
   `OnShutdown` を実装する（[`../docs/QUICKSTART.md`](../docs/QUICKSTART.md) 参照）。
4. 描画系のヘッダは、個別に並べる代わりに **`#include "render/Render.h"`** 一行で
   RHI と高レベルヘルパ（`CStandardShader` / `CSpriteBatch` / `FFont` など）を
