...
//
// 学習ポイント:
//   ・FSparseSet ベースの O(1) Add/Remove
//   ・FWorld::Query<>().EachParallel(fn, grain) で並列処理。fn は Entity ごとに
//     別スレッドで独立に呼ばれるので、共有資源には触らないこと
//   ・FMessageBroker はシングルスレッド前提の同期 pub/sub。よって publish は
...
void HelloECSApp::OnStart() noexcept {
    ...
    // ここから FMessageBroker::Publish (シングルスレッド契約) を呼んで安全。
...
void HelloECSApp::OnUpdate(f32 dt) noexcept {
    ...
    // ラムダ内は別スレッドから呼ばれるので、外部の共有資源 (FMessageBroker /
    ...
    GetWorld().Query<Position, Velocity>().EachParallel(
        [dt, sw, sh](FEntityId, Position& p, Velocity& v){
            p.v.x += v.v.x * dt;
            p.v.y += v.v.y * dt;
...
void HelloECSApp::OnRender() noexcept {
    ...
    GetWorld().Query<Position, FColor>().Each(
        [this, r](FEntityId, Position& p, FColor& c){
