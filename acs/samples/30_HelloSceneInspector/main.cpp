...
//
// 動作:
//   ・GameFramework Pillar A の FScene 上に複数階層の FNode2D ツリーを構築:
//        root
//        ├── wheel        (自転、spoke の親)
//        │   ├── spoke[0] (オフセット)
//        │   └── spoke[1] (オフセット)
//        └── player       (IInspectableProvider 実装、Inspector で field 編集)
//   ・`FHierarchyPanel` が "FScene Hierarchy" window でツリーを描画。クリックで
//     `FSelectionService` に選択を伝播。
//   ・`FInspectorPanel` が "Inspector" window で `FInspectorSeam` 経由で
//     選択 FNode の Provider field を編集。
//   ・`FEditorToolbar` が "Editor Toolbar" window で Play/Pause/Step 等の
//     コマンドを発行。
//   ・main menu bar "File > Save FScene / Load FScene" は永続化フックを呼び出す
...
//   SceneInspectorScene.{h,cpp}   - FNode2D ツリー + PanelLayout を持つ FScene
//   SceneInspectorApp.{h,cpp}     - FGame 派生クラス (ImGui lifecycle ラッパ)
//
// 必須バックエンド: ACS_RENDER_DX12_RAW (samples/21_HelloImGui /
// 29_HelloParticleEditor と同じ理由で、FImGuiCtx が DX12 raw backend 経由のため)。
