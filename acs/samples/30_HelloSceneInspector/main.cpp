...
//   ・`AHierarchyPanel` が "Scene Hierarchy" window でツリーを描画。クリックで
//     `CSelectionService` に選択を伝播。
//   ・`AInspectorPanel` が "Inspector" window で `CInspectorSeam` 経由で
//     選択 Node の Provider field を編集。
//   ・`AEditorToolbar` が "Editor Toolbar" window で Play/Pause/Step 等の
//     コマンドを発行。
...
//
// 構成:
//   main.cpp                      - ACS_GAME_MAIN(CSceneInspectorApp) のみ
//   SceneNodes.{h,cpp}            - APlayerNode (Provider 実装) + AWheelNode
//   PanelLayout.{h,cpp}            - 4 panel + Selection + Seam の束ね
//   SceneInspectorScene.{h,cpp}    - ANode ツリー + CPanelLayout を持つ Scene
//   SceneInspectorApp.{h,cpp}      - CGame 派生クラス (ImGui lifecycle ラッパ)
//
...
ACS_GAME_MAIN(helloscene::CSceneInspectorApp)
