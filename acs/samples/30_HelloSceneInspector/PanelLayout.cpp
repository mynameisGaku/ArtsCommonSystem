...
// HelloSceneInspector — CPanelLayout 実装。
...
void CPanelLayout::Init() noexcept {
    m_Seam.Init();
    m_Selection.Init();
    ...
    // AEditorToolbar の最小契約: Init() で内部状態を default に戻す
    // (Play/Pause/Step トグルは内部で持つ)。
    m_Toolbar.Init();
}

void CPanelLayout::Shutdown() noexcept {
...
void CPanelLayout::RegisterProvider(acs::game::FNodeId node_id,
...
void CPanelLayout::SelectInitial(acs::game::FNodeId id) noexcept {
    m_Selection.SelectNode(id);
}

void CPanelLayout::DrawUI(acs::game::CGame& game, acs::game::ANode& root) noexcept {
    // AEditorPanel 基底に乗っているため、フレーム毎に依存を bind してから no-arg DrawUI。
    ...
    // selection は Hierarchy 側が CSelectionService に書き、Inspector が読む。
