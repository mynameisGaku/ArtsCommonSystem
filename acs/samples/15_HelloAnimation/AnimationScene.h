// SPDX-License-Identifier: Apache-2.0
// HelloAnimation — シーン描画ロジック + スキンメッシュ手続き生成。
// CApplication からリソース参照を受け取り「CSky → 地面 → スキンメッシュ」を 1
// フレーム分まとめて描く。状態は持たず純粋な描画ユーティリティに近い。
#pragma once

#include "Types.h"

#include "asset/SkinnedMesh.h"
#include "math/Camera.h"
#include "math/Mat.h"
#include "memory/SharedPtr.h"
#include "render/IRhiCommandList.h"
#include "render/RenderAssets.h"
#include "render/SkinnedShader.h"
#include "render/Sky.h"
#include "render/StandardShader.h"

namespace helloanim {

class CAnimationScene {
public:
    // 4 ボーンの「ヘビ風」円柱メッシュを手続き生成 (アニメ含む)。
    // 失敗時は空の TSharedPtr を返す (呼び出し元で null チェック)。
    static acs::TSharedPtr<acs::ASkinnedMeshAsset> BuildSnake() noexcept;

    // 1 フレームの描画 (CSky → 地面 → スキンメッシュ)。
    //   sky        : 描画する CSky (App 所有、SunDirection / SunColor 取得用)
    //   std_shader : 地面用 CStandardShader
    //   skin_shader: スキンメッシュ用 CSkinnedShader (palette を流す)
    //   cl         : 現在の Render Pass の CommandList
    //   camera     : ViewProjection / Eye 取得用
    //   plane      : 床 (PrimitivePlane の GPU mesh)
    //   snake_gpu  : スキンメッシュ GPU 表現
    //   palette    : 現在のボーンパレット (bones の世界変換)
    //   palette_n  : palette の有効ボーン数
    void Render(acs::CSky&                sky,
                acs::CStandardShader&     std_shader,
                acs::CSkinnedShader&      skin_shader,
                acs::IRhiCommandList&    cl,
                const acs::CCamera&       camera,
                const acs::FGpuMesh&      plane,
                const acs::FSkinnedGpuMesh& snake_gpu,
                const acs::FMat4*         palette,
                acs::u32                 palette_n) noexcept;
};

} // namespace helloanim
