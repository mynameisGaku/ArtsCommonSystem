...
//   HelloAnimationApp.{h,cpp}   - FApplication 派生クラス (resource 所有 + 入力)
//
// 動作:
//   ・4 ボーンの長い円柱を生成 (ヘビ風)
//   ・各ボーンの回転を sin 波でアニメーションしてうねうね動く
//   ・地面 + 1 灯ライティング + FSky で雰囲気付け
//
// 操作:
//   ← →   : カメラ周回
//   Space  : 再生 / 一時停止
...
//
// 学習ポイント:
//   ・FSkinnedMeshAsset / FBone / FAnimation の構築
//   ・FSkinnedShader によるスキニング描画
//   ・FAnimationPlayer で時刻 → ボーンパレット
//   ・GPU スキニング: VS で 4 ボーンの加重平均
//
// ACS_DEFINE_MAIN は HelloAnimationApp を main エントリに登録 (FApplication
