...
ACS_TEST(GameFixedRuntimeInput, LegacyOrbitCameraInterpolatesBetweenFixedTicks)
{
    ...
    scene->SetFreeCameraEnabled(false);
    /** 自由camera無効化時に即座に表示する現在camera位置。 */
    const FVec3 free_camera_disabled = scene->Camera().Eye();
    EXPECT_TRUE(free_camera_disabled.z > before.z + 0.50f);
    EXPECT_TRUE(free_camera_disabled.z < before.z + 0.60f);
    game.UpdateForTest(0.0625f);
    /** 無効中に古い補間区間を繰り返さない次tick境界のcamera位置。 */
    const FVec3 disabled_boundary = scene->Camera().Eye();
    EXPECT_NEAR(disabled_boundary.x, free_camera_disabled.x, 1.0e-5f);
    EXPECT_NEAR(disabled_boundary.y, free_camera_disabled.y, 1.0e-5f);
    EXPECT_NEAR(disabled_boundary.z, free_camera_disabled.z, 1.0e-5f);
    EXPECT_EQ(source.CaptureCount(), 2u);

    scene->SetFreeCameraEnabled(true);
