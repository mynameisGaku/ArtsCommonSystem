...
namespace {
EBtStatus RetSuccess(void*, f32) noexcept { return EBtStatus::Success; }
EBtStatus RetFailure(void*, f32) noexcept { return EBtStatus::Failure; }
EBtStatus RetRunning(void*, f32) noexcept { return EBtStatus::Running; }
} // namespace
...
ACS_TEST(BehaviorTree, CompareVar) {
    struct Bb { f32 hp; i32 ammo; bool alive; };
    Bb bb{ 25.0f, 3, true };
    ...
    EXPECT_TRUE (BtCompareVar(&bb, static_cast<u32>(offsetof(Bb, alive)), EBtVarType::Bool, EBtCompareOp::NotEqual, 0.0f));
...
ACS_TEST(BtSerialize, DynamicBlackboardRoundTrip) {
    ...
    // save: 3 種の変数 + Compare ノードを書き出す。
    {
        ...
        panel.Init();
        FBtBlackboard bb;
        bb.Add("hp",    EBtVarType::F32);  bb.SetF32 ("hp",    42.5f);
        bb.Add("ammo",  EBtVarType::I32);  bb.SetI32 ("ammo",  7);
        bb.Add("alive", EBtVarType::Bool); bb.SetBool("alive", true);
        panel.SetDynamicBlackboard(&bb);
