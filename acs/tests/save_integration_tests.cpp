...
// SaveArchive 上位統合: TSaveSlot / CProgression の transactional contract 回帰テスト
#include "test/Test.h"
...
void RegisterTwoMilestones(CProgression& progression) noexcept
{
...
ACS_TEST(SaveIntegration, SaveSlotSizeMismatchDoesNotChangeDirectHelperOutput)
{
    ...
    EXPECT_TRUE(CSaveArchive::WriteToFile(path.Get(), 1u,
...
ACS_TEST(SaveIntegration, ProgressionEntryLimitIsRejectedBeforeMutation)
{
    ...
    u8 payload[8] = {};
    WriteU32LE(payload + 0, 100u);
    WriteU32LE(payload + 4, CProgression::kMaxPersistedMilestones + 1u);
    EXPECT_TRUE(CSaveArchive::WriteToFile(path.Get(), 1u,
                                          payload, sizeof(payload)).IsOk());
...
ACS_TEST(SaveIntegration, ProgressionCrcFailurePreservesCurrentState)
{
    ...
    EXPECT_TRUE(PatchFileBytes(path.Get(), CSaveArchive::kHeaderSize,
                               &corrupt_xp, 1u));
