...
ACS_TEST(CollisionWorld2D, BasicOverlapAndRemove) {
    FCollisionWorld2D w;
    w.Init(64.0f);
    const FShapeId box  = w.AddAabb(Aabb2{ {100, 0}, {16, 16} });
    const FShapeId ball = w.AddCircle(Circle{ {0, 0}, 16.0f });
    EXPECT_EQ(w.ShapeCount(), 2u);

    TArray<FShapeId> hits;
    w.OverlapAabb(Aabb2{ {0, 0}, {8, 8} }, hits);
...
ACS_TEST(CollisionWorld2D, HugeShapeGoesToOverflowListAndIsStillFound) {
    ...
    const FShapeId huge  = w.AddAabb(Aabb2{ {0, 0}, {1e30f, 1e30f} });
    const FShapeId small = w.AddAabb(Aabb2{ {10, 10}, {4, 4} });

    // 小さいクエリでも巨大形状は退避リスト経由で必ず候補に入る。
    TArray<FShapeId> hits;
    w.OverlapAabb(Aabb2{ {10, 10}, {1, 1} }, hits);
    ...
    RayHit2 rh;
    FShapeId hit_id;
    EXPECT_TRUE(w.Raycast(Ray2{ {10, -50}, {0, 1} }, 100.0f, rh, hit_id));
...
ACS_TEST(CollisionWorld2D, HugeQueryFallsBackToLinearScan) {
    FCollisionWorld2D w;
    w.Init(64.0f);
    const FShapeId a = w.AddAabb(Aabb2{ { 1000,  1000}, {8, 8} });
    const FShapeId b = w.AddAabb(Aabb2{ {-1000, -1000}, {8, 8} });

    // クエリ範囲が巨大でも全 slot 線形走査で完走し、全形状を正しく返す。
    TArray<FShapeId> hits;
    w.OverlapAabb(Aabb2{ {0, 0}, {1e30f, 1e30f} }, hits);
    EXPECT_TRUE(Contains(hits, a));
    EXPECT_TRUE(Contains(hits, b));
    EXPECT_EQ(hits.Size(), 2u);

    // 巨大円クエリも同様に完走する。
    w.OverlapCircle(Circle{ {0, 0}, 1e30f }, hits);
    EXPECT_TRUE(Contains(hits, a));
    EXPECT_TRUE(Contains(hits, b));
}
