...
public static class ProjectManager
{
    ...
    private static readonly HashSet<string> EngineBaseClasses = new() { "FComponent2D", "FNode2D", "FScene2D" };
    ...
    public static System.Collections.Generic.List<string> GenerateClass(Project p, string className, string baseClass)
    {
        ...
        string rootEngine = baseIsEngine ? baseClass : ResolveRootEngineBase(p, baseClass);   // 連鎖を辿る
        bool isComponent  = rootEngine == "FComponent2D";   // 直/間接に FComponent2D 由来か
        ...
        if (baseClass == "FComponent2D")   // 直接の FComponent2D 派生のみ Kind を宣言
        {
        ...
        else if (isComponent)   // ユーザーコンポーネントの派生 (Kind は基底から継承)
        {
            h.Append("    // 基底のコンポーネント挙動を継承。プロパティ追加は ACS_PROPERTY()。\n");
            h.Append("    ACS_PROPERTY() float value = 0.0f;\n\n");
            h.Append("    void OnUpdate(acs::f32 dt) noexcept override { (void)dt; }\n");
        }
