...
[T3]::acs_editor_set_view3d($h, 1)               # seed default (床+立方体+球 = 3)
Check "seeded 3 nodes" ([T3]::acs_editor_node3d_count($h) -eq 3)

$id = [T3]::acs_editor_add_node3d($h, 1, "Ball2") # sphere
Check "add returns id" ($id -ge 0)
Check "now 4 nodes" ([T3]::acs_editor_node3d_count($h) -eq 4)
...
$txt = Utf8Z $buf
Check "serialize has ACS3D header" ($txt.Contains("ACS3D"))
Check "serialize has Ball2" ($txt.Contains("Ball2"))

$h2 = [T3]::acs_editor_create()
[void][T3]::acs_editor_scene3d_load_text($h2, $txt)
Check "loaded 4 nodes" ([T3]::acs_editor_node3d_count($h2) -eq 4)
# 同じ id の transform が往復で保たれる
