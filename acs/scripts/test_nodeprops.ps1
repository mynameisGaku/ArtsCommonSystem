...
[Np]::acs_editor_node_set_base($h,$id,99.0)   # 再設定 (往復テスト用)
...
Check "NFLG line emitted" (($txt -split "`n") | Where-Object { $_ -eq "NFLG 4 0 0 7" })
...
[Np]::acs_editor_destroy($h); [Np]::acs_editor_destroy($h2)
