...
# 既定カメラ (screen==world) で検証用ノードの中心を矩形に入れて選択を確認する。
param(
...
try {
    ...
    $hasHost = $true
    Check "new host node count is zero" ([Bx]::acs_editor_node_count($h) -eq 0)
    Check "new host selection is empty" ([Bx]::acs_editor_selection_count($h) -eq 0)
    Check "new host primary is absent" ([Bx]::acs_editor_selected($h) -eq -1)
    if ([Bx]::acs_editor_scene_load_text($h, $scene) -ne 1) {
        throw "acs_editor_scene_load_text failed"
    }
    Check "fixture contains five nodes" ([Bx]::acs_editor_node_count($h) -eq 5)
    Check "fixture selection is explicitly empty" ([Bx]::acs_editor_selection_count($h) -eq 0)
    Check "fixture primary is explicitly absent" ([Bx]::acs_editor_selected($h) -eq -1)

# 検証点: 1=(320,230)、2=(372,230)、3=(282,254)、4=(520,150)、5=(180,330)。
